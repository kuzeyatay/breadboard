...
test("real browser downloads, panel actions and interrupted downloads survive Electron restarts", () => {
  const desktop = path.resolve(__dirname, "../..");
  const fixture = path.join(desktop, "tests/fixtures/browser-downloads.cjs");
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bb-downloads-restart-"));
