...
def main():
    ...
    with torch.no_grad():
        for batch in tqdm(inference_loader):
            ...
            # Use a run-local temporary file. The former hard-coded /tmp path
            # failed on Windows and let concurrent runs overwrite each other.
            tmp_output_path_mesh = output_dir / (f".{batch['name'][0]}.obj")
            try:
                ...
                tmp_output_path_mesh.unlink(missing_ok=True)
