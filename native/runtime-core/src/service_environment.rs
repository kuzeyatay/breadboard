...
fn write_cliproxy_runtime_config(
    ...
    let management_key = serde_json::to_string(management_key)
        .map_err(TrustedServiceEnvironmentError::InvalidDesktopConfig)?;
    // Do not use Rust's `\` line continuation for this YAML. It removes all
    // whitespace at the beginning of the next source line, including the two
    // spaces that make these mappings children of their parent keys. A flat
    // `secret-key` leaves CLIProxy's remote-management block unset, so every
    // authenticated management request is deliberately answered with 404.
    let config = format!(
        concat!(
...
fn build_voicebox_environment(
    ...
    // Python's sqlite3 driver cannot open the Windows verbatim (`\\?\`) path
    // retained by Runtime's canonical data-root pin. Present the already-
    // trusted child operands in their normal absolute spelling, just as the
    // Scriberr SQLite service below requires.
    let data = child_argv_path_presentation(&paths.data_root().join("runtime").join("voicebox"))?;
    builder.insert("PYTHONUNBUFFERED", "1")?;
    builder.insert("PYTHONDONTWRITEBYTECODE", "1")?;
...
fn build_deep_research_environment(
    ...
    builder.insert("OPENAI_BASE_URL", endpoints.chatmock_v1_url())?;
    builder.insert("OPENAI_API_KEY", "local")?;
    // The sidecar's own provider selection reads `CHATMOCK_BASE_URL` and
    // `CHATMOCK_API_KEY`; with only the OPENAI_* names above it started with
    // "model: none/unset", answered health with `ready: false`, and every
    // Max Research drive lost its broadest participant to "running but not
    // configured to answer". Hand it both spellings of the same gateway.
    builder.insert("CHATMOCK_BASE_URL", endpoints.chatmock_v1_url())?;
    builder.insert("CHATMOCK_API_KEY", "local")?;
    builder.insert(
        "CHATMOCK_MODEL",
        product_environment_value(os_environment, "CHATMOCK_MODEL")
...
mod tests {
    ...
    #[test]
    fn cliproxy_runtime_config_preserves_nested_yaml_mappings() {
        let (_temporary, paths, _config, _os_environment) = fixture();
        write_cliproxy_runtime_config(
            &paths,
            &endpoints(),
            "api-key-0123456789-abcdef",
            "management-key-0123456789-abcdef",
        )
        .unwrap();

        let config = fs::read_to_string(paths.data_root().join("cliproxy/config.yaml")).unwrap();
        assert!(config.contains("\napi-keys:\n  - \"api-key-0123456789-abcdef\"\n"));
        assert!(config.contains(
            "\nremote-management:\n  allow-remote: false\n  secret-key: \"management-key-0123456789-abcdef\"\n  disable-control-panel: true\n",
        ));
        assert!(config.contains(
            "\nquota-exceeded:\n  switch-project: true\n  switch-preview-model: false\n",
        ));
    ...
    #[test]
    fn voicebox_service_presents_sqlite_compatible_paths_to_child() {
        let (_temporary, paths, config, os_environment) = fixture();
        let services = TrustedServiceEnvironmentSet::load(
            RuntimeMode::Packaged,
            &paths,
        ...
        let voicebox = values(
            &services
                .prepare_for_launch_profile(
                    ...
                    &launch_profile(
                        RuntimeMode::Packaged,
                        TrustedServiceEnvironmentSource::Voicebox,
                    ),
                )
                .unwrap(),
        );
        let presented_data =
            child_argv_path_presentation(&paths.data_root().join("runtime").join("voicebox"))
                .unwrap();

        assert_eq!(
            voicebox["VOICEBOX_DATA_DIR"],
            presented_data.to_string_lossy()
        );
        assert_eq!(
            voicebox["VOICEBOX_MODELS_DIR"],
            presented_data.join("models").to_string_lossy()
        );
        assert_eq!(
            voicebox["VOICEBOX_STATUS_PATH"],
            presented_data.join("startup-status.json").to_string_lossy()
        );
        #[cfg(windows)]
        for name in [
            "VOICEBOX_DATA_DIR",
            ...
            assert!(!voicebox[name].starts_with(r"\\?\"));
        }
    }
