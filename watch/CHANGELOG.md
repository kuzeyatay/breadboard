...
### Changed
- Renamed the local repository from `claude-video` to `watch`.
- Added a Breadboard prebuilt capability contract and guarded `watch_run` path for ChatMock-backed agents.
- Replaced Claude-specific tool assumptions with runtime-neutral image-reading guidance.
- Added `agents/openai.yaml` metadata for compatible skill catalogs.
