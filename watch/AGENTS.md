...
Agent Skills package that gives an image-capable agent a video input. It supports Breadboard's ChatMock-backed Terminal, Codex, Claude Code, Cursor, GitHub Copilot, and other [Agent Skills](https://agentskills.io) hosts. Pure-stdlib Python orchestrates `yt-dlp`, `ffmpeg`, and an optional Whisper API.
...
| Surface | Install |
|---------|---------|
| Claude Code | `/plugin marketplace add bradautomates/claude-video` then `/plugin install watch@watch` |
