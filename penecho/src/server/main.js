...
// Breadboard: origins allowed to frame this server. Upstream always sends
// `frame-ancestors 'none'`, which makes an embedded canvas impossible; the
// Breadboard whiteboard card needs the garden page to be an allowed ancestor.
// Unset (the default) keeps upstream's behaviour byte for byte.
const FRAME_ANCESTORS = normalizeFrameAncestors(process.env.PENECHO_FRAME_ANCESTORS);
...
// Breadboard: a comma/space separated origin allowlist for CSP frame-ancestors.
// Each entry must be a bare origin (scheme + host + optional port); anything
// else is dropped rather than passed through into the policy.
function normalizeFrameAncestors(value) {
  const origins = String(value || "")
    ...
    .flatMap(entry => {
      let url;
      try { url = new URL(entry); } catch { return []; }
...
const server = http.createServer(async (req, res) => {
  ...
  if ((req.method === "GET" || req.method === "HEAD") && url.pathname === "/widget-host.html") {
    ...
    // Breadboard: when the canvas itself is framed, the widget host's ancestor
    // chain includes the embedding page, so the embedder has to be allowed here
    // too or every rendered widget is blocked inside an embedded canvas.
    const file = path.join(PUBLIC, "widget-host.html"), policy = `default-src 'none'; script-src 'self' 'unsafe-inline' 'unsafe-eval' 'wasm-unsafe-eval' https:; style-src 'unsafe-inline' https:; connect-src 'self' https:; img-src data: blob: https:; font-src data: https:; media-src data: blob: https:; frame-src 'self' blob:; worker-src blob: https:; object-src 'none'; form-action 'none'; base-uri 'none'; frame-ancestors 'self'${parentOrigin ? ` ${parentOrigin}` : ""}${FRAME_ANCESTORS ? ` ${FRAME_ANCESTORS}` : ""}`;
