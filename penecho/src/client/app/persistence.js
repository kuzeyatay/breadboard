// Canvas snapshots, export, drawing history, strokes, and lasso selection.
  ...
  function canvasIsEmpty() {
    return !tiles.size && !state.images.length && !state.textBoxes.length && (!pluginEnabled("animation") || !state.animations.length) && !visibleWidgets().length;
  }
  // `createId` stores under a caller-chosen id instead of a generated one, and
  // `quiet` skips the status line, the name field and the snapshot list refresh.
  // Both exist for the Breadboard board binding, whose id is fixed by the card
  // that hosts it and which writes on a timer rather than on a button press.
  async function saveSnapshot({ overwriteId = null, createId = null, name = null, location = state.snapshotLocation, quiet = false } = {}) {
    if (selectionAIBusy()) {
    ...
    const nameInput = document.querySelector("#historyName"),
      ...
      id = overwriteId || createId || `${Date.now()}-${crypto.randomUUID?.() || Math.random().toString(36).slice(2)}`,
      createdAt = Date.now(),
  ...
  function save() {
    ...
    state.future = [];
    // Every committed edit passes through here, which makes it the one place a
    // bound Breadboard board has to watch. A no-op unless a board is bound.
  ...
  function applyHistory(entry, side) {
    ...
    render();
    // Undo and redo change the canvas without committing a new history entry.
    breadboardBoardChanged();
  }
  function undo() {
    save();
