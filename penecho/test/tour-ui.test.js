...
test("feature tour persists seen ids, supports replay, and repositions accessibly", () => {
  ...
  // Still started from the bootstrap's double rAF, now skipped for an embedded
  // Breadboard board, which opens straight into the canvas.
  assert.match(app, /requestAnimationFrame\(\(\) => requestAnimationFrame\(\(\) => \{\s*if \(!boardRequested\(\)\) maybeStartOnboarding\(\);\s*\}\)\)/);
