...
test("canvas file actions are in the top-right header and available in History", () => {
  ...
  assert.match(app, /saveSnapshot\(\{\s*overwriteId\s*=\s*null,\s*createId\s*=\s*null,\s*name\s*=\s*null,\s*location\s*=\s*state\.snapshotLocation,\s*quiet\s*=\s*false\s*\}/);
...
test("Save canvas exposes non-blocking progress and completion feedback", () => {
  ...
  // The Breadboard board binding added `createId` and `quiet`, and moved the
  // empty-canvas condition into canvasIsEmpty(); the guards themselves and their
  // order are what this pins.
  assert.match(app, /async function saveSnapshot\(\{ overwriteId = null, createId = null, name = null, location = state\.snapshotLocation, quiet = false \} = \{\}\) \{[\s\S]*?selectionAIBusy\(\)[\s\S]*?await finalizeCanvasForSnapshot\(\)[\s\S]*?if \(canvasIsEmpty\(\)\)/);
  assert.match(app, /function canvasIsEmpty\(\) \{\s*return !tiles\.size/);
