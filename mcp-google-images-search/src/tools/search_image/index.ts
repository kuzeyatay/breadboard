...
export const handler: ToolCallback<typeof schema> = async ({ count = 2, query, safe = 'off', startIndex }) => {
  ...
  return {
    ...
    content: [
      ...
      // Breadboard: the structured result (titles, thumbnails, context links)
      // is built above but upstream only returns the bare links. Breadboard's
      // chat renderer needs the metadata to draw the image grid + lightbox
      // attribution, so append it as one JSON content item.
      {
        type: 'text' as const,
        text: JSON.stringify({ imageResults: result }),
      },
    ],
  };
};
