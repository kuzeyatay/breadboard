...
// Breadboard patch: `.pathname` of a Windows file URL is `/C:/…`, which Node
// then resolves against the cwd into `C:\C:\…` and fails to load. fileURLToPath
// is the same value on POSIX and the correct drive path on Windows.
const tscBin = fileURLToPath(new URL(require('typescript/package.json').bin.tsc, pathToFileURL(tsPkgPath)));
