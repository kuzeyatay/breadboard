...
description: Create and verify ASCII art, text-only visualizations, or text-based diagrams in any chat response or file. Use whenever a task asks for or would produce an ASCII diagram, ASCII visualization, text diagram, box-and-arrow layout, text flowchart, text architecture map, text sequence diagram, or similar monospaced visual. Follow the PLAN, DRAW, and VERIFY workflow before presenting the diagram. Do not use for Mermaid, SVG, raster images, or ordinary prose/code that is not a visual diagram.
license: MIT
metadata:
...
Breadboard's reviewed copy of `jasnell/opencode-skill-ascii-art-diagrams`.
The upstream PLAN, DRAW, and VERIFY procedure below remains authoritative. This
adapter only maps its project-directory assumptions onto Breadboard's two chat
surfaces.
...
## Delivering the diagram in Breadboard

- Return the finished diagram inline in a fenced `text` block. It is chat text,
  not an HTML artifact, image, Mermaid block, or source-code implementation.
- Treat every request to "confirm" the workflow as an internal checklist for
  the current turn. Do not make the user approve routine PLAN, DRAW, or VERIFY
  transitions unless the requested structure itself is genuinely ambiguous.
- In the authenticated Terminal, use the bundled `scripts/grid.py` and
  `scripts/verify.py` when the selected skill directory is available to the
  command executor. Garden deliberately has no shell; there, complete every
  equivalent manual verification check in the VERIFY phase. Never claim the
  Python verifier ran unless its successful output was actually observed.
- The server selects this skill for explicit ASCII, plain-text, terminal-style,
  or monospaced diagram intent before it considers the rendered Diagram Design
  skill. Generic diagram requests still belong to Diagram Design.
