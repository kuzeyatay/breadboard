#!/usr/bin/env bash
# build-skill.sh — package the watch skill as a portable .skill archive.
...
# well under the common 200-file cap, with no post-hoc stripping.
set -euo pipefail
...
if [ "$COUNT" -gt 200 ]; then
  echo "error: $COUNT files in zip; the supported cap is 200" >&2
...
echo "install with any compatible Agent Skills host"
