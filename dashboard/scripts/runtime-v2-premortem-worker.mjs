...
export function validateRuntimeV2PremortemRequest(value) {
  if (
  ...
  ) fail("The canonical Premortem Runtime request is invalid.");
  return value;
}

export function validateRuntimeV2PremortemScope(value) {
  if (
  ...
  ) fail("The Premortem worker requires authenticated conversation scope.");
  return value;
}

export function validateRuntimeV2PremortemEnvironment(environment) {
  for (const name of [
    ...
    if (!boundedText(environment[name], 2_048) || !path.isAbsolute(environment[name])) {
      fail("The sealed Premortem runtime paths are unavailable.");
    }
  }
  const sitePackages = environment.BREADBOARD_PREMORTEM_SITE_PACKAGES;
  if (sitePackages !== undefined && sitePackages !== "" &&
    (!boundedText(sitePackages, 2_048) || !path.isAbsolute(sitePackages))) {
    fail("The sealed Premortem package path is invalid.");
  }
}
...
function sourceLayout(dataRoot) {
  ...
  const development = fs.existsSync(
    path.join(developmentSourceRoot, "lib", "hermes", "premortem-service.ts"),
  );
  ...
  for (const relativePath of [
    ...
    path.join("lib", "hermes", "premortem-request.ts"),
    path.join("lib", "hermes", "premortem-service.ts"),
  ]) {
    if (!fs.existsSync(path.join(sourceRoot, relativePath))) {
      fail("The staged Premortem worker source closure is unavailable.");
    }
  }
...
function workspaceDirectory(workspaceKey) {
  ...
  if (!pathWithin(root, candidate) || samePath(root, candidate)) {
    fail("The Premortem workspace escaped the sealed Hermes root.");
  }
  fs.mkdirSync(candidate, { recursive: true });
  const canonical = fs.realpathSync.native(candidate);
  if (!samePath(candidate, canonical) || !pathWithin(root, canonical)) {
    fail("The Premortem workspace is indirect or outside the sealed Hermes root.");
  }
  return canonical;
}

function boundedFailure(error) {
  const raw = error instanceof Error ? error.message : "Premortem failed.";
  const clean = raw.replace(/\p{Cc}+/gu, " ").trim() || "Premortem failed.";
  const bytes = Buffer.from(clean, "utf8");
  return (bytes.byteLength <= 32 * 1024 ? bytes : bytes.subarray(0, 32 * 1024))
    .toString("utf8")
    .replace(/\uFFFD+$/u, "") || "Premortem failed.";
}

export async function executeRuntimeV2Premortem(launch, signal, progress) {
  validateRuntimeV2PremortemRequest(launch.request);
  validateRuntimeV2PremortemScope(launch.executionScope);
  validateRuntimeV2PremortemEnvironment(process.env);
  ...
  const service = await import(
    pathToFileURL(path.join(sourceRoot, "lib", "hermes", "premortem-service.ts")).href
  );
  if (
    typeof service.runPremortem !== "function" ||
    typeof service.validatePremortemArguments !== "function"
  ) fail("The staged Premortem executor is unavailable.");

  try {
    const args = service.validatePremortemArguments(launch.request.arguments);
    if (JSON.stringify(args) !== JSON.stringify(launch.request.arguments)) {
      fail("The canonical Premortem arguments changed during worker validation.");
    }
    progress.checkpoint({ stage: "running", percent: 10 });
    const result = await service.runPremortem({
      arguments: args,
    ...
    return {
      ok: false,
      operation: "command",
      errorCode: /^[a-z][a-z0-9_]{0,127}$/u.test(candidate) ? candidate : "premortem_failed",
...
if (typeof process.argv[1] === "string" && samePath(process.argv[1], ENTRYPOINT)) {
  void runRuntimeV2FiniteMcpWorker({
    name: "runtime-v2-premortem-worker",
    validateRequest: validateRuntimeV2PremortemRequest,
    validateExecutionScope: validateRuntimeV2PremortemScope,
    expectedInputCount: () => 0,
    execute: executeRuntimeV2Premortem,
  });
}
