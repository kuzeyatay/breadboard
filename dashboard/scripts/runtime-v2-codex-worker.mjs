...
if (typeof process.argv[1] === "string" && path.resolve(process.argv[1]) === path.resolve(entrypoint)) {
  void runRuntimeV2OuterAgentWorker("codex").catch((error) => {
    process.exitCode = 1;
    process.stderr.write(`[runtime-v2-codex-worker] startup failed: ${error instanceof Error ? error.message : String(error)}\n`);
  });
}
