...
} from "./runtime-v2-background-executor.mjs";
...
function validateIdentity(value) {
  if (
    ...
    fail("The Runtime V2 background worker identity is invalid.");
  }
  return value;
}
...
function validateExecutionScope(value) {
  if (
    ...
    (value.userId !== null && (!Number.isSafeInteger(value.userId) || value.userId < 1)) ||
    value.gardenId !== null ||
    value.conversationId !== null
  ) {
    fail("The Runtime V2 background worker scope is invalid.");
  }
  return value;
}
...
function resolveDataPath(dataRoot, relativePath) {
  ...
  if (segments.length < 2 || segments.some((segment) => !segment || segment === "." || segment === ".." || segment.includes("\\"))) {
    fail("The background worker start manifest contains an invalid relative path.");
  }
  const resolved = path.resolve(dataRoot, ...segments);
  if (!pathWithin(dataRoot, resolved)) fail("The background worker path escaped the data root.");
  return resolved;
}
...
function sourceLayout(dataRoot) {
  ...
  if (!fs.existsSync(path.join(sourceRoot, "lib", "db.ts"))) {
    fail("The staged Runtime V2 background source closure is unavailable.");
  }
...
export function loadRuntimeV2BackgroundLaunch(argv = process.argv.slice(2), launchDirectory = process.cwd()) {
  if (argv.length !== 1 || argv[0] !== START_MANIFEST_FILE) {
    fail("The Runtime V2 background worker requires exactly start.json.");
  }
  const canonicalLaunchDirectory = fs.realpathSync.native(path.resolve(launchDirectory));
  const manifest = readBoundedJson(path.join(canonicalLaunchDirectory, START_MANIFEST_FILE), MAX_START_MANIFEST_BYTES, "The worker start manifest");
  if (
    !hasExactKeys(manifest, ["protocolVersion", "identity", "executionScope", "inputManifestPath", "inputBlobs", "workspacePath", "checkpointPath", "resultPath"]) ||
    manifest.protocolVersion !== PROTOCOL_VERSION ||
    !Array.isArray(manifest.inputBlobs) ||
    manifest.inputBlobs.length !== 0
  ) {
    fail("The Runtime V2 background start manifest has an unsupported shape.");
  }
  ...
  for (const field of ["inputManifestPath", "workspacePath", "checkpointPath", "resultPath"]) {
    if (manifest[field] !== expected[field]) fail(`The background ${field} is not identity-bound.`);
  }
  let dataRoot = canonicalLaunchDirectory;
  for (let index = 0; index < expected.attemptRoot.split("/").length; index += 1) dataRoot = path.dirname(dataRoot);
  if (!samePath(canonicalLaunchDirectory, resolveDataPath(dataRoot, expected.attemptRoot))) {
    fail("The background launch directory is not identity-bound.");
  }
  ...
  if (!workspaceMetadata.isDirectory() || workspaceMetadata.isSymbolicLink()) {
    fail("The background worker workspace is unavailable.");
  }
  const request = readBoundedJson(resolveDataPath(dataRoot, manifest.inputManifestPath), MAX_INPUT_MANIFEST_BYTES, "The canonical background request");
  validateRuntimeV2BackgroundRequest(request, executionScope);
  return {
    dataRoot,
    identity,
    executionScope,
...
export function parseRuntimeV2BackgroundStopRecord(line) {
  if (typeof line !== "string" || Buffer.byteLength(line, "utf8") > MAX_STOP_RECORD_BYTES || !line.endsWith("\n") || line.slice(0, -1).includes("\n")) {
    fail("The Runtime V2 background stop record is invalid.");
  }
  let value;
  try { value = JSON.parse(line.slice(0, -1)); } catch { fail("The Runtime V2 background stop record is invalid JSON."); }
...
function startStopInput(onStop, onFault) {
  ...
  const onData = (chunk) => {
    ...
    if (Buffer.byteLength(buffered, "utf8") > MAX_STOP_RECORD_BYTES) {
      poisoned = true;
      onFault(new Error("The background stop record exceeded its bound."));
      return;
    }
    ...
    buffered = "";
    try {
      parseRuntimeV2BackgroundStopRecord(line);
      if (stopped || remainder) fail("The background worker received more than one stop record.");
      stopped = true;
      onStop();
    } catch (error) {
      poisoned = true;
  ...
  return { requested: () => stopped, close: () => { process.stdin.off("data", onData); process.stdin.pause(); } };
}

function writeResultOnce(resultPath, value) {
  const bytes = Buffer.from(`${JSON.stringify(value)}\n`, "utf8");
  if (bytes.byteLength < 1 || bytes.byteLength > MAX_RESULT_BYTES) fail("The background result exceeded its bound.");
  try { fs.lstatSync(resultPath); fail("The durable background result already exists."); } catch (error) { if (error?.code !== "ENOENT") throw error; }
...
export async function runRuntimeV2BackgroundWorker() {
  console.log = (...values) => console.error(...values);
  console.info = (...values) => console.error(...values);
  const launch = loadRuntimeV2BackgroundLaunch();
  ...
  const events = createRuntimeV2WorkerEventWriter(launch.identity, { heartbeatStage: "background-task", heartbeatIntervalMs: HEARTBEAT_INTERVAL_MS });
  const abort = new AbortController();
  let fault = null;
  let heartbeat = null;
  let acknowledged = false;
  const acknowledge = () => {
    if (!acknowledged) { acknowledged = true; events.cancellationAcknowledged(); }
    abort.abort(new DOMException("Runtime cancellation requested", "AbortError"));
  };
  const stop = startStopInput(acknowledge, (error) => { fault = error; acknowledge(); });
  try {
    events.ready();
    if (stop.requested()) return;
    heartbeat = events.startHeartbeat();
    await heartbeat.ready;
    const result = await executeRuntimeV2BackgroundRequest({
    ...
    writeResultOnce(launch.resultPath, { protocolVersion: PROTOCOL_VERSION, identity: launch.identity, completionSequence, result });
    events.complete(launch.resultRelativePath);
  } catch (error) {
    if (!stop.requested()) {
      events.failed("BACKGROUND_WORKER_FAILED", "Runtime background work failed.");
      process.exitCode = 1;
      console.error("[runtime-v2-background-worker] Execution failed:", error);
    }
  } finally {
...
const invokedAsEntrypoint = typeof process.argv[1] === "string" && samePath(process.argv[1], ENTRYPOINT_PATH);
if (invokedAsEntrypoint) {
  void runRuntimeV2BackgroundWorker().catch((error) => {
    process.exitCode = 1;
    fs.writeSync(2, `[runtime-v2-background-worker] Startup failed: ${error instanceof Error ? error.message : String(error)}\n`);
  });
}
