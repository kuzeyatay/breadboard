...
import {
  ...
  writeRecallInstallStatus,
} from "./runtime-v2-recall-install-executor.mjs";
...
const MAX_INPUT_MANIFEST_BYTES = 4 * 1024;
...
function validateIdentity(value) {
  if (
    ...
    fail("The Recall install worker identity is invalid.");
  return value;
}
...
function validateExecutionScope(value) {
  if (
    ...
    fail("The Recall install worker scope is invalid.");
  return value;
}
...
function resolveDataPath(dataRoot, relativePath) {
  ...
  if (
    ...
    fail(
      "The Recall install start manifest contains an invalid relative path.",
    );
...
function validateRequest(value) {
  if (
    !hasExactKeys(value, ["protocolVersion", "action"]) ||
    value.protocolVersion !== PROTOCOL_VERSION ||
    value.action !== "install"
  )
    fail("The canonical Recall install request is invalid.");
  return value;
}

export function loadRuntimeV2RecallInstallLaunch(
  argv = process.argv.slice(2),
  launchDirectory = process.cwd(),
) {
  if (argv.length !== 1 || argv[0] !== START_MANIFEST_FILE) {
    fail("The Recall install worker requires exactly start.json.");
  }
  ...
  const manifest = readBoundedJson(
    ...
    "The Recall install worker start manifest",
  );
  if (
    ...
    fail("The Recall install worker start manifest has an unsupported shape.");
  ...
  for (const field of [
    ...
    if (manifest[field] !== expected[field]) {
      fail(`The Recall install ${field} is not identity-bound.`);
    }
  }
  ...
  if (
    ...
    fail("The Recall install launch directory is not identity-bound.");
  }
  ...
  if (!workspaceMetadata.isDirectory() || workspaceMetadata.isSymbolicLink()) {
    fail("The Recall install private workspace is unavailable.");
  }
  return {
    dataRoot,
    identity,
    ...
    request: validateRequest(
      readBoundedJson(
        ...
        "The canonical Recall install request",
      ),
    ),
...
export function parseRuntimeV2RecallInstallStopRecord(line) {
  if (
    typeof line !== "string" ||
    ...
    fail("The Recall install stop record is invalid.");
  let value;
  try {
    value = JSON.parse(line.slice(0, -1));
  } catch {
    fail("The Recall install stop record is invalid JSON.");
  }
  if (
...
function startStopInput(onStop, onFault) {
  ...
  const onData = (chunk) => {
    ...
    if (Buffer.byteLength(buffered, "utf8") > MAX_STOP_RECORD_BYTES) {
      poisoned = true;
      onFault(new Error("The Recall install stop record exceeded its bound."));
      return;
    }
    ...
    buffered = "";
    try {
      parseRuntimeV2RecallInstallStopRecord(line);
      if (stopped || remainder)
        fail("The Recall install worker received more than one stop record.");
      stopped = true;
      onStop();
    } catch (error) {
      poisoned = true;
...
function writeResultOnce(resultPath, value) {
  ...
  if (bytes.byteLength < 1 || bytes.byteLength > MAX_RESULT_BYTES) {
    fail("The Recall install result exceeded its bound.");
  }
  try {
    fs.lstatSync(resultPath);
    fail("The durable Recall install result already exists.");
  } catch (error) {
...
function writeInterruptedStatus(launch, startedAt) {
  const plan = buildRecallInstallPlan({ dataRoot: launch.dataRoot });
  writeRecallInstallStatus(plan.statusPath, {
    phase: "interrupted",
    message: "The capture engine install was interrupted.",
    startedAt,
  });
}

export async function runRuntimeV2RecallInstallWorker() {
  ...
  const acknowledge = () => {
    ...
    abort.abort(
      new DOMException("Runtime cancellation requested", "AbortError"),
    );
  };
  ...
  try {
    ...
    events.progress("preparing", 0, 2);
    const result = await executeRecallInstall({
      dataRoot: launch.dataRoot,
      signal: abort.signal,
    });
    events.progress("finalizing", 1, 2);
    await heartbeat.stop();
    heartbeat = null;
    await new Promise((resolve) => setImmediate(resolve));
    if (fault) throw fault;
    if (stop.requested()) return;
    events.progress("finalizing", 2, 2);
    ...
    if (!stop.requested()) {
      events.failed(
        "RECALL_INSTALL_FAILED",
        "The Recall capture engine could not be installed.",
      );
      process.exitCode = 1;
      console.error(
        "[runtime-v2-recall-install-worker] Install failed:",
        error,
      );
    }
  } finally {
    ...
    if (stop.requested()) {
      try {
        writeInterruptedStatus(launch, startedAt);
      } catch {
        /* Status remains honestly stalled. */
      }
    }
    stop.close();
  }
...
if (invokedAsEntrypoint) {
  void runRuntimeV2RecallInstallWorker().catch((error) => {
    process.exitCode = 1;
    fs.writeSync(
      2,
      `[runtime-v2-recall-install-worker] Startup failed: ${error instanceof Error ? error.message : String(error)}\n`,
      undefined,
      "utf8",
    );
  });
