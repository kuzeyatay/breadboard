...
function basicArguments(value) {
  return Array.isArray(value) &&
    ...
    Buffer.byteLength(value.join("\u0000"), "utf8") <= 8 * 1024;
}

export function validateRuntimeV2AgentLoopRequest(value) {
  if (
    ...
    fail("The canonical Agent Loop Runtime request is invalid.");
  }
  return value;
}

export function validateRuntimeV2AgentLoopScope(value) {
  if (
    ...
    fail("The Agent Loop worker requires authenticated conversation scope.");
  }
  return value;
}

export function validateRuntimeV2AgentLoopEnvironment(environment) {
  for (const name of [
    ...
    if (!boundedText(environment[name], 2_048) || !path.isAbsolute(environment[name])) {
      fail("The sealed Agent Loop runtime paths are unavailable.");
    }
  }
  const sitePackages = environment.BREADBOARD_AGENT_LOOP_SITE_PACKAGES;
  if (
    sitePackages !== undefined &&
    sitePackages !== "" &&
    (!boundedText(sitePackages, 2_048) || !path.isAbsolute(sitePackages))
  ) {
    fail("The sealed Agent Loop package path is invalid.");
  }
}
...
function sourceLayout(dataRoot) {
  ...
  const development = fs.existsSync(
    path.join(developmentSourceRoot, "lib", "hermes", "agent-loop-service.ts"),
  );
  ...
  for (const relativePath of [
    ...
    path.join("lib", "hermes", "agent-loop-request.ts"),
    path.join("lib", "hermes", "agent-loop-service.ts"),
  ]) {
    if (!fs.existsSync(path.join(sourceRoot, relativePath))) {
      fail("The staged Agent Loop worker source closure is unavailable.");
    }
  }
...
function workspaceDirectory(workspaceKey) {
  ...
  if (!pathWithin(root, candidate) || samePath(root, candidate)) {
    fail("The Agent Loop workspace escaped the sealed Hermes root.");
  }
  fs.mkdirSync(candidate, { recursive: true });
  const canonical = fs.realpathSync.native(candidate);
  if (!samePath(candidate, canonical) || !pathWithin(root, canonical)) {
    fail("The Agent Loop workspace is indirect or outside the sealed Hermes root.");
  }
  return canonical;
}

function boundedFailure(error) {
  const raw = error instanceof Error ? error.message : "The loop kit failed.";
  const clean = raw.replace(/\p{Cc}+/gu, " ").trim() || "The loop kit failed.";
  const bytes = Buffer.from(clean, "utf8");
  return (bytes.byteLength <= 32 * 1024 ? bytes : bytes.subarray(0, 32 * 1024))
    .toString("utf8")
    .replace(/\uFFFD+$/u, "") || "The loop kit failed.";
}

export async function executeRuntimeV2AgentLoop(launch, signal, progress) {
  validateRuntimeV2AgentLoopRequest(launch.request);
  validateRuntimeV2AgentLoopScope(launch.executionScope);
  validateRuntimeV2AgentLoopEnvironment(process.env);
  ...
  const service = await import(
    pathToFileURL(path.join(sourceRoot, "lib", "hermes", "agent-loop-service.ts")).href
  );
  if (
    typeof service.runAgentLoopKit !== "function" ||
    typeof service.validateAgentLoopRequestArguments !== "function"
  ) {
    fail("The staged Agent Loop executor is unavailable.");
  }

  try {
    const parsed = service.validateAgentLoopRequestArguments(launch.request.arguments);
    if (JSON.stringify(parsed.args) !== JSON.stringify(launch.request.arguments)) {
      fail("The canonical Agent Loop arguments changed during worker validation.");
    }
    progress.checkpoint({ stage: "running", percent: 10 });
    const result = await service.runAgentLoopKit({
    ...
    return {
      ok: false,
      operation: "command",
      errorCode: /^[a-z][a-z0-9_]{0,127}$/u.test(candidate)
        ? candidate
        : "agent_loop_failed",
...
if (typeof process.argv[1] === "string" && samePath(process.argv[1], ENTRYPOINT)) {
  void runRuntimeV2FiniteMcpWorker({
    name: "runtime-v2-agent-loop-worker",
    validateRequest: validateRuntimeV2AgentLoopRequest,
    validateExecutionScope: validateRuntimeV2AgentLoopScope,
    expectedInputCount: () => 0,
    execute: executeRuntimeV2AgentLoop,
  });
}
