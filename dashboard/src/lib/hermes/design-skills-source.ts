...
/** GitHub identity of the vendored design and animation skills pack. */
export const DESIGN_SKILLS_SOURCE = "emilkowalski/skills";
/** Human label used in inspection notices. */
export const DESIGN_SKILLS_LABEL = "emilkowalski/skills clone";
/** The pack keeps every skill in one tree. */
const SKILL_TREE_SUBPATH = "skills";
const MAX_SKILL_FILES = 200;
const MAX_SKILL_FILE_BYTES = 2_000_000;
const MAX_SKILL_TOTAL_BYTES = 10_000_000;

interface LocalDesignSkill {
  directory: string;
...
interface LocalDesignSnapshot {
  repository: string;
  revision: string;
  signature: string;
  skills: Map<string, LocalDesignSkill>;
}

export interface LocalDesignSyncResult {
  available: boolean;
  root: string | null;
...
let snapshotCache: LocalDesignSnapshot | null = null;
const appliedSnapshots = new WeakMap<SkillsCatalogStore, string>();

/**
 * Resolves the design-skills repository root. `DESIGN_SKILLS_ROOT` may point at
 * the repository itself or at its `skills/` tree; the repository root (the
 * directory containing `skills/`) is returned.
 */
export function localDesignSkillsRepository(): string | null {
  const configured = process.env.DESIGN_SKILLS_ROOT?.trim();
  const candidates = configured
    ? [path.resolve(configured), path.resolve(configured, "..")]
    : [path.join(repositoryRoot(), "emilkowalski-skills")];
...
export function synchronizeLocalDesignSkillsCatalog(input: {
  store?: SkillsCatalogStore;
  force?: boolean;
} = {}): LocalDesignSyncResult {
...
export function getLocalDesignSkill(upstreamId: string): {
  detail: SkillsShDetail;
  description: string | null;
  binaryPaths: string[];
  revision: string;
...
export function readLocalDesignSkillFiles(upstreamId: string): {
...
function loadSnapshot(repository: string, force: boolean): LocalDesignSnapshot {
  if (!force && snapshotCache?.repository === repository) return snapshotCache;
  const revision = readGitRevision(repository) ?? "local";
  const skills = new Map<string, LocalDesignSkill>();
  ...
  for (const entry of fs.readdirSync(tree, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    const id = `${DESIGN_SKILLS_SOURCE}/${slug}`;
    ...
    const record: CatalogSnapshotRecord = {
      ...
      installUrl: `https://github.com/${DESIGN_SKILLS_SOURCE}`,
      url: `https://github.com/${DESIGN_SKILLS_SOURCE}/tree/${revision}/${SKILL_TREE_SUBPATH}/${encodeURIComponent(slug)}`,
      duplicate: false,
      curated: false,
      ranks: {},
      slashCommand: `design:${slug}`,
    };
    skills.set(id, {
      directory,
      record,
...
function readSkillFiles(root: string): Array<{ relativePath: string; contents: Buffer }> {
  ...
  const visit = (directory: string) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) throw new Error(`Design skill contains a symbolic link: ${absolute}`);
      ...
      if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) {
        throw new Error(`Design skill file escapes its source directory: ${absolute}`);
      }
      const contents = fs.readFileSync(resolved);
      if (contents.byteLength > MAX_SKILL_FILE_BYTES) {
        throw new Error(`Design skill file exceeds the ${MAX_SKILL_FILE_BYTES}-byte review limit: ${relative}`);
      }
      totalBytes += contents.byteLength;
      if (totalBytes > MAX_SKILL_TOTAL_BYTES) {
        throw new Error(`Design skill exceeds the ${MAX_SKILL_TOTAL_BYTES}-byte review limit: ${root}`);
      }
      files.push({ relativePath: relative.replace(/\\/g, "/"), contents });
      if (files.length > MAX_SKILL_FILES) {
        throw new Error(`Design skill exceeds the ${MAX_SKILL_FILES}-file review limit: ${root}`);
      }
    }
  };
  visit(rootRealPath);
