...
export async function importArtifactImage(input: {
  ...
  // Keep generated bytes inside Breadboard-owned mutable storage. Windows temp
  // roots can be junctioned, scanned, or cleaned between the write and the
  // verified import boundary, which previously surfaced as a false 404.
  const stagingParent = path.join(dashboardDataDir(), "artifact-image-staging");
  fs.mkdirSync(stagingParent, { recursive: true });
  const stagingRoot = fs.mkdtempSync(path.join(stagingParent, "import-"));
  ...
  try {
    ...
    // A scanner can briefly retain a handle after the durable copy completes.
    // Cleanup is expendable and must not turn a verified artifact into a failed
    // tool call, so give Windows its bounded retry window and then leave the
    // unique staging directory for later housekeeping if it is still locked.
    try {
      ...
      // Best-effort cleanup only; the durable artifact is already independent.
    }
  }
}
