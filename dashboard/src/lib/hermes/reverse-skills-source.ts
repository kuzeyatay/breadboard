...
/** GitHub identity of the vendored reverse-engineering skills pack. */
export const REVERSE_SKILLS_SOURCE = "zhaoxuya520/reverse-skill";
/** Human label used in inspection notices. */
export const REVERSE_SKILLS_LABEL = "reverse-skill clone";
/**
 * The reverse pack keeps two skill trees: the routed reverse-engineering
 * modules under `skills/`, and the CTF competition sub-skills under
 * `CTF-Sandbox-Orchestrator/`. Both are ingested as one source.
 */
const SKILL_TREE_SUBPATHS = ["skills", "CTF-Sandbox-Orchestrator"] as const;
const MAX_SKILL_FILES = 200;
const MAX_SKILL_FILE_BYTES = 2_000_000;
const MAX_SKILL_TOTAL_BYTES = 10_000_000;

interface LocalReverseSkill {
  directory: string;
...
interface LocalReverseSnapshot {
  repository: string;
  revision: string;
  signature: string;
  skills: Map<string, LocalReverseSkill>;
}

export interface LocalReverseSyncResult {
  available: boolean;
  root: string | null;
...
let snapshotCache: LocalReverseSnapshot | null = null;
const appliedSnapshots = new WeakMap<SkillsCatalogStore, string>();

/**
 * Resolves the reverse-skill repository root that holds the skill trees.
 * `REVERSE_SKILLS_ROOT` may point at the repository itself or at either skill
 * tree; the repository root (the directory containing `skills/`) is returned.
 */
export function localReverseSkillsRepository(): string | null {
  const configured = process.env.REVERSE_SKILLS_ROOT?.trim();
  const candidates = configured
    ? [path.resolve(configured), path.resolve(configured, ".."), path.resolve(configured, "..", "..")]
    : [path.join(repositoryRoot(), "reverse-skill")];
...
export function synchronizeLocalReverseSkillsCatalog(input: {
  store?: SkillsCatalogStore;
  force?: boolean;
} = {}): LocalReverseSyncResult {
  const store = input.store ?? getSkillsCatalogStore();
  const repository = localReverseSkillsRepository();
  if (!repository) {
...
export function getLocalReverseSkill(upstreamId: string): {
  detail: SkillsShDetail;
  description: string | null;
  binaryPaths: string[];
  revision: string;
...
export function readLocalReverseSkillFiles(upstreamId: string): {
...
function loadSnapshot(repository: string, force: boolean): LocalReverseSnapshot {
  if (!force && snapshotCache?.repository === repository) return snapshotCache;
  const revision = readGitRevision(repository) ?? "local";
  const skills = new Map<string, LocalReverseSkill>();
  for (const subpath of SKILL_TREE_SUBPATHS) {
    ...
    for (const entry of fs.readdirSync(tree, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      const id = `${REVERSE_SKILLS_SOURCE}/${slug}`;
      // Slugs are unique across trees today; keep the first tree deterministic if that ever changes.
      ...
      const record: CatalogSnapshotRecord = {
        ...
        installUrl: `https://github.com/${REVERSE_SKILLS_SOURCE}`,
        url: `https://github.com/${REVERSE_SKILLS_SOURCE}/tree/${revision}/${subpath}/${encodeURIComponent(slug)}`,
        duplicate: false,
        curated: false,
        ranks: {},
        slashCommand: `reverse:${slug}`,
      };
      skills.set(id, {
        directory,
        record,
...
function readSkillFiles(root: string): Array<{ relativePath: string; contents: Buffer }> {
  ...
  const visit = (directory: string) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) throw new Error(`Reverse skill contains a symbolic link: ${absolute}`);
      ...
      if (contents.byteLength > MAX_SKILL_FILE_BYTES) {
        throw new Error(`Reverse skill file exceeds the ${MAX_SKILL_FILE_BYTES}-byte review limit: ${relative}`);
      }
      totalBytes += contents.byteLength;
      if (totalBytes > MAX_SKILL_TOTAL_BYTES) {
        throw new Error(`Reverse skill exceeds the ${MAX_SKILL_TOTAL_BYTES}-byte review limit: ${root}`);
      }
      files.push({ relativePath: relative.replace(/\\/g, "/"), contents });
      if (files.length > MAX_SKILL_FILES) {
        throw new Error(`Reverse skill exceeds the ${MAX_SKILL_FILES}-file review limit: ${root}`);
      }
    }
  };
  visit(rootRealPath);
