// Surface-independent core of the shared Hermes dispatch pipeline.
//
...
import type { HermesSurface, HermesMode } from "./config.ts";
...
export function prepareTurn(input: PrepareTurnInput): PreparedTurn {
  ...
  // Super agent widens the plan, never the broker's rules: an isolated session
  // gets nothing extra, and the elevated classes are brokered like any other.
  const plan =
    input.superAgent === true && !isolated
      ? elevateForSuperAgent(planned)
      : planned;
  ...
  return {
    ...
    // Pause for every missing filesystem grant that names a folder, whether the
    // target came from this sentence or from a verified earlier turn. Only the
    // user can supply a grant, so dispatching an under-authorized runtime just
    // spends a turn on the model explaining its own permissions. An untargeted
    // request has no folder to ask about, so it goes to the model to clarify.
    blocked:
      !grant.executable ||
      grant.pendingPermissions.some(
        (permission) =>
...
/**
 * Decide whether a failed Hermes turn may fall back to the legacy
 * ChatMock pipeline.
 *
...
export function decideFallback(
  ...
  if (mode === "required") {
    return {
      allowed: false,
      reason:
        "HERMES_MODE=required: the runtime failed and legacy ChatMock must not silently answer in its place.",
      stage,
    };
  }
...
export interface TurnDiagnostics {
  ...
  backend: "hermes" | "legacy_chatmock";
...
export function buildDiagnostics(
  ...
  return {
    ...
    hermesSessionId: context.hermesSessionId ?? null,
    ...
    backend: context.fallbackUsed ? "legacy_chatmock" : "hermes",
