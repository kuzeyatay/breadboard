...
export async function reviewExplanation(input: {
  ...
  if (!isExplanationCandidate(input.userRequest) || !input.answer.trim()) return finish();
  ...
  try {
    ...
    if (report.coverage.some(row => row.status === "uncertain")) {
    ...
    if (report.coverage.every(row => row.status === "covered" || row.status === "not_required")) {
      report.status = "reviewed";
      report.reason = "The reviewer found the required mechanisms in the answer.";
      return finish();
    }
    const required = report.obligations.filter(item => report.coverage.find(row => row.id === item.id)?.status !== "not_required");
    const repair = await call("repair", EXPLANATION_REPAIR_PROMPT,
      { ...packet, obligations: required, draft: input.answer, coverage: report.coverage.filter(row => row.status !== "not_required") }, repairSchema, 8_192);
