...
/** GitHub identity of the vendored Hallmark anti-slop design skill. */
export const HALLMARK_SKILLS_SOURCE = "nutlope/hallmark";
/** Human label used in inspection notices. */
export const HALLMARK_SKILLS_LABEL = "nutlope/hallmark clone";
/** The clone keeps its skill tree beside the demo site and docs. */
const SKILL_TREE_SUBPATH = "skills";
const MAX_SKILL_FILES = 200;
const MAX_SKILL_FILE_BYTES = 2_000_000;
const MAX_SKILL_TOTAL_BYTES = 10_000_000;

interface LocalHallmarkSkill {
  directory: string;
...
interface LocalHallmarkSnapshot {
  repository: string;
  revision: string;
  signature: string;
  skills: Map<string, LocalHallmarkSkill>;
}

export interface LocalHallmarkSyncResult {
  available: boolean;
  root: string | null;
...
let snapshotCache: LocalHallmarkSnapshot | null = null;
const appliedSnapshots = new WeakMap<SkillsCatalogStore, string>();

/**
 * Resolves the hallmark repository root. `HALLMARK_SKILLS_ROOT` may point at
 * the repository itself or at its `skills/` tree; the repository root (the
 * directory containing `skills/`) is returned.
 */
export function localHallmarkSkillsRepository(): string | null {
  const configured = process.env.HALLMARK_SKILLS_ROOT?.trim();
  const candidates = configured
    ? [path.resolve(configured), path.resolve(configured, "..")]
    : [path.join(repositoryRoot(), "hallmark")];
...
export function synchronizeLocalHallmarkSkillsCatalog(input: {
  store?: SkillsCatalogStore;
  force?: boolean;
} = {}): LocalHallmarkSyncResult {
...
export function getLocalHallmarkSkill(upstreamId: string): {
  detail: SkillsShDetail;
  description: string | null;
  binaryPaths: string[];
  revision: string;
...
export function readLocalHallmarkSkillFiles(upstreamId: string): {
...
function loadSnapshot(repository: string, force: boolean): LocalHallmarkSnapshot {
  if (!force && snapshotCache?.repository === repository) return snapshotCache;
  const revision = readGitRevision(repository) ?? "local";
  const skills = new Map<string, LocalHallmarkSkill>();
  ...
  for (const entry of fs.readdirSync(tree, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    const id = `${HALLMARK_SKILLS_SOURCE}/${slug}`;
    ...
    const record: CatalogSnapshotRecord = {
      ...
      installUrl: `https://github.com/${HALLMARK_SKILLS_SOURCE}`,
      url: `https://github.com/${HALLMARK_SKILLS_SOURCE}/tree/${revision}/${SKILL_TREE_SUBPATH}/${encodeURIComponent(slug)}`,
      duplicate: false,
      curated: false,
      ranks: {},
      // The pack ships exactly one skill, already named "hallmark" — a
      // hallmark: namespace would only repeat the slug.
      slashCommand: slug,
    };
    skills.set(id, {
      directory,
...
function readSkillFiles(root: string): Array<{ relativePath: string; contents: Buffer }> {
  ...
  const visit = (directory: string) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) throw new Error(`Hallmark skill contains a symbolic link: ${absolute}`);
      ...
      if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) {
        throw new Error(`Hallmark skill file escapes its source directory: ${absolute}`);
      }
      const contents = fs.readFileSync(resolved);
      if (contents.byteLength > MAX_SKILL_FILE_BYTES) {
        throw new Error(`Hallmark skill file exceeds the ${MAX_SKILL_FILE_BYTES}-byte review limit: ${relative}`);
      }
      totalBytes += contents.byteLength;
      if (totalBytes > MAX_SKILL_TOTAL_BYTES) {
        throw new Error(`Hallmark skill exceeds the ${MAX_SKILL_TOTAL_BYTES}-byte review limit: ${root}`);
      }
      files.push({ relativePath: relative.replace(/\\/g, "/"), contents });
      if (files.length > MAX_SKILL_FILES) {
        throw new Error(`Hallmark skill exceeds the ${MAX_SKILL_FILES}-file review limit: ${root}`);
      }
    }
  };
  visit(rootRealPath);
