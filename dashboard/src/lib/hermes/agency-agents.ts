...
export interface AgencyAgentCatalog {
  ...
  message: string | null;
  source: "environment" | "managed" | "development_fallback" | "explicit" | null;
...
function publicMessage(status: AgencyAgentCatalog["status"]): string | null {
  if (status === "missing") {
    return "The Agency Agents catalog could not be found. Restart Breadboard or repair the installation.";
  }
  if (status === "invalid") {
...
function configuredRoot(options: LoadAgencyAgentCatalogOptions): {
  ...
  // An environment override is optional, not a dependency. In particular, a
  // checkout moved out of OneDrive used to leave a stale absolute path behind
  ...
  // The desktop supervisor provides the immutable application root in both
  // development and packaged builds. This is the normal product path; users
  // should never need to configure a source checkout themselves.
  if (process.env.BREADBOARD_REPO_ROOT?.trim()) {
  ...
  // Preserve the invalid override as the final diagnostic target only when no
  // managed catalog can take its place.
  if (process.env.AGENCY_AGENTS_PATH?.trim()) {
    return {
  ...
  return { rootPath: null, source: null };
}
...
/**
 * The Chief of Staff persona is the "company org chart" front door: when it is the
 * active agency agent, Breadboard layers an orchestration directive + the live
 * roster on top of its persona so it can delegate to specialists via the Hermes
 * `delegate_task` tool using the `[persona: <slug>]` hiring convention.
 */
export const CHIEF_OF_STAFF_SLUG = "chief-of-staff";

/** The persona tag an orchestrator prepends to a delegated task's text. */
export const DELEGATION_PERSONA_TAG = /^\s*\[persona:\s*([a-z0-9][a-z0-9-]*)\]\s*/i;
...
/**
 * A compact, division-grouped index of the roster the orchestrator can hire from.
 * Excludes the Chief of Staff itself. Empty string when no catalog is available.
 */
export function renderRosterIndex(catalog: AgencyAgentCatalog): string {
  if (catalog.status !== "ready" || catalog.agents.length === 0) return "";
  ...
  for (const [division, agents] of [...byDivision.entries()].sort((a, b) =>
    a[0].localeCompare(b[0]),
  )) {
    const lines = agents
      .sort((a, b) => a.slug.localeCompare(b.slug))
      .map((agent) => `- ${agent.slug}: ${truncateDescription(agent.description)}`);
...
/**
 * The Breadboard-side orchestration layer appended to the Chief of Staff persona:
 * the operating loop + the `delegate_task` hiring convention + the live roster.
 * Kept out of the vendored persona markdown so the roster stays in sync with disk.
 */
export function renderChiefOfStaffOrchestration(catalog: AgencyAgentCatalog): string {
  const directive = [
    "## Orchestration directive (Breadboard runtime)",
    "Beyond the persona above, you lead a company of specialist agents. Operating loop for every request:",
    "1. Recall the provided context. 2. Classify: trivial/conversational → answer directly in 1–2 lines; specialized or multi-part → deploy specialists. 3. Synthesize their results into one coherent answer in your own voice — you own the final response, not your workers.",
    "",
    "To deploy a specialist, call the `delegate_task` tool with the task text beginning with a persona tag so the worker adopts that exact expert identity:",
    "    [persona: <slug>] <the concrete goal + all the context the specialist needs>",
    "- Pass multiple tasks in one `delegate_task` call to run independent work in parallel.",
    "- Pick the single most-fitting slug per task (most specific wins).",
    "- Don't delegate a one-line answer, and don't re-delegate your whole job to one worker.",
    "- Briefly name who you're bringing in before delegating so the user sees the company at work.",
  ].join("\n");
  const roster = renderRosterIndex(catalog);
  if (!roster) return directive;
  return `${directive}\n\n## Available specialists (use the slug in [persona: <slug>])\n\n${roster}`;
}
