...
export const SCIENTIFIC_SKILLS_SOURCE = "k-dense-ai/scientific-agent-skills";
const MAX_SKILL_FILES = 200;
const MAX_SKILL_FILE_BYTES = 2_000_000;
const MAX_SKILL_TOTAL_BYTES = 10_000_000;

interface LocalScientificSkill {
  directory: string;
...
interface LocalScientificSnapshot {
  root: string;
  revision: string;
  signature: string;
  skills: Map<string, LocalScientificSkill>;
}

export interface LocalScientificSyncResult {
  available: boolean;
  root: string | null;
...
let snapshotCache: LocalScientificSnapshot | null = null;
const appliedSnapshots = new WeakMap<SkillsCatalogStore, string>();

export function localScientificSkillsRoot(): string | null {
  const configured = process.env.SCIENTIFIC_AGENT_SKILLS_ROOT?.trim();
  const candidates = configured
    ? [path.resolve(configured), path.resolve(configured, "skills")]
    : [path.join(repositoryRoot(), "scientific-agent-skills", "skills")];
...
export function synchronizeLocalScientificSkillsCatalog(input: {
  store?: SkillsCatalogStore;
  force?: boolean;
} = {}): LocalScientificSyncResult {
  const store = input.store ?? getSkillsCatalogStore();
  const root = localScientificSkillsRoot();
  if (!root) {
    return { available: false, root: null, revision: null, added: 0, updated: 0, total: 0, skipped: true };
  }
  const snapshot = loadSnapshot(root, input.force === true);
...
export function getLocalScientificSkill(upstreamId: string): {
  detail: SkillsShDetail;
  description: string | null;
  binaryPaths: string[];
  revision: string;
  ...
  const root = localScientificSkillsRoot();
  if (!root) return null;
  const snapshot = loadSnapshot(root, false);
...
export function readLocalScientificSkillFiles(upstreamId: string): {
...
function loadSnapshot(root: string, force: boolean): LocalScientificSnapshot {
  if (!force && snapshotCache?.root === root) return snapshotCache;
  const repository = path.dirname(root);
  const revision = readGitRevision(repository) ?? "local";
  const skills = new Map<string, LocalScientificSkill>();
  for (const entry of fs.readdirSync(root, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    const id = `${SCIENTIFIC_SKILLS_SOURCE}/${slug}`;
    ...
    const record: CatalogSnapshotRecord = {
      ...
      installUrl: `https://github.com/${SCIENTIFIC_SKILLS_SOURCE}`,
      url: `https://github.com/${SCIENTIFIC_SKILLS_SOURCE}/tree/${revision}/skills/${encodeURIComponent(slug)}`,
      duplicate: false,
      curated: false,
      ranks: {},
      slashCommand: `k-dense-ai:${slug}`,
    };
    skills.set(id, {
      directory,
      record,
  ...
  snapshotCache = { root, revision, signature, skills };
...
function readSkillFiles(root: string): Array<{ relativePath: string; contents: Buffer }> {
  ...
  const visit = (directory: string) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) throw new Error(`Scientific skill contains a symbolic link: ${absolute}`);
      ...
      if (contents.byteLength > MAX_SKILL_FILE_BYTES) {
        throw new Error(`Scientific skill file exceeds the ${MAX_SKILL_FILE_BYTES}-byte review limit: ${relative}`);
      }
      totalBytes += contents.byteLength;
      if (totalBytes > MAX_SKILL_TOTAL_BYTES) {
        throw new Error(`Scientific skill exceeds the ${MAX_SKILL_TOTAL_BYTES}-byte review limit: ${root}`);
      }
      files.push({ relativePath: relative.replace(/\\/g, "/"), contents });
      if (files.length > MAX_SKILL_FILES) {
        throw new Error(`Scientific skill exceeds the ${MAX_SKILL_FILES}-file review limit: ${root}`);
      }
    }
  };
  visit(rootRealPath);
