...
//
// Hermes sessions run inside an ephemeral `.runtime/hermes` workspace.
...
export function createFilesystemGrantStore(
  ...
  function list(userId: number, includeRevoked = false): ApprovedFilesystemRoot[] {
    const rows = database
      .prepare(
        `SELECT * FROM hermes_filesystem_grants
  ...
  function grant(request: GrantRequest): ApprovedFilesystemRoot {
    ...
    database
      .prepare(
        `INSERT INTO hermes_filesystem_grants
    ...
    const row = database
      .prepare(
        "SELECT * FROM hermes_filesystem_grants WHERE user_id = ? AND canonical_path = ?",
      )
  ...
  function revoke(userId: number, grantId: string): boolean {
    return (
      database
        .prepare(
          `UPDATE hermes_filesystem_grants
