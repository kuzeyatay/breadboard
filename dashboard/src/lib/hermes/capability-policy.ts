...
// Baseline knowledge-work tools. `garden_save_note` belongs here alongside the
// proposal tools: authoring in the user's own garden is the work this mode is
// for, and it is unrelated to the repository mutation this mode withholds. The
// same holds for the structure tools — filing a note under the right folder is
// knowledge work, not engineering, so it stays available in this mode.
...
export function outcomeWithoutCapabilityTokens(value: string): string {
  ...
  const token = /^\/[a-z0-9][a-z0-9_.:-]*(?:\s+|$)/i;
...
export function implementationPathPatterns(
  ...
  if (/\b(?:capabilit(?:y|ies)|slash)\s+(?:palette|picker)|command hub|composer\b/i.test(outcome)) {
    add(
      ...
      "dashboard/src/app/api/hermes/commands/**",
      "dashboard/tests/hermes-capability-palette.test.mjs",
      "dashboard/tests/hermes-commands.test.mjs",
    );
  }
  if (/\b(?:connection flow|add connection|mcp connection)\b/i.test(outcome)) {
    add(
      "dashboard/src/app/components/hermes/command-hub.tsx",
      "dashboard/src/app/api/hermes/mcp/**",
      "dashboard/src/lib/hermes/mcp-connections.ts",
      "dashboard/tests/hermes-mcp-connections.test.mjs",
    );
  }
  ...
  if (/\b(?:garden chat|garden assistant|garden tool|proposal)\b/i.test(outcome)) {
    add(
      ...
      "dashboard/src/lib/hermes/garden-chat-adapter.ts",
      "dashboard/src/app/api/chat/**",
      "hermes-config/agent/breadboard-garden.md",
      "dashboard/tests/hermes-live-routing.test.mjs",
    );
  }
  if (/\b(?:server[- ]side|authorization|api route|endpoint|session policy|permission)\b/i.test(outcome)) {
    add(
      "dashboard/src/app/api/hermes/**",
      "dashboard/src/lib/hermes/**",
      "dashboard/tests/hermes-*.test.mjs",
    );
  }
...
export function connectionToolAllowed(tool: string): boolean {
