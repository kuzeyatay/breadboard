...
/** GitHub identity of the vendored software-engineering skills pack. */
export const ENGINEERING_SKILLS_SOURCE = "addyosmani/agent-skills";
/** Human label used in inspection notices. */
export const ENGINEERING_SKILLS_LABEL = "agent-skills clone";
/**
 * The pack ships agents, commands and hooks beside its skills; only the skills
 * tree is a catalog source, so the rest of the clone is deliberately ignored.
 */
...
interface LocalEngineeringSkill {
  directory: string;
...
interface LocalEngineeringSnapshot {
  repository: string;
  revision: string;
  signature: string;
  skills: Map<string, LocalEngineeringSkill>;
}

export interface LocalEngineeringSyncResult {
  available: boolean;
  root: string | null;
...
let snapshotCache: LocalEngineeringSnapshot | null = null;
const appliedSnapshots = new WeakMap<SkillsCatalogStore, string>();

/**
 * Resolves the agent-skills repository root. `AGENT_SKILLS_ROOT` may point at
 * the repository itself or at its `skills/` tree; the repository root (the
 * directory containing `skills/`) is returned.
 */
export function localEngineeringSkillsRepository(): string | null {
  const configured = process.env.AGENT_SKILLS_ROOT?.trim();
  const candidates = configured
    ? [path.resolve(configured), path.resolve(configured, "..")]
    : [path.join(repositoryRoot(), "agent-skills")];
  for (const candidate of candidates) {
    ...
    if (hasSkills) return fs.realpathSync(candidate);
  }
  return null;
}

export function synchronizeLocalEngineeringSkillsCatalog(input: {
  store?: SkillsCatalogStore;
  force?: boolean;
} = {}): LocalEngineeringSyncResult {
...
export function getLocalEngineeringSkill(upstreamId: string): {
  detail: SkillsShDetail;
  description: string | null;
  binaryPaths: string[];
  revision: string;
...
export function readLocalEngineeringSkillFiles(upstreamId: string): {
...
function loadSnapshot(repository: string, force: boolean): LocalEngineeringSnapshot {
  if (!force && snapshotCache?.repository === repository) return snapshotCache;
  const revision = readGitRevision(repository) ?? "local";
  const skills = new Map<string, LocalEngineeringSkill>();
  ...
  for (const entry of fs.readdirSync(tree, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    const id = `${ENGINEERING_SKILLS_SOURCE}/${slug}`;
    ...
    const record: CatalogSnapshotRecord = {
      ...
      installUrl: `https://github.com/${ENGINEERING_SKILLS_SOURCE}`,
      url: `https://github.com/${ENGINEERING_SKILLS_SOURCE}/tree/${revision}/${SKILL_TREE_SUBPATH}/${encodeURIComponent(slug)}`,
      duplicate: false,
      curated: false,
      ranks: {},
      slashCommand: `eng:${slug}`,
    };
    skills.set(id, {
      directory,
      record,
...
function readSkillFiles(root: string): Array<{ relativePath: string; contents: Buffer }> {
  ...
  const visit = (directory: string) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) throw new Error(`Engineering skill contains a symbolic link: ${absolute}`);
      ...
      if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) {
        throw new Error(`Engineering skill file escapes its source directory: ${absolute}`);
      }
      const contents = fs.readFileSync(resolved);
      if (contents.byteLength > MAX_SKILL_FILE_BYTES) {
        throw new Error(`Engineering skill file exceeds the ${MAX_SKILL_FILE_BYTES}-byte review limit: ${relative}`);
      }
      totalBytes += contents.byteLength;
      if (totalBytes > MAX_SKILL_TOTAL_BYTES) {
        throw new Error(`Engineering skill exceeds the ${MAX_SKILL_TOTAL_BYTES}-byte review limit: ${root}`);
      }
      files.push({ relativePath: relative.replace(/\\/g, "/"), contents });
      if (files.length > MAX_SKILL_FILES) {
        throw new Error(`Engineering skill exceeds the ${MAX_SKILL_FILES}-file review limit: ${root}`);
      }
    }
  };
  visit(rootRealPath);
