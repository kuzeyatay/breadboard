...
export type RuntimeMcpConfig =
  ...
  | {
      ...
      /**
       * Server-only OAuth provider used by the in-process MCP proxy. It is never
       * serialized to Hermes, an API response, or an audit event.
       */
...
export function listMcpConnections(
  ...
  const rows = db
    .prepare(
      `SELECT * FROM hermes_mcp_connections WHERE user_id = ? ${enabledOnly ? "AND enabled = 1" : ""} ORDER BY display_name`,
    )
...
export function saveMcpConnection(
  ...
  const result = db
    .prepare(
      `INSERT INTO hermes_mcp_connections (user_id, slug, display_name, transport, config_json, enabled, approved_at) VALUES (?, ?, ?, ?, ?, 1, datetime('now'))`,
    )
    .run(
      userId,
      parsed.slug,
...
export function setMcpConnectionEnabled(
  ...
  return (
    db
      .prepare(
        "UPDATE hermes_mcp_connections SET enabled = ?, updated_at = datetime('now') WHERE id = ? AND user_id = ?",
      )
...
export function deleteMcpConnection(
  ...
  db.prepare(
    "DELETE FROM hermes_mcp_connections WHERE id = ? AND user_id = ?",
  ).run(id, userId);
  return connection;
}
