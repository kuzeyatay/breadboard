...
function uniqueSlug(userId: number, requested: string, excludingId?: number): string {
  ...
  for (let suffix = 0; suffix < 1_000; suffix += 1) {
    ...
    const row = db.prepare(
      "SELECT id FROM hermes_prompts WHERE user_id = ? AND slug = ? AND (? IS NULL OR id <> ?)",
...
export function listPrompts(userId: number): SavedPrompt[] {
  const rows = db.prepare(
    "SELECT id, legacy_id, slug, title, content, category, favorite FROM hermes_prompts WHERE user_id = ? ORDER BY favorite DESC, updated_at DESC, id DESC",
...
export function resolvePrompt(userId: number | null, slug: string): SavedPrompt | null {
  ...
  const row = db.prepare(
    "SELECT id, legacy_id, slug, title, content, category, favorite FROM hermes_prompts WHERE user_id = ? AND slug = ?",
...
export function createPrompt(userId: number, input: { title: string; content: string; category?: string; favorite?: boolean; legacyId?: string }): SavedPrompt {
  ...
  const result = db.prepare(
    `INSERT INTO hermes_prompts (user_id, legacy_id, slug, title, content, category, favorite)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
export function updatePrompt(userId: number, id: number, input: { title: string; content: string; category?: string; favorite?: boolean }): SavedPrompt | null {
  const exists = db.prepare("SELECT id FROM hermes_prompts WHERE id = ? AND user_id = ?").get(id, userId);
  if (!exists) return null;
  const slug = uniqueSlug(userId, input.title, id);
  db.prepare(
    `UPDATE hermes_prompts SET slug = ?, title = ?, content = ?, category = ?, favorite = ?, updated_at = datetime('now') WHERE id = ? AND user_id = ?`,
...
export function deletePrompt(userId: number, id: number): boolean {
  return db.prepare("DELETE FROM hermes_prompts WHERE id = ? AND user_id = ?").run(id, userId).changes > 0;
}
...
export function importLegacyPrompts(userId: number, values: unknown[]): { imported: number; skipped: number } {
  ...
  const insert = db.transaction(() => {
    for (const value of values.slice(0, 200)) {
      ...
      const exists = db.prepare("SELECT id FROM hermes_prompts WHERE user_id = ? AND legacy_id = ?").get(userId, legacyId);
