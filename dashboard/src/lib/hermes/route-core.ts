// Pure, framework-free request/response helpers shared by the Hermes route
...
import { HermesRpcErrorResponse } from "../agent-runtime/hermes-wire.ts";
...
/** Throws a 503 ApiError when Hermes is disabled — never crash or hang. */
export function requireEnabled(): void {
  ...
  if (!runtime.hermes.baseUrl || !runtime.hermes.sessionToken) {
    throw new ApiError(
      ...
      "Hermes is not enabled on this server.",
    );
  }
}
...
export function describeError(error: unknown): { status: number; body: Record<string, unknown> } {
  ...
  if (error instanceof HermesRpcErrorResponse) {
    return {
      status: 503,
      body: {
        error: "The agent runtime is unavailable.",
        code: error.code ?? "hermes_rpc_error",
        recoverable: true,
      },
    };
  }
