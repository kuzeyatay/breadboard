...
/** GitHub identity of the vendored OfficeCLI document-authoring skills pack. */
export const OFFICE_SKILLS_SOURCE = "iOfficeAI/OfficeCLI";
/** Human label used in inspection notices. */
export const OFFICE_SKILLS_LABEL = "OfficeCLI clone";
...
interface LocalOfficeSnapshot {
  repository: string;
  revision: string;
  signature: string;
  skills: Map<string, LocalOfficeSkill>;
}

export interface LocalOfficeSyncResult {
  available: boolean;
  root: string | null;
...
let snapshotCache: LocalOfficeSnapshot | null = null;
const appliedSnapshots = new WeakMap<SkillsCatalogStore, string>();

/**
 * Resolves the OfficeCLI repository root. `OFFICE_SKILLS_ROOT` may point at
 * the repository itself or at its `skills/` tree; the repository root (the
 * directory containing `skills/`) is returned.
 */
export function localOfficeSkillsRepository(): string | null {
  const configured = process.env.OFFICE_SKILLS_ROOT?.trim();
  const candidates = configured
    ? [path.resolve(configured), path.resolve(configured, "..")]
    : [path.join(repositoryRoot(), "OfficeCLI")];
...
export function synchronizeLocalOfficeSkillsCatalog(input: {
  store?: SkillsCatalogStore;
  force?: boolean;
} = {}): LocalOfficeSyncResult {
...
export function getLocalOfficeSkill(upstreamId: string): {
  detail: SkillsShDetail;
  description: string | null;
  binaryPaths: string[];
  revision: string;
...
export function readLocalOfficeSkillFiles(upstreamId: string): {
...
function loadSnapshot(repository: string, force: boolean): LocalOfficeSnapshot {
  if (!force && snapshotCache?.repository === repository) return snapshotCache;
  const revision = readGitRevision(repository) ?? "local";
  const skills = new Map<string, LocalOfficeSkill>();
  ...
  for (const entry of fs.readdirSync(tree, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    const id = `${OFFICE_SKILLS_SOURCE}/${slug}`;
    ...
    const record: CatalogSnapshotRecord = {
      ...
      installUrl: `https://github.com/${OFFICE_SKILLS_SOURCE}`,
      url: `https://github.com/${OFFICE_SKILLS_SOURCE}/tree/${revision}/${SKILL_TREE_SUBPATH}/${encodeURIComponent(slug)}`,
      duplicate: false,
      curated: false,
      ranks: {},
      slashCommand: `office:${slug}`,
    };
    skills.set(id, {
      directory,
      record,
...
function readSkillFiles(root: string): Array<{ relativePath: string; contents: Buffer }> {
  ...
  const visit = (directory: string) => {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) throw new Error(`Office skill contains a symbolic link: ${absolute}`);
      ...
      if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) {
        throw new Error(`Office skill file escapes its source directory: ${absolute}`);
      }
      const contents = fs.readFileSync(resolved);
      if (contents.byteLength > MAX_SKILL_FILE_BYTES) {
        throw new Error(`Office skill file exceeds the ${MAX_SKILL_FILE_BYTES}-byte review limit: ${relative}`);
      }
      totalBytes += contents.byteLength;
      if (totalBytes > MAX_SKILL_TOTAL_BYTES) {
        throw new Error(`Office skill exceeds the ${MAX_SKILL_TOTAL_BYTES}-byte review limit: ${root}`);
      }
      files.push({ relativePath: relative.replace(/\\/g, "/"), contents });
      if (files.length > MAX_SKILL_FILES) {
        throw new Error(`Office skill exceeds the ${MAX_SKILL_FILES}-file review limit: ${root}`);
      }
    }
  };
  visit(rootRealPath);
