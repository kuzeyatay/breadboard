  ...
  decisions: [{
    ...
    interaction: {
      ...
      controls: [{
        ...
        defaultValue:
          "in-range number, exact declared select option, 0 for button, or false for toggle",
        evidence: [{
          anchor: "exact evidence anchor (empty array for pure protocol_action)",
...
export function validateModelVisualNecessityBatch(input: {
  ...
  for (const decision of parsedDecisions) {
    ...
    const contractProblems = validateVisualizationContractUnitRepair({
      ...
      repair: {
        unitId: decision.unitId,
        interactionGoal: decision.interaction.interactionGoal,
        learnerAction: decision.interaction.learnerAction,
