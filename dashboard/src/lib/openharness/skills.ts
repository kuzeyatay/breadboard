...
const APPROVABLE_AGENTS = new Set(["breadboard-terminal"]);
...
export async function runSkillsCli(
  ...
  const command = process.platform === "win32" && fs.existsSync(windowsNpx) ? process.execPath : "npx";
...
export function parseSkillSearchOutput(output: string): SkillCandidate[] {
  ...
  return lines.flatMap((line, index) => {
    ...
    const detailsUrl = lines.slice(index + 1, index + 3)
      .map((next) => next.replace(/^└\s*/, ""))
      .find((next) => /^https:\/\/skills\.sh\//i.test(next)) ?? `https://skills.sh/${repository}/${name}`;
    return [{
      id: packageId,
      name,
      package: packageId,
...
function validPackage(repository: string, name: string): boolean {
  return /^[a-z0-9_.-]+\/[a-z0-9_.-]+$/i.test(repository) && /^[a-z0-9_.-]+$/i.test(name);
}
...
function ensureInside(root: string, target: string): string {
  ...
  if (relative.startsWith("..") || path.isAbsolute(relative)) throw new Error("Path escapes the skills root");
  return resolved;
}
...
export async function downloadSkillToQuarantine(
  ...
  if (!validPackage(candidate.repository, candidate.name) || candidate.package !== `${candidate.repository}@${candidate.name}`) {
  ...
  try {
    ...
    if (!skillDirectory) throw new Error("The Skills CLI did not produce the selected skill");
    const files = readBoundedSkillFiles(skillDirectory);
    const lock = readSkillLock(staging, candidate.name);
    return quarantineSkill({
      candidate: {
        ...candidate,
        source: typeof lock?.sourceUrl === "string" ? lock.sourceUrl : candidate.source,
        version: typeof lock?.skillFolderHash === "string" ? lock.skillFolderHash : candidate.version,
      },
      files,
    });
  } finally {
...
function findInstalledSkill(root: string, name: string): string | null {
  ...
  return listFilesRecursive(root, 6)
    .find((file) => path.basename(file).toLowerCase() === "skill.md" && path.basename(path.dirname(file)) === name)
    ?.replace(/[\\/]SKILL\.md$/i, "") ?? null;
}

function readSkillLock(root: string, name: string): Record<string, unknown> | null {
  const lockFile = [path.join(root, "skills-lock.json"), path.join(root, ".agents", ".skill-lock.json")]
    .find((candidate) => fs.existsSync(candidate));
  if (!lockFile) return null;
  try {
    const lock = JSON.parse(fs.readFileSync(lockFile, "utf8")) as { skills?: Record<string, Record<string, unknown>> };
...
function readBoundedSkillFiles(root: string): Record<string, Buffer> {
  ...
  if (paths.length > MAX_SKILL_FILES) throw new Error(`Skill exceeds the ${MAX_SKILL_FILES}-file quarantine limit`);
  let total = 0;
  return Object.fromEntries(paths.map((file) => {
    const size = fs.statSync(file).size;
    if (size > MAX_SKILL_FILE_BYTES) throw new Error("Skill contains a file larger than the quarantine limit");
    total += size;
    if (total > MAX_SKILL_TOTAL_BYTES) throw new Error("Skill exceeds the total quarantine size limit");
...
export function quarantineSkill(input: {
  ...
  fs.writeFileSync(path.join(dir, QUARANTINE_MANIFEST), JSON.stringify(report, null, 2), "utf8");
  return report;
}
...
export function inspectQuarantine(name: string, candidate?: SkillCandidate): QuarantineReport {
  ...
  return { ...report, integrityVerified: saved ? sameHashes(saved.fileHashes, report.fileHashes) : report.integrityVerified };
}
...
function inspectFiles(
  ...
  const fileHashes = Object.fromEntries(files.map((file) => [file, sha256(fs.readFileSync(path.join(dir, file)))]));
  const skillMdPath = path.join(dir, "SKILL.md");
  const hasSkillMd = fs.existsSync(skillMdPath);
  const skillMarkdown = hasSkillMd ? fs.readFileSync(skillMdPath, "utf8") : "";
  const frontmatterName = hasSkillMd ? parseSkillName(skillMarkdown) : undefined;
  const discoveredScripts = files.filter((file) => /\.(sh|bash|ps1|bat|cmd|js|mjs|cjs|py|rb)$/i.test(file));
  const externalNetworkRequirements = [...new Set(files.flatMap((file) => {
    try {
      return fs.readFileSync(path.join(dir, file), "utf8").match(/https?:\/\/[^\s)\]"']+/g) ?? [];
    } catch {
      return [];
    }
  }))].slice(0, 20);
  const requestedPermissions = derivePermissions(skillMarkdown, discoveredScripts, externalNetworkRequirements, candidate?.requestedPermissions ?? saved?.requestedPermissions ?? []);
  const risks: string[] = [];
  if (!hasSkillMd) risks.push("Missing SKILL.md manifest.");
  if (frontmatterName && sanitizeSkillName(frontmatterName) !== name) risks.push(`Manifest name "${frontmatterName}" does not match "${name}".`);
  if (discoveredScripts.length) risks.push(`Contains script files: ${discoveredScripts.join(", ")}`);
  if (externalNetworkRequirements.length) risks.push("References external network locations.");
  for (const file of files) {
    try {
      const contents = fs.readFileSync(path.join(dir, file), "utf8");
      if (/curl\s|wget\s|rm\s+-rf|child_process|eval\(/i.test(contents)) risks.push(`Suspicious command pattern in ${file}`);
    } catch {
  ...
  return {
    ...
    risks: [...new Set(risks)],
    riskSummary: risks.length ? `${risks.length} risk signal(s) require review.` : "No static risk signals detected; human review is still required.",
    reviewState: "quarantined",
    approvedAgents: saved?.approvedAgents ?? ["breadboard-terminal"],
  };
}
...
function derivePermissions(
  ...
  if (scripts.length || /allowed-tools:[\s\S]*(bash|shell)/i.test(markdown)) permissions.add("shell");
  if (urls.length || /\b(fetch|http|api|network)\b/i.test(markdown)) permissions.add("network");
  if (/\b(read|inspect|open)\b[^\n]*(file|document|repository)/i.test(markdown)) permissions.add("filesystem-read");
  if (/\b(write|edit|modify|create|delete)\b[^\n]*(file|document|repository)/i.test(markdown)) permissions.add("filesystem-write");
  return [...permissions];
}

export function promoteSkill(
  name: string,
  options?: { overwrite?: boolean; approvedAgents?: string[]; approvedPermissions?: SkillPermission[] },
): { promotedPath: string; report: QuarantineReport } {
  const report = inspectQuarantine(name);
  if (!report.integrityVerified) throw new Error("Quarantined files changed after review");
  if (!report.hasSkillMd) throw new Error("Refusing to promote a skill without a SKILL.md manifest");
  if (!report.frontmatterName || sanitizeSkillName(report.frontmatterName) !== report.name) {
    throw new Error("Refusing to promote a skill whose manifest name does not match its quarantine name");
  }
  const source = ensureInside(quarantineRoot(), report.name);
  const target = ensureInside(approvedRoot(), report.name);
  if (fs.existsSync(target) && !options?.overwrite) throw new Error("A skill with this name is already approved");
  fs.rmSync(target, { recursive: true, force: true });
  fs.mkdirSync(path.dirname(target), { recursive: true });
  fs.cpSync(source, target, { recursive: true });
  fs.rmSync(path.join(target, QUARANTINE_MANIFEST), { force: true });
  const approvedAgents = (options?.approvedAgents ?? []).filter((agent) => APPROVABLE_AGENTS.has(agent));
  const approved = {
    ...report,
    reviewState: "approved" as const,
    approvedAgents: approvedAgents.length ? approvedAgents : ["breadboard-terminal"],
...
export function rejectQuarantine(name: string): void {
  fs.rmSync(ensureInside(quarantineRoot(), sanitizeSkillName(name)), { recursive: true, force: true });
}
...
function updateApprovedRegistry(report: QuarantineReport): void {
  ...
  try {
    registry = JSON.parse(fs.readFileSync(file, "utf8")) as { skills: Record<string, unknown> };
  } catch {
    registry = { skills: {} };
  }
  registry.skills[report.name] = { ...report, approvedAt: new Date().toISOString() };
...
function readQuarantineManifest(dir: string): QuarantineReport | null {
  try {
    return JSON.parse(fs.readFileSync(path.join(dir, QUARANTINE_MANIFEST), "utf8")) as QuarantineReport;
  } catch {
    return null;
  }
}
