...
export const SKILLS_SH_API_BASE = "https://skills.sh/api/v1";
export const SKILLS_SH_MAX_PAGE_SIZE = 500;
export const SKILLS_SH_CLI_PACKAGE = "skills@1.5.9";
...
export class SkillsShConfigurationError extends Error {
  constructor(message = "skills.sh authentication is not configured. Link this project to Vercel or set VERCEL_OIDC_TOKEN on the server.") {
    super(message);
    this.name = "SkillsShConfigurationError";
  }
}
...
export class VercelOidcTokenProvider implements SkillsShTokenProvider {
  async getToken(): Promise<string> {
    try {
      const token = await getVercelOidcToken();
      if (token?.trim()) return token.trim();
    } catch {
      // The SDK intentionally throws outside a Vercel-linked environment.
    }
    const token = process.env.VERCEL_OIDC_TOKEN?.trim();
    if (!token) throw new SkillsShConfigurationError();
    return token;
  }
}
...
export class SkillsShClient {
  ...
  constructor(options: SkillsShClientOptions = {}) {
    this.tokenProvider = options.tokenProvider ?? new VercelOidcTokenProvider();
    this.fetchImpl = options.fetchImpl ?? fetch;
    this.baseUrl = (options.baseUrl ?? SKILLS_SH_API_BASE).replace(/\/$/, "");
  ...
  async detail(source: string, slug: string, signal?: AbortSignal): Promise<SkillsShDetail> {
    ...
    const files = body.files === null ? null : array(body.files, "skill detail files").map((value) => {
      ...
      if (typeof file.path !== "string" || typeof file.contents !== "string") {
        throw new TypeError("skills.sh returned a malformed skill file");
      }
  ...
  private async requestJson(
    ...
    for (let attempt = 0; attempt <= this.retries; attempt += 1) {
      ...
      try {
        ...
        const error = new SkillsShApiError(message, response.status, response.headers);
        ...
        if (error instanceof SkillsShApiError && !retryableStatus(error.status)) throw error;
        lastError = error;
    ...
    throw new Error("skills.sh request failed");
  }
}
...
function record(value: unknown, label: string): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new TypeError(`skills.sh returned malformed ${label}`);
  }
  return value as Record<string, unknown>;
}

function array(value: unknown, label: string): unknown[] {
  if (!Array.isArray(value)) throw new TypeError(`skills.sh returned malformed ${label}`);
  return value;
}
...
function sourcePath(source: string): string {
  ...
  if (segments.length < 1) throw new TypeError("Invalid skill source");
...
async function providerErrorMessage(response: Response): Promise<string> {
  ...
  return `skills.sh returned HTTP ${response.status}`;
}
