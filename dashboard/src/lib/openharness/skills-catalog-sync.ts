...
export async function synchronizeSkillsCatalog(
  ...
  inFlightSync = performSynchronization(client, store, options.signal).finally(() => {
    inFlightSync = null;
  });
  return inFlightSync;
}
...
export function revalidateSkillsCatalogInBackground(): void {
  ...
  void synchronizeSkillsCatalog({ store }).catch(() => {
...
async function performSynchronization(
  ...
  try {
    ...
    if (error instanceof SkillsShApiError) rateLimit = error.rateLimit;
    const message = error instanceof Error ? error.message : "skills.sh synchronization failed";
  ...
  const worker = async () => {
    while (cursor < installed.length) {
      ...
      if (skill.installedPath && skill.localHash) {
        try {
          if (hashInstalledDirectory(skill.installedPath) !== skill.localHash) {
...
function hashInstalledDirectory(root: string): string {
  const files: string[] = [];
  ...
  return crypto
    .createHash("sha256")
    .update(JSON.stringify(Object.entries(hashes).sort(([left], [right]) => left.localeCompare(right))))
    .digest("hex");
}
