...
import { getRuntimeSessionByChatSession, recordAuditEvent } from "./runtime-store.ts";
...
export async function openGardenAgentChat(
  ...
  const clusterSlug = typeof payload.clusterSlug === "string" ? payload.clusterSlug.trim() : "";
  if (!clusterSlug) throw new ApiError(400, "garden_required", "clusterSlug is required.");
  ...
  if (!Number.isInteger(chatSessionId) || chatSessionId <= 0) {
    throw new ApiError(400, "chat_session_required", "A Breadboard chat session is required.");
  }
  const chat = db
    .prepare("SELECT id FROM chat_sessions WHERE id = ? AND user_id = ? AND cluster_id = ?")
    .get(chatSessionId, userId, access.clusterId) as { id: number } | undefined;
  if (!chat) throw new ApiError(404, "chat_session_not_found", "Chat session not found.");
...
function parseActivePage(value: unknown): { slug: string; title?: string } | null {
  if (!value || typeof value !== "object") return null;
  const slug = typeof (value as { slug?: unknown }).slug === "string"
...
function gardenTurnContext(
  ...
  const selected = Array.isArray(selectedDocumentSlugs)
    ? selectedDocumentSlugs.filter((value): value is string => typeof value === "string").slice(0, 12)
    : [];
  return [
    `Authorized garden: ${gardenSlug}`,
    `Breadboard chat session: ${chatSessionId}`,
    page ? `Current page: ${page.title ?? page.slug} (${page.slug})` : "Current page: garden workspace",
    selected.length ? `User-selected garden documents: ${selected.join(", ")}` : "",
    "Use only the garden_* tools. All modifications must remain typed Breadboard proposals.",
  ].filter(Boolean).join("\n");
}
...
function legacyGardenEventStream(
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(output) {
      const emit = (value: unknown) => output.enqueue(encoder.encode(`data: ${JSON.stringify(value)}\n\n`));
      emit({ type: "runtime", backend: "openharness", fallback: false, sessionId: session.row.id });
      try {
        let connected!: () => void;
        const ready = new Promise<void>((resolve) => { connected = resolve; });
        const events = gateway.subscribeToSession(
        ...
        await Promise.race([
          ...
          firstEvent.then((result) => {
            if (result.done) throw new Error("OpenHarness event stream closed before the prompt was sent.");
          }),
        ]);
        await sendMessage();
        for (let next = await firstEvent; !next.done; next = await events.next()) {
          ...
          if (event.type === "tool.completed") {
            emit({ type: "tool", status: event.payload.success ? "completed" : "failed", ...event.payload });
            recordAuditEvent({
          ...
          if (event.type === "error") emit({ type: "error", error: event.payload.message });
          if (event.type === "session.status" && event.payload.status === "idle") break;
        }
        markStatus(session, controller.signal.aborted ? "aborted" : "idle");
        recordAuditEvent({
          eventType: controller.signal.aborted ? "session.cancelled" : "message.completed",
          runtimeSessionId: session.row.id,
          userId: session.row.user_id,
          gardenId: session.row.garden_id,
        });
        ...
        emit({ type: "error", error: error instanceof Error ? error.message : "OpenHarness stream failed." });
        recordAuditEvent({
          eventType: "error",
          runtimeSessionId: session.row.id,
          userId: session.row.user_id,
