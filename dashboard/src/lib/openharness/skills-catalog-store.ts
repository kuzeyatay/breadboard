...
export class SkillsCatalogStore {
  ...
  markQuarantined(upstreamId: string, upstreamHash: string, revisionKey: string): void {
    this.db.prepare(
      `UPDATE skills_catalog SET review_status = 'quarantined',
       ...
       upstream_hash = ?, quarantine_revision = ? WHERE upstream_id = ?`,
    ).run(upstreamHash, revisionKey, upstreamId);
  }
