// Vendored from simstudioai/sim (Apache-2.0), apps/sim/lib/guardrails/pii-entities.ts,
// verbatim — pure catalog, no imports to fix. Breadboard has no `validate_pii.ts`
// (no Presidio sidecar; see ./local-pii.ts for the regex-only equivalent) and no
// settings UI referencing every field here yet — kept as the full catalog so a
// future Presidio sidecar, or a richer settings picker, is a drop-in.
