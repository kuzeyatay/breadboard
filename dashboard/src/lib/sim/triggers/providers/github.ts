// Vendored from simstudioai/sim (Apache-2.0), apps/sim/lib/webhooks/providers/github.ts, adapted for Breadboard.
// Adaptation: @sim/logger -> local shim; @sim/security/compare -> local
// safeCompare; @sim/utils/object -> local isRecordLike; the dynamic
// `@/triggers/github/utils` import -> the vendored `../github/utils` sibling.
...
export const githubHandler: WebhookProviderHandler = {
  ...
  async matchEvent({
    ...
    if (triggerId && triggerId !== 'github_webhook') {
      ...
      const { isGitHubEventMatch } = await import('../github/utils')
