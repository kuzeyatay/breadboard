// Vendored wholesale from simstudioai/sim (Apache-2.0), apps/sim/lib/webhooks/providers/utils.ts, adapted for Breadboard.
// Adaptation: @sim/logger -> local createLogger shim; @sim/security/compare
// safeCompare and @sim/security/hash sha256Hex -> local timing-safe helper
// (crypto.timingSafeEqual) in ../security.

import { NextResponse } from 'next/server'
import { createLogger, type Logger } from '../support'
import { safeCompare, sha256Hex } from '../security'
import type { AuthContext, EventFilterContext } from './types'
 ...
 * Used by generic webhooks and the default handler.
 */
 ...
 * Shared by providers that use a simple event-type filter (Stripe, etc.).
 */
