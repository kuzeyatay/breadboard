// Vendored from simstudioai/sim (Apache-2.0) — apps/sim/executor/variables/resolver.ts; adapted for Breadboard.
...
import { mergeLargeValueKeys } from '@/lib/sim/core/execution/payloads/access-keys'
import { isLargeArrayManifest } from '@/lib/sim/core/execution/payloads/large-array-manifest-metadata'
import {
  ...
  isLargeValueRef,
  type LargeValueRef,
} from '@/lib/sim/core/execution/payloads/large-value-ref'
import { isLikelyReferenceSegment } from '@/lib/sim/core/workflows/sanitization/references'
import { BlockType, parseReferencePath, REFERENCE } from '@/lib/sim/executor/constants'
import type { ExecutionState, LoopScope } from '@/lib/sim/executor/execution/state'
import type { ExecutionContext } from '@/lib/sim/executor/types'
import { createEnvVarPattern, createReferencePattern } from '@/lib/sim/executor/utils/reference-validation'
import { BlockResolver } from '@/lib/sim/executor/variables/resolvers/block'
import { EnvResolver } from '@/lib/sim/executor/variables/resolvers/env'
import { LoopResolver } from '@/lib/sim/executor/variables/resolvers/loop'
import { ParallelResolver } from '@/lib/sim/executor/variables/resolvers/parallel'
import {
  type AsyncPathNavigator,
  RESOLVED_EMPTY,
  type ResolutionContext,
  type Resolver,
} from '@/lib/sim/executor/variables/resolvers/reference'
import { WorkflowResolver } from '@/lib/sim/executor/variables/resolvers/workflow'
import type { SerializedBlock, SerializedWorkflow } from '@/lib/sim/serializer/types'
...
export class VariableResolver {
  ...
  private async maybeOffloadInlineFunctionContextValue(
    ...
    try {
      const { storeLargeValue } = await import('@/lib/sim/core/execution/payloads/store')
