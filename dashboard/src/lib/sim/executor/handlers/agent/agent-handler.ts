// Vendored from simstudioai/sim (Apache-2.0) — apps/sim/executor/handlers/agent/agent-handler.ts; adapted for Breadboard.
import { createLogger } from '@/lib/sim/core/logger'
import { toError } from '@/lib/sim/core/utils/errors'
import { sleep } from '@/lib/sim/core/utils/helpers'
import { isPlainRecord } from '@/lib/sim/core/utils/object'
import { truncate } from '@/lib/sim/core/utils/string'
import { normalizeStringRecord, normalizeWorkflowVariables } from '@/lib/sim/core/core/utils/records'
import {
  projectModelSchemaAnnotations,
  projectResolvedModelInput,
  selectModelSchemaInputPaths,
} from '@/lib/sim/core/execution/model-input-provenance'
import type { McpToolSchema } from '@/lib/sim/core/mcp/types'
import { createMcpToolId } from '@/lib/sim/core/mcp/utils'
import {
  type AutoMediaKind,
...
} from '@/lib/sim/core/model-router/resolve'
import { importWorkspaceFileSecretProvenanceForModelView } from '@/lib/sim/core/uploads/contexts/workspace/workspace-file-secret-provenance'
import {
  getFileExtension,
  MODEL_SUPPORTED_IMAGE_MIME_TYPES,
  processFilesToUserFiles,
  type RawFileInput,
} from '@/lib/sim/core/uploads/utils/file-utils'
import { selectModelBoundFileInputPaths } from '@/lib/sim/core/uploads/utils/model-input'
import { hydrateUserFilesWithBase64 } from '@/lib/sim/core/uploads/utils/user-file-base64.server'
import { resolveCustomBlockToolBinding } from '@/lib/sim/core/workflows/custom-blocks/operations'
import { getCustomToolById } from '@/lib/sim/core/workflows/custom-tools/operations'
import { getAllBlocks } from '@/lib/sim/blocks'
import { assembleCustomBlockInputMapping, isCustomBlockType } from '@/lib/sim/blocks/custom/build-config'
import type { BlockOutput } from '@/lib/sim/blocks/types'
import { normalizeFileInput } from '@/lib/sim/blocks/utils'
import {
  validateBlockType,
...
} from '@/lib/sim/core/ee/access-control/utils/permission-check'
import { AGENT, BlockType, DEFAULTS, stripCustomToolPrefix } from '@/lib/sim/executor/constants'
import { memoryService } from '@/lib/sim/executor/handlers/agent/memory'
import {
  buildLoadSkillTool,
  buildSkillsSystemPromptSection,
  resolveSkillMetadata,
} from '@/lib/sim/executor/handlers/agent/skills-resolver'
import type {
  AgentInputs,
  Message,
  StreamingConfig,
  ToolInput,
} from '@/lib/sim/executor/handlers/agent/types'
import { parseResponseFormat } from '@/lib/sim/executor/handlers/shared/response-format'
import type { BlockHandler, ExecutionContext, StreamingExecution } from '@/lib/sim/executor/types'
import { collectBlockData } from '@/lib/sim/executor/utils/block-data'
import { buildAPIUrl, buildAuthHeaders } from '@/lib/sim/executor/utils/http'
import { stringifyJSON } from '@/lib/sim/executor/utils/json'
import { projectResolvedSecretDiagnosticContent } from '@/lib/sim/executor/utils/resolved-secret-content-projection'
import { prepareResolvedSecretProjectedInputs } from '@/lib/sim/executor/utils/resolved-secret-input-projection'
import { refuseResolvedSecretProjection } from '@/lib/sim/executor/utils/resolved-secret-projection-refusal'
import type {
...
} from '@/lib/sim/providers/file-attachments.server'
import { isAutoModel, SIM_AUTO_MODEL_ID } from '@/lib/sim/providers/models'
import {
  type ProviderToolInputProvenance,
  registerProviderToolInputProvenance,
  registerProviderToolModelInputRegistry,
} from '@/lib/sim/providers/tool-input-provenance'
import type { ProviderToolConfig } from '@/lib/sim/providers/types'
import { getProviderFromModel, transformBlockTool } from '@/lib/sim/providers/utils'
import type { SerializedBlock } from '@/lib/sim/serializer/types'
import { filterSchemaForLLM, type ToolSchema, ToolSchemaEnrichmentError } from '@/lib/sim/core/tools-shim/params'
import { getTool } from '@/lib/sim/tools/utils'
import { getToolAsync } from '@/lib/sim/core/tools-shim/utils.server'
...
export class AgentBlockHandler implements BlockHandler {
  ...
  private async filterUnavailableMcpTools(
    ...
    // Breadboard has no sim MCP server table; treat every referenced server as
    // available and let the tool call itself surface connectivity failures.
