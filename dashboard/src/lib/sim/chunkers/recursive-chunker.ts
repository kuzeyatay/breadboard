// Vendored from simstudioai/sim (Apache-2.0) — apps/sim/lib/chunkers/recursive-chunker.ts;
// adapted for Breadboard (relative `.ts` imports, local logger shim).

import { createLogger } from './logger.ts'
import { MAX_CHUNKING_SEPARATOR_LENGTH, MAX_CHUNKING_SEPARATORS } from './constants.ts'
import type { Chunk, RecursiveChunkerOptions } from './types.ts'
import {
  addOverlap,
  buildChunks,
  cleanText,
