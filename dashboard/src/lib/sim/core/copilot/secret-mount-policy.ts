// Vendored from simstudioai/sim (Apache-2.0) — apps/sim/lib/copilot/secret-mount-policy.ts; adapted for Breadboard.
 ...
 * `all` policy; an explicit invalid scope fails closed.
 */
...
/** A selected policy denies the whole request when any reference is unlisted, so code
 * never runs with a surprising partial environment. */
