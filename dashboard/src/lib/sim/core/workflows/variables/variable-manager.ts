// Vendored from simstudioai/sim (Apache-2.0) — apps/sim/lib/workflows/variables/variable-manager.ts; adapted for Breadboard.
...
export class VariableManager {
  ...
  private static convertToNativeType(
    ...
    const unquoted: unknown =
      // `[\s\S]` rather than `.` + the `s` flag: the dashboard targets ES2017.
      typeof value === 'string' ? value.replace(/^["']([\s\S]*)["']$/, '$1') : value
