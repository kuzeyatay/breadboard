...
export { isProviderQuotaOrCreditError } from "./http-502-retry";
import {
  KNOWLEDGE_TRANSACTION_SHA256,
  createKnowledgeWriteTransaction,
  fsyncKnowledgeDirectory,
  fsyncKnowledgeFile,
  hashKnowledgeFile,
...
export {
  ...
  type KnowledgeWriteTransaction,
  type KnowledgeWriteTransactionOptions,
  type KnowledgeWriteRecovery,
} from "./knowledge-write-transaction.ts";
...
interface ClusterKnowledgeCacheEntry {
  ...
  /** When this entry was last checked against the files on disk. */
  validatedAt: number;
}
...
class InvalidKnowledgeJsonError extends Error {
  constructor(cause?: unknown) {
    super("Model response did not contain valid JSON");
    this.name = "InvalidKnowledgeJsonError";
    this.cause = cause;
  }
}
...
function parseJsonObject(value: string): unknown {
  ...
  try {
    ...
    throw new InvalidKnowledgeJsonError(parseError);
  }
}
...
// Quartz treats a raw `<` in Markdown as the beginning of an HTML element. A
// model can legitimately write comparison operators or LaTeX fragments such as
// `p<1`, but a malformed fragment like `<p<1,\\quad ...` makes Quartz's
// ContentPage plugin abort the entire garden build. Keep code and math spans
// byte-for-byte intact, and encode every other angle bracket at the Markdown
// write boundary. This is deliberately applied after frontmatter is separated:
// YAML metadata is not parsed as HTML and may contain literal operators.
const KNOWLEDGE_PROTECTED_SPANS_RE =
  /```[\s\S]*?```|~~~[\s\S]*?~~~|`[^`\n]+`|\$\$[\s\S]*?\$\$|\$[^$\n]+?\$|\\\[[\s\S]*?\\\]|\\\([\s\S]*?\\\)/g;
const KNOWLEDGE_MASK_OPEN = String.fromCharCode(0);
const KNOWLEDGE_MASK_CLOSE = String.fromCharCode(1);

export function sanitizeKnowledgeMarkdownForQuartz(content: string): string {
  const frontmatter =
    content.match(/^---\r?\n[\s\S]*?\r?\n---(?:\r?\n|$)/)?.[0] ?? "";
  const body = frontmatter ? content.slice(frontmatter.length) : content;
  const spans: string[] = [];
  const masked = body.replace(KNOWLEDGE_PROTECTED_SPANS_RE, (value) => {
    spans.push(value);
    return `${KNOWLEDGE_MASK_OPEN}${spans.length - 1}${KNOWLEDGE_MASK_CLOSE}`;
  });
  return `${frontmatter}${masked
    .replace(/</g, "&lt;")
    .replace(
      new RegExp(`${KNOWLEDGE_MASK_OPEN}(\\d+)${KNOWLEDGE_MASK_CLOSE}`, "g"),
...
function writeKnowledgeTextFile(
  ...
  content = sanitizeKnowledgeMarkdownForQuartz(content);
...
const MAX_KNOWLEDGE_JSON_ATTEMPTS = 3;
const MAX_KNOWLEDGE_TRANSPORT_ATTEMPTS = 3;

function isTransientKnowledgeGatewayError(error: unknown): boolean {
  // The council answers 502 for every route failure, including the requested
  // model and its fallbacks all refusing for quota or credits. Replaying that
  // within seconds cannot succeed, so only an unexplained 502 is retried.
  return (
    httpStatusFromError(error) === 502 &&
    !isExplicitProviderQuotaResetError(error) &&
    !isProviderQuotaOrCreditError(error)
  );
}
...
async function requestKnowledgeExtraction({
  ...
  for (let attempt = 1; attempt <= MAX_KNOWLEDGE_JSON_ATTEMPTS; attempt += 1) {
    let rawContent: string | undefined;
    for (
      let transportAttempt = 1;
      transportAttempt <= MAX_KNOWLEDGE_TRANSPORT_ATTEMPTS;
      transportAttempt += 1
    ) {
      try {
              ...
              messages: [
                { role: "system", content: KNOWLEDGE_SYSTEM_PROMPT },
                {
                  ...
                  content:
                    ...
                    (attempt > 1
                      ? "The previous response was malformed JSON. Return the complete graph again as one valid JSON object, escape every backslash inside JSON strings, and do not emit raw control characters.\n\n"
                      : "\n\n") +
                    locatedText,
                },
              ],
        ...
        if (
          !isTransientKnowledgeGatewayError(error) ||
          transportAttempt === MAX_KNOWLEDGE_TRANSPORT_ATTEMPTS
        ) {
          throw error;
        }
        onTransientTransportRetry?.(
          transportAttempt + 1,
          MAX_KNOWLEDGE_TRANSPORT_ATTEMPTS,
        );
      }
    }
    ...
    try {
      ...
      if (
        ...
        attempt === MAX_KNOWLEDGE_JSON_ATTEMPTS
      ) {
        throw error;
      }
      onMalformedJsonRetry?.(attempt + 1, MAX_KNOWLEDGE_JSON_ATTEMPTS);
    }
  }
  ...
  try {
    if (chunks.length <= 1) {
      ...
      const extraction = await requestKnowledgeExtraction({
        ...
        onTransientTransportRetry: (attempt, totalAttempts) =>
          onProgress?.(
            `Retrying concept extraction after a temporary upstream error (${attempt}/${totalAttempts})…`,
          ),
      });
      try {
        checkpoint?.save({
    ...
    for (let index = 0; index < chunks.length; index += 1) {
      ...
      try {
        const extraction = await requestKnowledgeExtraction({
          ...
          onMalformedJsonRetry: (attempt, totalAttempts) =>
            onProgress?.(
              `Retrying concept extraction for section ${index + 1} of ${chunks.length} after malformed JSON (${attempt}/${totalAttempts})…`,
            ),
          onTransientTransportRetry: (attempt, totalAttempts) =>
            onProgress?.(
              `Retrying concept extraction for section ${index + 1} of ${chunks.length} after a temporary upstream error (${attempt}/${totalAttempts})…`,
            ),
        });
...
function wikilinkTargets(body: string): string[] {
  body = withoutDetachedVisualPayloads(body);
...
/**
 * How long a caller that only needs summary figures may reuse the last
 * validated graph without re-checking the files on disk (DATA-01). Validating
 * costs one `stat` per markdown file: for a garden of several thousand notes
 * that is over a hundred milliseconds, and the dashboard was paying it on
 * every single read just to print a note count.
 */
export const GARDEN_SUMMARY_FRESHNESS_MS = 10_000;

export function scanClusterKnowledge(
  contentPath: string,
  clusterSlug: string,
  options: { migrateSources?: boolean; maxStaleMs?: number } = {},
): ClusterKnowledge {
  const clusterDir = path.join(contentPath, clusterSlug.trim());
  ...
  // A caller that asked for summary figures and validated this garden a
  // moment ago gets that answer back without touching the filesystem. Callers
  // that need an exact graph pass no window and always revalidate.
  if (options.maxStaleMs !== undefined) {
    const fresh = clusterKnowledgeCache.get(path.resolve(clusterDir));
    if (
      fresh &&
      fresh.expiresAt > Date.now() &&
      Date.now() - fresh.validatedAt <= options.maxStaleMs
    ) {
      return fresh.knowledge;
    }
  }
  ...
  for (const { entry, filePath, folder, relPath, stat } of markdownEntries) {
    ...
    const content = fs.readFileSync(filePath, "utf-8");
    const { data, body: rawBody } = parseMarkdownFile(content);
    const body = withoutDetachedVisualPayloads(rawBody);
    const slug = entry.replace(/\.md$/, "");
