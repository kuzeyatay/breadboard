...
export function loadSpotifyAgentDefinition(): SpotifyAgentDefinition {
  return {
    ...
    instructions: [
      ...
      "For a new play request, call spotify_play with query. Breadboard starts the resolved queue on an available Spotify phone and also renders the inline player. If no phone is available, status=ready means the queue is only prepared in the inline player; status=playing means playback started. Respond naturally and do not use a fixed announcement.",
