...
// streaming answer: it appears as it is written and stays as the body of the
// turn. Around it sit the two things a long run needs to be legible — a line
// for what the workspace is doing, and the files the run left behind.
//
...
export default function InlineOpenscienceRun({
  runId,
  task,
  persistedContent = "",
  persistedOutcome,
  ...
  const [failedTools, setFailedTools] = useState(0);
  ...
  const base = `/api/openscience/runs/${runId}`;
  ...
  const reportTerminal = useCallback(
    (outcome: "completed" | "failed" | "aborted", content: string) => {
      ...
      if (outcome === "completed") notifyTaskCompleted(`OpenScience — ${task.slice(0, 80)}`);
    ...
    (event: RunEvent) => {
      ...
      switch (event.type) {
        case "run.started":
          ...
          setHarness(asString(payload.harness));
          break;
        ...
        case "tool.used":
          ...
          break;
        case "tool.failed":
          // Shown rather than counted silently: a run whose experiment would
          // not execute is the failure mode that reads most like a lazy model.
          setFailedTools((current) => current + 1);
          break;
        case "permission.answered": {
          if (asString(payload.response) !== "reject") break;
          const permission = asString(payload.permission);
          setRefusals((current) =>
        ...
        case "deliverable.ready":
          setDeliverables((current) => {
            ...
            if (!filePath || current.some((item) => item.path === filePath)) return current;
            return [...current, { path: filePath, size: asNumber(payload.size) }];
          });
          break;
        case "run.completed": {
        ...
        case "run.aborted": {
          ...
          const message =
            ...
            (outcome === "aborted" ? "The run was stopped." : "The OpenScience run failed.");
          setStatus(outcome);
          setFailure(message);
        ...
        onUnavailable: (reason) => {
          ...
          setFailure(
            reason === "run_not_found"
              ...
              : "The OpenScience event stream is unavailable.",
          );
        },
      });
    };
    ...
    <>
      ...
      <div className="bb-agent-run-card overflow-hidden">
        ...
        <div className="space-y-[13px] p-[21px]">
          {/* The runtime loads its skill library and opens its storage before
              the first token can arrive, and that is the slowest part of a cold
              run — so it is said out loud rather than left as an empty card. */}
          {!terminal && !answer ? (
          ...
          {deliverables.length ? (
            <section className="bb-agent-run-panel p-[13px]">
              <p className="bb-agent-run-label mb-[8px]">Made · {deliverables.length}</p>
                ...
                {deliverables.map((item) => (
                  <li key={item.path}>
                    <a
                      ...
                      href={`${base}/deliverables?path=${encodeURIComponent(item.path)}`}
                      download
                    >
          ...
          {refusals.length ? (
            <p className="bb-agent-run-label">
              Kept inside the workspace — refused {refusals.join(", ")}
            </p>
          ) : null}
          ...
          {tools || failedTools ? (
            <p className="bb-agent-run-label">
              ...
              {failedTools ? ` · ${failedTools} failed` : ""}
            </p>
          ) : null}
        </div>
      </div>
