...
export async function GET(
  ...
  try {
    ...
    return await outerAgentEventsResponse({
      request,
      runId,
      readView: (since) => readOuterAgentRunView("legal", userId, runId, since),
      // A long agreement can be quiet for minutes; this preserves the card's
      // historical replay cadence and keepalive behavior.
      pollMs: 800,
    });
  } catch (error) {
