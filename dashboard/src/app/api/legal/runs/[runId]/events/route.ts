...
import { getEventsSince, isTerminal } from "@/lib/legal/run-manager.ts";
  ...
  try {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        // A model reading a long agreement is quiet for minutes at a time, so
        // the ping is what keeps the connection from being reaped mid-review.
