...
import { startRun } from "@/lib/open-gym/run-manager.ts";
...
export async function POST(request: Request) {
  try {
    ...
    const requestedEffort = typeof body.reasoningEffort === "string"
    ...
    const reasoningEffort = requestedEffort === "max"
      ? "xhigh"
    ...
    const run = startRun({
      ...
      reasoningEffort,
      baseUrl: baseURL,
      maxSteps: maxStepsSetting(agentSettingsFor(userId, "open-gym"), 16),
