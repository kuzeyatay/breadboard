...
export async function POST(request: Request) {
  try {
    ...
    if (text.length > 64 * 1024) return NextResponse.json({ ok: false, error: "payload_too_large" }, { status: 413 });
    const body = text ? JSON.parse(text) as Record<string, unknown> : {};
    const brief = typeof body.brief === "string" ? body.brief.trim() : "";
    const model = typeof body.model === "string" ? body.model.trim() : "";
    if (!brief) return NextResponse.json({ ok: false, error: "empty_brief" }, { status: 400 });
    if (brief.length > 20_000) return NextResponse.json({ ok: false, error: "brief_too_long" }, { status: 400 });
    ...
    const conflict = findCapabilityConflict({
      ...
      activeRuntimeAgentId: "resource2skill",
    });
    if (conflict) return NextResponse.json({ ok: false, error: conflict.code, message: conflict.message }, { status: 400 });
    const parsed = parseResource2SkillBrief(brief);
    if (!parsed.task) return NextResponse.json({ ok: false, error: "empty_brief" }, { status: 400 });
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "runtime_error" }, { status: 502 });
  }
}
