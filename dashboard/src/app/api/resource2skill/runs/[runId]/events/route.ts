...
import { getEventsSince, isTerminal } from "@/lib/resource2skill/run-manager.ts";
...
export async function GET(request: Request, { params }: { params: Promise<{ runId: string }> }) {
  try {
      ...
      start(controller) {
        const flush = () => {
          for (const event of getEventsSince(userId, runId, cursor)) {
            controller.enqueue(encoder.encode(`id: ${event.sequenceNumber}\nevent: ${event.type}\ndata: ${JSON.stringify(event)}\n\n`));
        ...
        timer = setInterval(() => {
          try {
            if (flush()) { if (timer) clearInterval(timer); controller.close(); }
            else controller.enqueue(encoder.encode(": ping\n\n"));
          } catch { if (timer) clearInterval(timer); try { controller.close(); } catch { /* closed */ } }
        }, 600);
        request.signal.addEventListener("abort", () => { if (timer) clearInterval(timer); try { controller.close(); } catch { /* closed */ } });
      },
    ...
    return NextResponse.json({ ok: false, error: status === 404 ? "run_not_found" : "internal_error" }, { status });
  }
}
