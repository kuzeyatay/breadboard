...
import { getEventsSince, isTerminal } from "@/lib/tradingagents/run-manager.ts";
  ...
  try {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        // A multi-agent analysis is quiet for minutes at a time, so the ping is
        // what keeps the connection from being reaped between sections.
