...
export async function GET(
  ...
  try {
    ...
    return await outerAgentEventsResponse({
      request,
      runId,
      readView: (since) => readOuterAgentRunView("trading-agent", userId, runId, since),
      // A multi-agent analysis is quiet for minutes; retain the existing
      // sub-second keepalive/replay cadence.
      pollMs: 800,
    });
  } catch (error) {
