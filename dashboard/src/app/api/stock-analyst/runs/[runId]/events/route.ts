...
import { getEventsSince, isTerminal } from "@/lib/stock-analyst/run-manager.ts";
  ...
  try {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        // A staged pipeline is quiet for a minute at a time while one agent
        // thinks, so the ping is what keeps the connection from being reaped
        // between stages.
