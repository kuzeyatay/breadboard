...
import { isSetupAction, SetupError } from "@/lib/stock-analyst/setup.ts";
import {
 ...
 * requesting the supervised backend's normal idle stop. A key is written and
    ...
    if (body.action === "stop") {
      return NextResponse.json({
        ...
        result: {
          ok: true,
          message:
            "Runtime will stop Stock Analyst after active runs release and its idle timeout expires.",
          detail: "",
        },
      });
    }
