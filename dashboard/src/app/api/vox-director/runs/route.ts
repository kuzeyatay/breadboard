...
import { parseVoxDirectorRequest, VOX_DIRECTOR_AGENT_ID } from "@/lib/vox-director/identity.ts";
...
export async function POST(request: Request) {
  try {
    ...
    // The user's stored defaults are where the production starts; a flag in the
    // brief still overrides every one of them.
    const settings = agentSettingsFor(userId, VOX_DIRECTOR_AGENT_ID);
    const parsed = parseVoxDirectorRequest(brief, voxDirectorDefaults(settings));
    if (!parsed.brief) {
    ...
    // The narrator is the voice the person already chose for speech, when they
    // chose one; the audio backend falls back to a preset voice otherwise.
    ...
    const run = startRun({
      ...
      reasoningEffort,
      baseUrl: baseURL,
      checkpoint: voxDirectorCheckpoint(settings),
      voiceProfileId,
      // The chat this was launched from, so a brief that refers back to it
      // ("make one about the second idea") resolves instead of arriving as a
      // bare fragment.
