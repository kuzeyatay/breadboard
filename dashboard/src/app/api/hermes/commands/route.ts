...
function requestedSurface(value: string | null): HermesSurface {
  return value && (HERMES_SURFACES as readonly string[]).includes(value)
    ? (value as HermesSurface)
    : "dashboard_terminal";
}
...
export async function GET(request: Request) {
  try {
    ...
    // The large Agency Agents roster has its own lazy endpoint. The main
    // capability palette stays small and still includes Breadboard's ARIS agent.
    const items = registryItemsForUser(userId, context, { includeAgencyAgents: false });
