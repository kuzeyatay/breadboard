    ...
    return NextResponse.json(
      {
        ...
        storedChatModel: state.storedChatModel,
...
/**
 * Set the model the composer's `chat` sentinel runs on. This is the
 * Intelligence menu's pick and nothing else: the background model that Learn,
 * councils and Thought Topology use is `default`, set on the profile page.
 * ChatMock validates that a configured provider can serve the id.
 */
export async function PUT(request: Request) {
  try {
    ...
    const state = await setChatModel(request, body.model);
