...
import { getEventsSince } from "@/lib/video-use/runtime-run-manager.ts";
...
export async function GET(
  ...
  try {
    ...
    if (!(request.headers.get("accept") ?? "").includes("text/event-stream")) {
      return NextResponse.json({ ok: true, events: await getEventsSince(userId, runId, since) });
    }
    return await outerAgentEventsResponse({
      request,
      runId,
      pollMs: 600,
      readView: (cursor) => readOuterAgentRunView("video-use", userId, runId, cursor),
    });
  } catch (error) {
