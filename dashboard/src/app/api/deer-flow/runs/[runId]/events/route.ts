...
import { getEventsSince, isTerminal } from "@/lib/deer-flow/run-manager.ts";
  ...
  try {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        // A DeerFlow turn is quiet for minutes at a time while a sandbox command
        // or a delegated subagent works, so the ping is what keeps the
        // connection from being reaped between steps.
