...
import { getEventsSince, isTerminal } from "@/lib/paper-trader/run-manager.ts";
  ...
  try {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        // Bringing the backend up is one quiet minute on a cold start, so the
        // ping is what keeps the connection from being reaped meanwhile.
