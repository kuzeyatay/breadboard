...
import { getEventsSince, isTerminal } from "@/lib/money-printer/run-manager.ts";
  ...
  try {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        // Encoding a minute of 1080p footage is quiet for minutes at a time, so
        // the ping is what keeps the connection from being reaped between
        // stages.
