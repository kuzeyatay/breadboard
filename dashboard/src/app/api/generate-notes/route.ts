...
function chatPlacementCandidates({
  ...
  return scanClusterKnowledge(contentPath, clusterSlug)
    .nodes.filter(
      (node) =>
        node.type !== "source-document" && node.type !== "cluster-index",
    )
    .map((node) => {
...
async function decideChatNotePlacement({
  ...
  if (candidates.length === 0)
    return { action: "create", reason: "No candidate pages exist." };
  ...
  try {
    ...
    if (
      parsed.action === "merge" &&
      candidates.some((note) => note.slug === parsed.targetSlug)
    ) {
      return parsed;
    }
    return {
      action: "create",
      reason: parsed.reason ?? "No valid merge target selected.",
    };
  } catch {
    return {
      action: "create",
      reason: "Placement decision could not be parsed.",
    };
  }
}
...
async function harmonizeChatNote({
  ...
  const fmMatch = existingFileContent.match(
    /^(---\r?\n[\s\S]*?\r?\n---\r?\n?)/,
  );
...
export async function POST(request: Request) {
  try {
    ...
    if (mode === "chat-note") {
      ...
      const preliminaryTags = semanticTagsFromText(
        `${title}\n${sourceContent}`,
        8,
      );
      ...
      if (
        placementDecision.action === "merge" &&
        placementDecision.targetSlug
      ) {
        const targetPath =
          resolveClusterNoteFile(
            contentPath,
        ...
        if (fs.existsSync(targetPath)) {
          const target = placementCandidates.find(
            (note) => note.slug === placementDecision.targetSlug,
          );
          const existingFileContent = fs.readFileSync(targetPath, "utf-8");
          ...
          try {
            if (
              ...
              fs.readFileSync(targetPath, "utf-8") !== existingFileContent
            ) {
              return NextResponse.json(
                {
                  error:
                    "The target page changed while the merge was being prepared. Please try again.",
                },
                { status: 409 },
              );
            }
      ...
      try {
        ...
        if (fs.existsSync(newNotePath)) {
          return NextResponse.json(
            {
              error:
                "A page with that name was created while this note was being prepared. Please try again.",
            },
            { status: 409 },
          );
        }
        fs.writeFileSync(newNotePath, newNoteContent, "utf-8");
        refreshClusterIndex(contentPath, normalizedClusterSlug);
      } finally {
        lease.release();
      }
      ...
      return NextResponse.json({
        success: true,
        notes: [{ slug: finalSlug, title, action: "created" }],
      });
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Could not parse textbook pages from model response" },
        { status: 500 },
      );
    }
    ...
    for (const note of notes) {
      ...
      const tags = normalizeTopicTags(
        [
          ...(note.tags ?? []),
          ...semanticTagsFromText(`${note.title}\n${note.content}`, 8),
        ],
        note.content,
        8,
      );
      ...
      const notePath = path.join(clusterDir, `${finalSlug}.md`);
      const lease = acquireGardenMutationLease(
        clusterDir,
        "save-textbook-page",
      );
      try {
        ...
        fs.writeFileSync(notePath, fileContent, "utf-8");
        refreshClusterIndex(contentPath, normalizedClusterSlug);
      } finally {
        lease.release();
      }
