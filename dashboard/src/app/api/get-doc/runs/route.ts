...
export async function POST(request: Request) {
  try {
    ...
    // the message still wins, which parseDocumentSearchRequest enforces.
    const searchRequest = parseDocumentSearchRequest(
      task,
      getDocDefaults(agentSettingsFor(userId, "get-doc")),
    );
    ...
    return NextResponse.json({ ok: true, run, request: searchRequest }, { status: 201 });
  } catch (error) {
