  ...
  try {
    ...
    return new Response(fs.readFileSync(file.path), {
      headers: {
        ...
        "Content-Security-Policy": "default-src 'none'; img-src data: https:; style-src 'unsafe-inline'; font-src data:; media-src https: data:; frame-ancestors 'self'; form-action 'none'; base-uri 'none'",
      },
    });
  } catch (error) {
