...
// Verify the Hermes migration is additive and backward compatible: an old
...
function applyMigration(db) {
  db.exec(`
    CREATE TABLE IF NOT EXISTS hermes_runtime_sessions (
      ...
      chat_session_id INTEGER, hermes_session_id TEXT, agent_name TEXT NOT NULL,
  ...
  ensureColumn(db, "hermes_runtime_sessions", "active_directory", "active_directory TEXT");
  ensureColumn(db, "hermes_runtime_sessions", "filesystem_mode", "filesystem_mode TEXT NOT NULL DEFAULT 'restricted'");
  ensureColumn(db, "hermes_runtime_sessions", "capability_mode", "capability_mode TEXT NOT NULL DEFAULT 'knowledge'");
  ensureColumn(db, "hermes_runtime_sessions", "capability_decision_id", "capability_decision_id INTEGER");
  ensureColumn(db, "hermes_capability_decisions", "authorized_path_patterns", "authorized_path_patterns TEXT NOT NULL DEFAULT '[]'");
  ensureColumn(db, "hermes_capability_decisions", "allowed_command_patterns", "allowed_command_patterns TEXT NOT NULL DEFAULT '[]'");
}
...
test("migration is additive and existing data survives", () => {
  ...
  const migratedSession = db.prepare("SELECT capability_mode, filesystem_mode FROM hermes_runtime_sessions WHERE id = ?").get(rt.lastInsertRowid);
  assert.equal(migratedSession.capability_mode, "knowledge");
  assert.equal(migratedSession.filesystem_mode, "restricted");
  const decisionColumns = db.prepare("PRAGMA table_info(hermes_capability_decisions)").all().map((column) => column.name);
  assert.ok(decisionColumns.includes("authorized_path_patterns"));
  assert.ok(decisionColumns.includes("allowed_command_patterns"));

  db.prepare(
    "INSERT INTO hermes_messages (runtime_session_id, role, content, order_index) VALUES (?, ?, ?, ?)",
  ).run(rt.lastInsertRowid, "assistant", "hello", 0);
  const rtMsg = db.prepare("SELECT content FROM hermes_messages WHERE runtime_session_id = ?").get(rt.lastInsertRowid);
...
test("migration on an empty database creates all runtime tables", () => {
  ...
  applyMigration(db);
  const tables = db
    .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'hermes%'")
    .all()
    .map((t) => t.name)
    .sort();
