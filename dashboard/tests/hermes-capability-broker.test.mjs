...
import {
  brokerCapabilities,
  summarizeGrant,
} from "../src/lib/hermes/capability-broker.ts";
...
function broker(request, options = {}) {
  const plan = planTask({
    ...
    priorRequests: options.priorRequests,
  });
...
test("deletion requires a delete grant, not merely a write grant", () => {
  const writeOnly = broker(
    "Delete the duplicate files in my Downloads folder after showing me the candidates.",
    {
      grants: [
...
test("an unidentified deletion is never satisfied by an unrelated delete grant", () => {
  // The grant carries the delete bit for Downloads. The request names nothing,
  // so there is no target inside Downloads or anywhere else, and borrowing that
  // grant would authorize removal in a folder the request never mentioned.
  const grant = broker("please delete them all", {
    grants: [grantRoot({ permissions: { read: true, delete: true } })],
  });
  assert.ok(!grant.grantedCapabilities.includes("destructive_filesystem"));
  assert.deepEqual(grant.authorizedRoots, []);
});
...
test("a non-interactive delivery channel never exposes Terminal execution", () => {
  const grant = broker("Run the existing test suite and explain the failures.", {
    grants: [grantRoot({ canonicalPath: "/repo", displayName: "repo" })],
