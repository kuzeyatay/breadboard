...
import { authorizeQuartzAccess } from "../src/lib/hermes/quartz-support.ts";
...
function cleanup() {
  db.prepare("DELETE FROM hermes_runtime_sessions WHERE workspace_key LIKE ?").run(`${P}%`);
...
test("deleting a chat session cascades to its runtime session (no orphaned runtime access)", () => {
  ...
  const rt = Number(
    db.prepare(
      "INSERT INTO hermes_runtime_sessions (surface, user_id, chat_session_id, agent_name, cluster_id, garden_id, workspace_key) VALUES ('garden_chat', ?, ?, 'breadboard-garden', ?, ?, ?)",
    ).run(userId, cs, ids.pubChat, `${P}pubchat`, `${P}wk`).lastInsertRowid,
  );
  assert.ok(db.prepare("SELECT 1 FROM hermes_runtime_sessions WHERE id = ?").get(rt));
  ...
  assert.equal(db.prepare("SELECT 1 FROM hermes_runtime_sessions WHERE id = ?").get(rt), undefined);
});
