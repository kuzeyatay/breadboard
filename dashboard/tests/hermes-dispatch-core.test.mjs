...
import {
  ...
  buildDiagnostics,
  redactPath,
} from "../src/lib/hermes/dispatch-core.ts";

const WORKSPACE = "/runtime/hermes/ws1";
...
function prepare(request, options = {}) {
  return prepareTurn({
    ...
    interactiveApprovals: options.interactiveApprovals,
...
test("a conversational turn is never blocked on a folder it never mentioned", () => {
  // Regression: "group" is a filesystem mutation verb, so this advice question
  // planned a targetless write, and the turn stalled on a permission prompt
  // with no path instead of reaching the model.
  const prepared = prepare(
    "year2, course codes are 5ece0 and 5epf0 and i dont have any electives that i am considering, and it shouldnt be a group thing",
  );
  assert.equal(prepared.blocked, false);
...
test("an unresolved deletion never borrows an unrelated delete grant", () => {
  // The shape that produced a lecture instead of a permission request: the
  // previous turn listed folders, "please delete them all" resolved to nothing,
  // and the only standing grant (Downloads, with delete) satisfied the plan. The
  // turn then ran with deletion authorized in a folder nobody had mentioned, and
  // the only work left for the model was to explain its own permissions.
  const downloads = path.join(path.parse(WORKSPACE).root, "Users", "me", "Downloads");
  const prepared = prepare("please delete them all", {
    grants: [
      root({
        canonicalPath: downloads,
        displayName: "Downloads",
  ...
  assert.deepEqual(prepared.grant.authorizedRoots, []);
  assert.deepEqual(prepared.decision.authorizedDeleteTargets, []);
  assert.equal(prepared.decision.mode, "knowledge");
  // Nothing was identified, so there is no folder to put on a permission card:
  // the turn goes to the model to ask which files are meant.
  assert.equal(prepared.blocked, false);
});

test("verified folders from the previous answer become their own approvals", () => {
  const folders = [
    path.join(path.parse(WORKSPACE).root, "Users", "Public", "wpilib"),
    path.join(path.parse(WORKSPACE).root, "ProgramData", "National Instruments"),
  ];
  const prepared = prepare("please delete them all", {
    grants: [],
    resolvedResources: folders.map((value) => ({
      kind: "path",
      value,
      absolute: true,
  ...
  assert.equal(prepared.blocked, true);
  // Each folder is offered as itself. A folder read as a *file* would put its
  // parent on the card instead, which for C:\Users\me\.gradle means asking to
  // delete inside the whole user profile.
  assert.deepEqual(
    prepared.pendingPermissions.map((item) => item.path),
    folders,
  );
  for (const permission of prepared.pendingPermissions) {
    assert.equal(permission.kind, "filesystem");
    assert.deepEqual(permission.operations, ["read", "delete"]);
    assert.equal(permission.targetPath, undefined);
  }
});

test("a later missing folder grant pauses the turn instead of half-running it", () => {
  // The first step is satisfiable from an existing grant, but the step that
  // delivers the outcome is not. Only the user can supply the missing grant, so
  // dispatching would spend the turn on an explanation rather than the work.
  const documents = path.join(os.homedir(), "Documents");
  const prepared = prepare("Delete the duplicate files in my Documents folder.", {
    grants: [
      root({
  ...
  assert.ok(prepared.grant.grantedCapabilities.includes("filesystem_read"));
  assert.ok(prepared.grant.withheldCapabilities.includes("destructive_filesystem"));
  assert.equal(prepared.blocked, true);
  assert.equal(prepared.pendingPermissions[0].path, documents);
});
...
test("a non-interactive turn cannot project Terminal execution authority", () => {
  const prepared = prepare("Run the existing test suite and explain the failures.", {
    grants: [root({ canonicalPath: "/repo", displayName: "repo" })],
...
test("diagnostics answer the questions the status interface must answer", () => {
  ...
  const diagnostics = buildDiagnostics(prepared, {
    ...
    gardenId: "physics",
    runtimeSessionId: 42,
    hermesSessionId: "oh-1",
    eventStreamConnected: true,
  });
  assert.equal(diagnostics.backend, "hermes");
