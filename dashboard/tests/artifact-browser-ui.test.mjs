...
test('artifact browser filters, sorts, attaches in one click and fits narrow screens', { timeout: 60_000 }, async () => {
  ...
  ` }, outfile: 'fixture.js', bundle: true, write: false, platform: 'browser', format: 'iife', jsx: 'automatic', alias: { '@': path.join(root, 'src') }, plugins: [{ name: 'viewer-boundaries', setup(builder) {
    ...
    builder.onLoad({ filter: /.*/, namespace: 'fixture' }, args => ({ loader: 'tsx', resolveDir: root, contents: args.path.endsWith('viewer') ? `
      ...
      export const artifactDescription=item=>item.kind,artifactPdfHref=()=>null,artifactUrl=item=>'/thumb/'+item.id;
  ...
  const utilityCss = await postcss([tailwind()]).process('@import "tailwindcss" source(none); @source "../src/app/components/hermes/artifact-panel.tsx";', { from: path.join(root, 'tests/archive-fixture.css') });
  ...
  try {
    ...
    await page.route('http://localhost:53148/**',route=>{
      ...
      if(url.pathname==='/api/hermes/artifacts')return route.fulfill({json:{artifacts:items}});
    ...
    await page.getByRole('button',{name:'Attach Energy dashboard to chat',exact:true}).waitFor();
    await page.getByRole('button',{name:'Web & interactive',exact:true}).click();
    assert.equal(await page.getByRole('button',{name:/^Attach .* to chat$/}).count(),2);
    await page.getByRole('searchbox').fill('heat html');
    assert.equal(await page.getByRole('button',{name:/^Attach .* to chat$/}).count(),1);
    await page.getByRole('button',{name:'Attach Heat transfer explorer to chat',exact:true}).click();
    await page.getByRole('button',{name:'Remove Heat transfer explorer from chat',exact:true}).waitFor();
    assert.equal(await page.evaluate(()=>window.attachCalls),1);
    await page.getByRole('button',{name:'Remove Heat transfer explorer from chat',exact:true}).click();
    await page.getByRole('button',{name:'Attach Heat transfer explorer to chat',exact:true}).waitFor();
    assert.equal(await page.evaluate(()=>window.attachCalls),2);
    await page.getByRole('searchbox').fill('missing');
    await page.getByRole('button',{name:'Clear filters',exact:true}).click();
    assert.equal(await page.getByRole('button',{name:/^Attach .* to chat$/}).count(),5);
    await page.getByRole('combobox',{name:'Sort artifacts'}).selectOption('name');
    assert.equal(await page.getByRole('button',{name:/^Attach .* to chat$/}).first().getAttribute('aria-label'),'Attach Autumn in the garden to chat');
    ...
    await page.getByRole('button',{name:'Attach Energy dashboard to chat',exact:true}).click();
    await page.getByRole('button',{name:'Remove Energy dashboard from chat',exact:true}).waitFor();
