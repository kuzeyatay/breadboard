...
function surfaceCard(root, file) {
  ...
  visit(source);if(!found)throw Error("Missing music card in "+file);
  let slot;
  function findSlot(node) {
    if(ts.isJsxOpeningElement(node) && node.tagName.getText(source)==="MessageActionsSlot") slot=node.getText(source);
    ts.forEachChild(node,findSlot);
  }
  findSlot(source);if(!slot)throw Error("Missing message action slot in "+file);
  return {card:found,slot};
}
export async function musicBrowser() {
  ...
  const bundle=await esbuild.build({stdin:{resolveDir:root,loader:"tsx",contents:`
    ...
    import {MessageActionsSlot} from './src/app/components/assistant-message-actions';
    ...
    const runInFlight=false,lastVisibleAssistantIndex=0,delegationInFlight=false,delegationInterrupted=false,editingAssistantMessageId=null,assistantMessageEditId='music-message';
    const branchNavigationForAssistant=()=>({current:2,total:2,onPrevious:()=>window.branches.push(-1),onNext:()=>window.branches.push(1)});
    const branchForAssistant=()=>({id:'music-branch',activeIndex:1,variants:[{},{}]}),onSwitchBranch=(_id,direction)=>window.branches.push(direction);
    ...
    const card=surface==='garden'?(${garden.slot}${garden.card}</MessageActionsSlot>):(${terminal.slot}${terminal.card}</MessageActionsSlot>);
  ...
  const cssSource=fs.readFileSync(path.join(root,"src/app/globals.css"),"utf8").replace('@source "../**/*.{js,mjs,cjs,ts,tsx,jsx,mdx}";','@source "./components/hermes/inline-music-producer-run.tsx"; @source "./components/assistant-message-actions.tsx"; @source "./components/agents/music-producer-setup.tsx";');
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    if(req.url.startsWith("/api/")){
      ...
      else if(req.url==="/api/music-producer/health") {if(fixture.failHealth)res.statusCode=503;res.end(JSON.stringify(fixture.failHealth?{error:"Provider unavailable"}:{state:fixture.healthState??"stopped",message:fixture.healthMessage??"Prepared. Starts on generation.",stoppedGate:fixture.stoppedGate,settings:fixture.settings??{mode:"managed",externalUrl:"",model:"acestep-v15-turbo",resonantSlug:"",keyConfigured:false},resonant:"Not configured."}));}
      else if(req.url==="/api/music-producer/settings") {
        if(fixture.failSettings){res.statusCode=400;res.end(JSON.stringify({error:"The endpoint URL is invalid."}));}
        else {const value=JSON.parse(body);fixture.settings={...value,keyConfigured:value.apiKey===undefined?Boolean(fixture.settings?.keyConfigured):Boolean(value.apiKey)};delete fixture.settings.apiKey;res.end('{"ok":true}');}
      }
      else if(req.url.startsWith("/api/music-producer/setup")){
        if(req.method==="GET"&&fixture.failSetup){res.statusCode=503;res.end('{"error":"Setup status is unavailable."}');}
        else {if(req.method==="POST")fixture.setupState="running";if(req.method==="DELETE")fixture.setupState="cancelled";res.end(JSON.stringify({jobId:fixture.setupState?"job_setup":null,state:fixture.setupState,stage:fixture.setupState,message:fixture.setupMessage,detail:fixture.setupDetail}));}
      }
  ...
  await context.addInitScript(()=>{
    window.streams=[];window.terminals=[];window.notifications=[];window.retries=0;window.edits=[];window.branches=[];
