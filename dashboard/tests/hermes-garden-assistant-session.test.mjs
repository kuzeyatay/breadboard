...
// `chatSessionId`. The Hermes garden adapter authorizes that id against
...
test("new and restored Garden chats are canonically bound before their first turn", () => {
  assert.match(
    chatSessionsRoute,
    /ensureConversationForLegacyChatSession\(\s*Number\(result\.lastInsertRowid\),\s*userId/,
  );
  assert.match(adapter, /ensureConversationForLegacyChatSession\(/);
  assert.doesNotMatch(adapter, /getConversationForLegacyChatSession\(/);
});

test("Garden Chat receives the same durable memory bundle as Terminal", () => {
  // The invariant is that both surfaces load the bundle through one shared
  // function, not what that function is called — asserting the name alone
  // would pass while Garden Chat quietly drifted onto its own loader.
  const loaderName = (source) =>
    source.match(/(loadConversationMemoryBundle\w*)\(\{/)?.[1] ?? null;
  const gardenLoader = loaderName(adapter);
  assert.ok(gardenLoader, "Garden Chat must load a conversation memory bundle");
  assert.equal(
    gardenLoader,
    loaderName(turnService),
    "Garden Chat and Terminal must load the same bundle",
  );
  assert.match(adapter, /projectScopeId: "breadboard"/);
  assert.match(adapter, /composeMemoryContext\(memory\)/);
  assert.ok(
    adapter.indexOf("composeMemoryContext(memory)") <
      adapter.indexOf("gardenTurnContext("),
    "shared memory must be composed before the narrower active-Garden context",
  );
});
...
test("a blocked turn never prompts the model", () => {
  ...
  assert.match(shortCircuit, /sink\.close\(\)/);
...
test("both surfaces apply the brokered tool map through one shared helper", () => {
  ...
  const core = read("dashboard/src/lib/hermes/dispatch-core.ts");
