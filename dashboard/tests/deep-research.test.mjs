...
test("the command is recognized with or without a trailing question", () => {
  ...
  assert.equal(identity.taskFromDeepResearchCommand("/agents:deep-research tidal energy"), "tidal energy");
  assert.equal(identity.taskFromDeepResearchCommand("  /Agents:Deep-Research  tides  "), "tides");
  assert.equal(identity.taskFromDeepResearchCommand("/agents:deep-research"), "");
  // Not the command: a normal prompt must never be hijacked.
  assert.equal(identity.taskFromDeepResearchCommand("research tidal energy"), null);
  assert.equal(identity.taskFromDeepResearchCommand("/agents:deep-researchx tides"), null);
  assert.equal(identity.taskFromDeepResearchCommand("/agents:agent-tars open example.com"), null);
});

test("a plain question uses the chat defaults", () => {
  assert.deepEqual(identity.parseResearchRequest("how do tidal turbines scale?"), {
...
test("inline flags replace the removed dialog's controls", () => {
  assert.deepEqual(identity.parseResearchRequest("--answer -b 5 -d 3 who runs the grid?"), {
    query: "who runs the grid?",
    breadth: 5,
    depth: 3,
    output: "answer",
  });
  assert.deepEqual(identity.parseResearchRequest("shore power --breadth=4 --depth=2"), {
    query: "shore power",
    breadth: 4,
    depth: 2,
    output: "report",
...
test("flag values are clamped and unknown flags stay part of the question", () => {
  const clamped = identity.parseResearchRequest("--breadth 99 --depth 0 grid inertia");
...
test("the user message keeps the command so the transcript replays correctly", () => {
  ...
  assert.equal(identity.deepResearchUserMessage("   "), "/agents:deep-research");
});
...
test("selecting Deep Research activates it in chat instead of opening a dialog", () => {
  ...
  assert.equal(fs.existsSync(fileURLToPath(new URL("../src/app/components/agents/deep-research.tsx", import.meta.url))), false);
...
test("every chat host routes the command to a run and renders it inline", () => {
  ...
  for (const host of [terminal, garden]) {
    assert.match(host, /taskFromDeepResearchCommand\(text\)/);
    assert.match(host, /deepResearch\.launch\(/);
    assert.match(host, /routeDeepResearchCommand\(previousUser\.content, \{ branchGroupId \}\)/);
...
test("a delegated run stays private and leaves the originating assistant message intact", () => {
  ...
  const panel = source("src/app/components/hermes/agent-runtime-panel.tsx");
  const route = source("src/app/api/hermes/sessions/[sessionId]/external-turns/route.ts");

  assert.match(session, /beginDelegatedExternalAgentTurn/);
  assert.match(session, /pendingDelegatedExternalTurnRef\.current/);
  assert.match(session, /delegatedExternalTurnIdsRef\.current\.add\(clientMessageId\)/);
  assert.match(session, /attachToExistingTurn =\s*\n\s*input\.attachToExistingTurn === true \|\|\s*\n\s*delegatedExternalTurnIdsRef\.current\.has/);
...
test("duplicate submit events cannot launch duplicate Deep Research turns", () => {
  ...
  const garden = source("src/app/components/hermes/garden-agent-chat.tsx");
  const hook = source("src/app/components/hermes/use-deep-research-agent.ts");
  const legacyGarden = source("src/app/gardens/[clusterSlug]/workspace-client.tsx");

  assert.match(hook, /const launchingRef = useRef\(false\)/);
  assert.match(hook, /if \(launchingRef\.current\) return/);
  assert.match(hook, /launchingRef\.current = true;[\s\S]*?setLaunching\(true\)/);
  assert.match(hook, /finally \{[\s\S]*?launchingRef\.current = false;[\s\S]*?setLaunching\(false\)/);
  for (const host of [terminal, garden]) {
    assert.match(host, /const deepResearchDispatchingRef = useRef\(false\)/);
    assert.match(host, /deepResearch\.launching \|\| deepResearchDispatchingRef\.current/);
    assert.match(host, /finally \{[\s\S]*?deepResearchDispatchingRef\.current = false/);
  }
...
test("the inline run card uses the paper design tokens, not the dialog's palette", () => {
  const inline = source("src/app/components/hermes/inline-deep-research-run.tsx");
...
test("mode defaults to optional and only accepts known values", () => {
  ...
  assert.equal(config.deepResearchMode({ DEEP_RESEARCH_MODE: "REQUIRED" }), "required");
  assert.equal(config.deepResearchMode({ DEEP_RESEARCH_MODE: "disabled" }), "disabled");
  assert.equal(config.deepResearchMode({ DEEP_RESEARCH_MODE: "nonsense" }), "optional");
  assert.equal(config.deepResearchEnabled({ DEEP_RESEARCH_MODE: "disabled" }), false);
...
test("run requests are validated with stable error codes", () => {
  assert.deepEqual(config.validateRunRequest(null), { ok: false, error: "invalid_body" });
  assert.deepEqual(config.validateRunRequest({}), { ok: false, error: "invalid_query" });
  assert.deepEqual(config.validateRunRequest({ query: "   " }), { ok: false, error: "invalid_query" });
  assert.deepEqual(
    config.validateRunRequest({ query: "x".repeat(config.RUN_LIMITS.maxQueryLength + 1) }),
  ...
  assert.deepEqual(config.validateRunRequest({ query: "tides", breadth: 2.5 }), {
    ok: false,
    error: "invalid_breadth",
  });
...
test("valid run requests are normalized (trimmed query, defaults, report output)", () => {
  ...
  assert.deepEqual(result, {
    ok: true,
    value: { query: "tidal energy", breadth: 3, depth: 2, output: "report" },
  });
  ...
  assert.equal(config.validateRunRequest({ query: "q", output: "essay" }).value.output, "report");
});
...
test("health normalization keeps only known, non-secret fields", () => {
  const normalized = client.normalizeHealth({
    ...
    version: "1.0.0",
    model: { provider: "chatmock", model: "gpt-5.6-sol", endpoint: "http://127.0.0.1:8765/v1" },
...
test("a malformed, unhealthy, or wrong-service body normalizes to unavailable", () => {
  for (const body of [
    ...
    assert.deepEqual(normalized.persistence, { configured: false, healthy: false });
  }
});
...
const server = http.createServer((request, response) => {
  ...
  if (request.url === "/health") return send(200, { status: "ok", ready: false, search: {} });
  if (request.url?.startsWith("/runs/missing")) return send(404, { ok: false, error: "run_not_found" });
  if (request.url?.startsWith("/runs/r1/events")) {
    return send(200, { ok: true, data: [{ sequenceNumber: 1, type: "run.started", at: "now", payload: {} }] });
  }
...
test("service error codes surface as typed errors, not raw HTTP detail", async () => {
  await assert.rejects(
    ...
    (error) => error instanceof client.DeepResearchServiceError && error.code === "run_not_found",
  );
});
...
test("an unreachable service reports unavailable instead of throwing from health()", async () => {
  ...
  await assert.rejects(
    () => offline.getRun("r1", 1),
    (error) => error instanceof client.DeepResearchServiceError && error.code === "unavailable",
  );
});
...
test("structured research evidence keeps only registered, safe web sources", () => {
  const snapshot = events.normalizeResearchEvidenceSnapshot({
    ...
    evidence: [
      { id: "E1", claim: "The report was published.", sourceIds: ["S1", "missing"] },
      { id: "E2", claim: "A claim with no registered source.", sourceIds: ["S2"] },
    ],
    warnings: [
      { code: "branch_failed", message: "One branch timed out.", recoverable: true },
    ],
  });
...
test("legacy visited URLs normalize into stable source records", () => {
  const snapshot = events.normalizeResearchEvidenceSnapshot({
    visitedUrls: ["https://example.com/a", "https://example.com/a", "http://example.com/b"],
  });
  assert.deepEqual(
