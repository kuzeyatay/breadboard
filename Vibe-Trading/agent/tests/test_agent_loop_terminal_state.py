...
class _StubLLMNoFinal:
    ...
    Exhausts the one allowed empty-response retry because `final_content` stays
    empty and no `metrics.csv` is written.
    """
...
class _StubLLMEmptyThenSuccess(_StubLLMSuccess):
    """One empty relay turn followed by a usable final answer."""
    ...
    def stream_chat(
        ...
        response = _StubLLMResponse()
        if self.calls > 1:
            response.content = "done after retry"
        return response
...
def test_empty_model_response_returns_specific_reason(
    ...
    """Two empty no-tool outputs fail only after the bounded retry."""
    llm = _StubLLMNoFinal()
...
def test_empty_final_iteration_is_retried_once(
    tmp_path: Path,
) -> None:
    """The final text-only slot gets one retry without extending tool use."""
    llm = _StubLLMEmptyThenSuccess()
    agent = _build_agent(llm, max_iter=1, tmp_run_dir=tmp_path / "run")

    result = agent.run(user_message="anything")

    assert result["status"] == "success"
    assert result["content"] == "done after retry"
    assert result["iterations"] == 1
    assert llm.calls == 2
