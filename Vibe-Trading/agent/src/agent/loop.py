...
class AgentLoop:
    ...
    def run(self, user_message: str, history: Optional[List[Dict[str, Any]]] = None, session_id: str = "") -> Dict[str, Any]:
        ...
        empty_model_response_retries = 0
            ...
            while iteration < self.max_iterations:
                ...
                if not response.has_tool_calls:
                    ...
                    if not final_content:
                        ...
                        # A relay can occasionally close a successful HTTP turn
                        # without emitting either content or tool calls. Retry
                        # that provider turn once instead of making an otherwise
                        # healthy 18-step analysis unavailable. At the iteration
                        # ceiling, reuse the final text-only slot so the retry is
                        # still text-only and cannot extend the tool budget.
                        if empty_model_response_retries < 1:
                            empty_model_response_retries += 1
                            trace.write(
                                {
                                    "type": "empty_model_response_retry",
                                    "iter": current_iter,
                                    "attempt": empty_model_response_retries,
                                }
                            )
                            self._emit(
                                "stream_reset",
                                {
                                    ...
                                    "model": self._llm_runtime.configured_model,
                                },
                            )
                            messages.append(
                                {
                                    "role": "system",
                                    "content": (
                                        "The provider returned an empty response. Retry once and "
                                        "return the requested final answer as plain text; do not "
