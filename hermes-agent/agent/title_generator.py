"""Auto-generate short session titles from the first user prompt.
...
_TITLE_PROMPT = (
    "Describe this chat in 3-4 words from the first user prompt. "
    "The description should capture the main topic or intent. "
...
def generate_title(
    ...
    try:
        ...
        if re.match(
            r"^(?:i(?:['’]?(?:ll|d|m)|\s+(?:am|can|will))|"
            r"let(?:['’]s|\s+me)|sure\b|here(?:['’]s|\s+is))\b",
            title,
            re.IGNORECASE,
        ):
            return None
