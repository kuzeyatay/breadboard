            ...
            if delta and delta.content:
                ...
                # box is already closed (tool boundary flush).
                    ...
                    try:
                        ...
                        pass
                # Gateway consumers (the desktop) have no `stream_delta_callback`
                # — it is the CLI's own channel — so the branch above drops
                # tool-call commentary on the floor for them, and it arrives
                # whole afterwards as `message.interim(already_streamed=False)`.
                # That is what makes narration like "rendering the reel now…"
                # appear in one lump rather than typing out.
                #
                # The suppression exists so chatty text does not interleave with
                # tool cards in a shared scrollback; a gateway consumer does not
                # have that problem, because it seals streamed text into its own
                # segment at every tool boundary. So give it the deltas.
