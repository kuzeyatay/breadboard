...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            from tools.tiktok_tools import is_tiktok_url, download_tiktok_video
            if is_tiktok_url(video_url):
                logger.info("Detected TikTok video URL, resolving stream via TikTok downloader...")
                _, tiktok_meta = await download_tiktok_video(video_url, temp_video_path)
                if tiktok_meta and tiktok_meta.get("title") and tiktok_meta.get("title") not in user_prompt:
                    extra_context = []
                    if tiktok_meta.get("title"):
                        extra_context.append(f"Title/Caption: {tiktok_meta['title']}")
                    if tiktok_meta.get("author"):
                        extra_context.append(f"Creator: {tiktok_meta['author']} (@{tiktok_meta.get('author_username', '')})")
                    if tiktok_meta.get("music_title"):
                        extra_context.append(f"Audio/Sound: {tiktok_meta['music_title']}")
                    if extra_context:
                        ctx_str = "\n".join(extra_context)
