...
def _registered_web_provider(backend: str):
    ...
    Discovery is forced first. ``web_extract_tool`` resolves the configured
    backend name *before* it calls :func:`_ensure_web_plugins_loaded`, so on a
    cold dispatch the registry was still empty here: a plugin-provided
    ``web.extract_backend`` looked unregistered, ``_get_capability_backend``
    discarded it, and the shared auto-detect fell through to a search-only
    backend — which then reported itself as search-only, blaming the user's
    correct configuration. Every legacy backend name is short-circuited before
    this function, so the added discovery only runs for plugin-provided names,
    and the discovery call is itself idempotent.
    """
    if not backend:
        return None
    try:
        ...
        if provider is None:
            provider = _rescan_for_web_provider(backend)
        return provider
...
# Backend names a forced rescan has already been spent on, so a genuinely
# misspelled or uninstalled backend costs one directory sweep per process
# rather than one on every web dispatch.
_RESCANNED_WEB_BACKENDS: set = set()


def _rescan_for_web_provider(backend: str):
    """Rescan the plugin tree once for *backend*, then look it up again.

    Discovery is cached for the life of the process (``discover_and_load``
    early-returns once ``_discovered`` is set), and a long-lived server holds
    that cache across every plugin added underneath it. So a backend the user
    has configured but the registry has never heard of has two possible
    causes, and only one of them is a mistake: a typo, or a plugin that landed
    after this process started. The second is what a bundled provider added by
    an update looks like, and treating it as the first is what turns a correct
    ``extract_backend: fetch`` into a silent downgrade to the search backend —
    which then reports itself as search-only and blames the config.

    One sweep per unknown name per process: enough to pick up a new plugin,
    bounded enough that a real typo cannot make every dispatch walk the tree.
    """
    if backend in _RESCANNED_WEB_BACKENDS:
        return None
    _RESCANNED_WEB_BACKENDS.add(backend)
    try:
        ...
        provider = get_provider(backend)
    except Exception as exc:  # noqa: BLE001 — rescan is best-effort
        logger.debug("web plugin rescan for %r failed: %s", backend, exc)
        return None
    if provider is not None:
        logger.info(
            "web provider %r appeared after a plugin rescan (it was installed "
...
def _get_capability_backend(capability: str) -> str:
    ...
    # The shared fallback is auto-detected for *search* — its candidate list is
    # ordered by search credentials — so for extract it can land on a provider
    # that has no extract at all (ddgs, searxng, brave-free). Substituting one
    # of those for a configured extract backend is the worst of both: the tool
    # fails anyway, and the error names the substitute rather than the backend
    # the user actually chose. Keep the configured name so the failure is about
    # the real subject; a capable fallback is still used as before.
    if (
        specific
        and specific != fallback
        and not _backend_supports_capability(fallback, capability)
    ):
        logger.warning(
            "web.%s_backend is %r but it did not resolve; not falling back to "
            "%r, which cannot %s",
            capability,
            specific,
            fallback,
...
def _backend_supports_capability(backend: str, capability: str) -> bool:
    """Whether *backend* can serve *capability*, as far as the registry knows.

    Unknown to the registry means "assume yes": the legacy probes below own
    those names, and this helper exists only to stop a known-incapable
    provider from being substituted for a configured one.
    """
    provider = _registered_web_provider(backend)
    if provider is None:
        return True
    ...
    except Exception as exc:  # noqa: BLE001 — a broken provider claims nothing
