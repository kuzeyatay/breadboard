...
0. **Format-aware compression** (tools.tokenjuice): Before anything is cut,
   the output is compressed according to what it actually is -- a JSON array
   collapses to a few records plus a count, a log collapses its repeated
   lines, HTML sheds its markup. Most oversized results fit the budget after
   this and never reach the layers below, so the model reads a thinned
   version of the whole output instead of the first 1,500 characters of it.
   Every elision is recoverable through ``expand_output``.
...
# Marks a result the compression stage already handled, so the per-turn pass
# does not compress a compression.
JUICED_TAG = "<juiced "
...
def _try_compress(content: str, tool_name: str, threshold: int | float) -> str | None:
    """Layer 0: compress to fit the threshold, or return None to fall through.

    Isolated behind a bare ``except`` on purpose. This runs on the path every
    oversized tool result takes, and a bug in a compressor must degrade to the
    old truncating behaviour rather than fail the turn.
    """
    if threshold == float("inf") or threshold <= 0:
        return None
    try:
        from tools.tokenjuice import compress

        result = compress(content, tool_name, budget=int(threshold))
    except Exception as exc:
        logger.debug("TokenJuice unavailable for %s: %s", tool_name, exc)
        return None
    ...
    logger.info(
        "Compressed tool result: %s (%s, %d -> %d chars, %d%% saved, handle %s)",
        tool_name,
        result.fmt,
        result.original_chars,
        result.kept_chars,
        int(round(result.ratio * 100)),
        result.handle,
    )
    return result.text
...
def maybe_persist_tool_result(
    ...
    # Layer 0: try to make it fit by compressing rather than cutting. A
    # success here is strictly better than everything below -- the model sees
    # the shape of the whole output, and the elided parts stay reachable
    # through the tokenjuice cache even on backends where a sandbox write
    # would have failed.
    juiced = _try_compress(content, tool_name, effective_threshold)
    if juiced is not None:
        return juiced
...
def enforce_turn_budget(
    ...
    for i, msg in enumerate(tool_messages):
        ...
        size = len(content)
        total_size += size
        if PERSISTED_OUTPUT_TAG not in content and JUICED_TAG not in content:
    ...
    for idx, size in candidates:
        ...
        # Try compression before spilling here too. The budget is whatever
        # this one result has to give up for the turn to fit, floored at the
        # preview size so the attempt is never asked for something a spill
        # would beat anyway.
        headroom = max(config.preview_size, size - (total_size - config.turn_budget))
        replacement = _try_compress(content, msg.get("name") or _BUDGET_TOOL_NAME, headroom)
