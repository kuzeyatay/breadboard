...
def get_pending_gateway_approval(session_key: str) -> Optional[dict]:
    """Return a snapshot of the oldest queued gateway approval, if any.

    Gateway notification callbacks are intentionally best-effort: a client can
    disconnect after the approval is queued but before its one-shot event is
    delivered. Status transports use this snapshot to recover the same pending
    prompt without taking ownership of, or mutating, the queue entry.
    """
    with _lock:
        queue = _gateway_queues.get(session_key)
        if not queue:
...
def request_tool_approval(
    ...
    *,
    ...
    Args:
        ...
        display_target: Optional exact operation text to render in the approval
            card. Plugins should use this when a concrete command or resource
            is more useful than the default synthetic tool label.
    ...
    resolved_display_target = (
        display_target
        if isinstance(display_target, str) and display_target.strip()
        else f"<{tool_name}> (plugin approval rule)"
    )

    return _run_approval_gate(
        pattern_key=pattern_key,
        description=description,
        display_target=resolved_display_target,
        approval_callback=approval_callback,
        cron_deny_message=(
