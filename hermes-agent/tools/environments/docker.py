...
# The strict profile is intentionally opinionated and bounded.  These are
# security ceilings, not general Docker defaults: the standard profile keeps
# honoring the operator's configured resources exactly as before.
_STRICT_DEFAULT_CPU = 1.0
_STRICT_MAX_CPU = 2.0
_STRICT_DEFAULT_MEMORY_MB = 1024
_STRICT_MAX_MEMORY_MB = 2048
_STRICT_PIDS_LIMIT = "128"
_STRICT_MAX_COMMAND_TIMEOUT = 120
_STRICT_FALLBACK_USER = "65534:65534"
_DOCKER_SECURITY_PROFILES = frozenset({"standard", "strict"})
...
def _normalize_security_profile(value: object) -> str:
    """Return a supported Docker security profile or fail closed.

    A misspelled strict profile must never silently fall back to the standard
    container.  ``legacy`` and ``default`` remain accepted aliases for the
    historical behavior so hand-written pre-release configs are harmless.
    """
    profile = str(value or "standard").strip().lower()
    if profile in {"default", "legacy"}:
        profile = "standard"
    if profile not in _DOCKER_SECURITY_PROFILES:
        choices = ", ".join(sorted(_DOCKER_SECURITY_PROFILES))
        raise ValueError(
            f"Invalid docker_security_profile {value!r}; expected one of: {choices}"
        )
    return profile


def _strict_user_spec() -> str:
    """Choose a writable-workspace-compatible UID:GID that is never root."""
    host_spec = _resolve_host_user_spec()
    if host_spec:
        uid, _, _gid = host_spec.partition(":")
        try:
            if int(uid) > 0:
                return host_spec
        except (TypeError, ValueError):
            pass
    return _STRICT_FALLBACK_USER


def _strict_tmpfs(path: str, *, size: str, user_spec: str, exec_ok: bool) -> list[str]:
    """Build a non-root-owned, size-bounded tmpfs mount."""
    uid, _, gid = user_spec.partition(":")
    execution = "exec" if exec_ok else "noexec"
    return [
        "--tmpfs",
        (
            f"{path}:rw,{execution},nosuid,nodev,size={size},"
...
def _build_security_args(
    ...
    run_exec: bool = False,
    *,
    strict_user_spec: str | None = None,
) -> list[str]:
    ...
    """
    if strict_user_spec is not None:
        # No capability is added back in strict mode.  The image filesystem is
        # immutable; only explicitly declared, size-bounded tmpfs mounts (and
        # an optional /workspace bind) are writable.
        return [
            "--cap-drop", "ALL",
            "--security-opt", "no-new-privileges",
            "--read-only",
            *_strict_tmpfs(
                "/tmp", size="256m", user_spec=strict_user_spec, exec_ok=False
            ),
            *_strict_tmpfs(
                "/var/tmp", size="128m", user_spec=strict_user_spec, exec_ok=False
            ),
            *_strict_tmpfs(
                "/run", size="64m", user_spec=strict_user_spec, exec_ok=False
            ),
        ]
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        security_profile = _normalize_security_profile(security_profile)
        strict = security_profile == "strict"

        if strict:
            if cwd != "/workspace" and not cwd.startswith("/workspace/"):
                logger.warning(
                    "Docker strict profile restricts cwd to /workspace; "
                    "ignoring configured cwd %r.",
                    cwd,
                )
                cwd = "/workspace"
            timeout = min(max(int(timeout), 1), _STRICT_MAX_COMMAND_TIMEOUT)

            # Strict is an isolation contract, so settings that would weaken
            # it are ignored rather than appended after the secure defaults.
            if volumes:
                logger.warning(
                    "Docker strict profile ignores docker_volumes; use the "
                    "explicit cwd-to-/workspace mount when host files are needed."
                )
            if forward_env or env:
                logger.warning(
                    "Docker strict profile does not forward host or configured "
                    "environment variables."
                )
            if extra_args:
                logger.warning(
                    "Docker strict profile ignores docker_extra_args because "
                    "arbitrary run flags can bypass the isolation boundary."
                )
            volumes = []
            forward_env = []
            env = {}
        ...
        self._max_command_timeout = (
            _STRICT_MAX_COMMAND_TIMEOUT if strict else None
        )
        self._strict_timeout_poisoned = False
        ...
        if strict and not limits_available:
            raise RuntimeError(
                "Docker strict profile requires CPU, memory, and PID cgroup "
                "limits, but this Docker host cannot enforce them."
            )
        if strict:
            strict_cpu = min(
                float(cpu) if cpu and float(cpu) > 0 else _STRICT_DEFAULT_CPU,
                _STRICT_MAX_CPU,
            )
            strict_memory = min(
                int(memory) if memory and int(memory) > 0 else _STRICT_DEFAULT_MEMORY_MB,
                _STRICT_MAX_MEMORY_MB,
            )
            resource_args.extend([
                "--cpus", str(strict_cpu),
                "--memory", f"{strict_memory}m",
                "--memory-swap", f"{strict_memory}m",
                "--pids-limit", _STRICT_PIDS_LIMIT,
            ])
        else:
        ...
        if not strict and disk > 0 and sys.platform != "darwin":
        ...
        writable_args = []
        strict_user = _strict_user_spec() if strict else None
        if strict:
            if not bind_host_cwd:
                writable_args.extend(
                    _strict_tmpfs(
                        "/workspace",
                        size="1g",
                        user_spec=strict_user,
                        exec_ok=True,
                    )
                )
            writable_args.extend(
        ...
        # Strict mode deliberately has no credential, cache, or skill mounts:
        # /workspace is its sole possible host-filesystem boundary.
        try:
            if strict:
                credential_mounts = ()
                skills_mounts = ()
                cache_mounts = ()
                logger.info(
                    "Docker strict profile: automatic credential, cache, and "
                    "skill mounts are disabled."
                )
            else:
        ...
        user_args: list[str] = []
        if strict:
            user_args = ["--user", strict_user]
            logger.info("Docker strict profile: running as non-root user %s", strict_user)
        elif run_as_host_user:
        ...
        if strict and image_uses_s6_init:
            raise RuntimeError(
                "Docker strict profile cannot run an image whose s6 entrypoint "
                "requires root. Choose a non-s6 execution image."
            )
        ...
        if strict:
            label_args.extend([
                "--label", "hermes-security-profile=strict",
            ])
        ...
        if strict:
            self._labels["hermes-security-profile"] = "strict"
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # Strict sessions receive only the fixed, non-secret process identity
        # above.  This also blocks implicit skill passthrough values, not just
        # the user-facing docker_forward_env list.
        if getattr(self, "_security_profile", "standard") == "strict":
            args = []
    ...
    def _remove_strict_container(self) -> bool:
        """Synchronously remove a strict container and return whether it is gone."""
        ...
        try:
            ...
            logger.error(
                "Failed to tear down strict container %s: %s",
                container_id[:12],
                exc,
            )
            return False
        if result.returncode != 0:
            logger.error(
                "Failed to tear down strict container %s: %s",
                container_id[:12],
                (result.stderr or "docker rm returned an error").strip(),
            )
            return False
        self._container_id = None
        return True
    ...
    def execute(self, command: str, cwd: str = "", **kwargs) -> dict:
        ...
        transparently before retrying once. A timed-out strict command removes
        its whole container so in-container descendants cannot outlive the
        host-side ``docker exec`` client; the next command starts fresh.
        """
        strict = getattr(self, "_security_profile", "standard") == "strict"
        if strict and getattr(self, "_strict_timeout_poisoned", False):
            if not self._remove_strict_container():
                return {
                    "output": (
                        "Strict Docker sandbox is unavailable because its "
                        "timed-out container could not be removed."
                    ),
                    "returncode": 125,
                }
            self._strict_timeout_poisoned = False
        if strict and not self._container_id:
            if not self._recreate_container():
                return {
                    "output": "Strict Docker sandbox could not be recreated.",
                    "returncode": 125,
                }

        max_command_timeout = getattr(self, "_max_command_timeout", None)
        if max_command_timeout is not None:
            requested_timeout = kwargs.get("timeout")
            if requested_timeout is None:
                kwargs["timeout"] = max_command_timeout
            else:
                kwargs["timeout"] = min(
                    max(int(requested_timeout), 1),
                    max_command_timeout,
                )
        result = super().execute(command, cwd, **kwargs)
        if strict and result.get("returncode") == 124:
            self._strict_timeout_poisoned = True
            if self._remove_strict_container():
                self._strict_timeout_poisoned = False
        if (
