        ...
        if terminal_env is None:
            ...
            if env_type in {"docker", "singularity", "modal", "daytona"}:
                container_config = {
                    ...
                    "docker_security_profile": config.get(
...
# The four file tools take absolute paths, so the boundary between the work and
# the runtime's own state is enforced here rather than assumed. See
# tools/path_roots.py for what the two roots are and why only one of them is
# refused by default.
def _refused_by_path_roots(path, operation):
    """Return a refusal string for *path*, or None. Never raises."""
    try:
        from tools.path_roots import check_path

        refusal = check_path(str(path or ""), operation)
    except Exception:
        return None
    return tool_error(refusal) if refusal else None


def _handle_read_file(args, **kw):
    tid = kw.get("task_id") or "default"
    refused = _refused_by_path_roots(args.get("path", ""), "read")
    if refused:
        return refused
...
def _handle_write_file(args, **kw):
    ...
    refused = _refused_by_path_roots(args["path"], "write")
    if refused:
        return refused
...
def _handle_patch(args, **kw):
    tid = kw.get("task_id") or "default"
    refused = _refused_by_path_roots(args.get("path"), "write")
    if refused:
        return refused
    return patch_tool(
