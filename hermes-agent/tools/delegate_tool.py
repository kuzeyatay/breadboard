...
def _build_child_system_prompt(
    ...
    *,
    ...
    """
    # Agency-agent persona injection: the orchestrator prefixes a delegated goal
    # with `[persona: <slug>]` to give this child a specialist identity. Strip
    # the tag and, when it resolves, lead the prompt with the persona. Fails open
    # to the plain goal if the roster is unavailable or the slug is unknown.
    persona_block = None
    try:
        from tools.agency_personas import render_persona_block, split_persona_tag

        persona_slug, goal = split_persona_tag(goal)
        if persona_slug:
            persona_block = render_persona_block(persona_slug)
    except Exception:
        persona_block = None
