...
from hermes_cli.default_soul import DEFAULT_SOUL_MD, is_migratable_default_soul
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed Bread's default SOUL.md, upgrading untouched older defaults.

    First run: write DEFAULT_SOUL_MD. Existing installs whose SOUL.md is still
    the old comment-only scaffold or the exact old Hermes Agent default get
    upgraded in place to DEFAULT_SOUL_MD. A SOUL.md the user actually customized
    is never touched.
    """
    ...
    if soul_path.exists():
        try:
            ...
            return
        if not is_migratable_default_soul(existing):
            return
        # Untouched shipped default -> upgrade to Bread's identity in place.
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        "docker_network": True,
        # Opt-in defense-in-depth profile for local code execution. ``strict``
        # is keyless and enforces a non-root, networkless, read-only,
        # resource-bounded container with only /workspace eligible for a host
        # bind mount. ``standard`` preserves the historical Docker behavior.
