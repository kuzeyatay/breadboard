...
# The Runtime V2 supervisor owns these values for desktop-launched Hermes.
# User, project, bootstrap, and managed dotenv files may still override normal
# provider configuration, but they must not change the process authority or
# private control-plane values selected by the runtime.
_DESKTOP_RUNTIME_ENV_NAMES = frozenset(
    {
        "HERMES_HOME",
        "HERMES_DESKTOP",
...
def _load_dotenv_with_fallback(path: Path, *, override: bool) -> None:
    desktop_runtime_env = None
    if os.environ.get("HERMES_DESKTOP") == "1":
        desktop_runtime_env = {
            name: os.environ[name]
            for name in _DESKTOP_RUNTIME_ENV_NAMES
    ...
    try:
        ...
        if desktop_runtime_env is not None:
            for name in _DESKTOP_RUNTIME_ENV_NAMES:
                if name in desktop_runtime_env:
                    os.environ[name] = desktop_runtime_env[name]
                else:
...
def load_hermes_dotenv(
    *,
    ...
    # Breadboard QA runs the shared development program against a disposable
    # profile. Every caller—not only the CLI entrypoint—must ignore the
    # checkout fallback so websocket/session imports cannot read or sanitize
    # hermes-agent/.env behind the harness's controlled environment.
    project_env_path = (
        None
        if os.environ.get("HERMES_QA_ISOLATED") == "1"
        else Path(project_env) if project_env else None
    )
