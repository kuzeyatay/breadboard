...
_QA_ISOLATED = os.environ.get("HERMES_QA_ISOLATED") == "1"
...
load_hermes_dotenv(project_env=None if _QA_ISOLATED else PROJECT_ROOT / ".env")
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    """
    if _QA_ISOLATED or not _is_windows():
        return
    if scripts_dir is None:
        scripts_dir = _venv_scripts_dir()
    if scripts_dir is None:
...
def cmd_update(args):
    ...
    """
    if _QA_ISOLATED:
        print("Hermes update is disabled in Breadboard QA isolation.")
        return
...
def main():
    ...
    try:
        if not _QA_ISOLATED and "update" not in sys.argv[1:]:
