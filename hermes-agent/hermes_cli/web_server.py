...
async def update_hermes():
    ...
    if os.environ.get("HERMES_QA_ISOLATED") == "1":
        message = "Hermes update is disabled in Breadboard QA isolation."
...
async def run_backup(body: BackupRequest):
    ...
    if output and os.environ.get("HERMES_QA_ISOLATED") == "1":
        raise HTTPException(
            status_code=403,
            detail="Custom backup paths are disabled in Breadboard QA isolation",
        )
    if output:
...
async def run_import(body: ImportRequest):
    ...
    if os.environ.get("HERMES_QA_ISOLATED") == "1":
        qa_root_value = (os.environ.get("BREADBOARD_QA_RUN_DIR") or "").strip()
        try:
            if not qa_root_value:
                raise ValueError("missing QA run root")
            Path(archive).expanduser().resolve().relative_to(
                Path(qa_root_value).expanduser().resolve()
            )
        except (OSError, ValueError):
            raise HTTPException(
                status_code=403,
                detail="Breadboard QA imports must stay inside the isolated run directory",
            )
    args = ["import", archive]
    if body.force:
        args.append("--force")
...
async def run_toolset_post_setup(
    ...
    if os.environ.get("HERMES_QA_ISOLATED") == "1":
        raise HTTPException(
            status_code=403,
            detail="Tool dependency installation is disabled in Breadboard QA isolation",
        )
