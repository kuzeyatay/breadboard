...
def run_backup(args) -> None:
    ...
    qa_isolated = os.environ.get("HERMES_QA_ISOLATED") == "1"
    qa_root = Path(
        os.environ.get("BREADBOARD_QA_RUN_DIR") or hermes_root
    ).expanduser().resolve()
    ...
    if args.output:
        ...
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = (
            hermes_root / "backups" / f"hermes-backup-{stamp}.zip"
            if qa_isolated
            else Path.home() / f"hermes-backup-{stamp}.zip"
        )
    ...
    if qa_isolated:
        try:
            out_path.resolve().relative_to(qa_root)
        except (OSError, ValueError):
            print(
                "Error: Breadboard QA backups must stay inside the isolated "
                f"run directory ({qa_root})"
            )
            sys.exit(1)
