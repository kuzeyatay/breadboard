...
def test_prompt_submit_golden_transcript_matches_flag_off_and_on(monkeypatch):
    ...
    usage_agent = _Agent()
    usage_before = server._get_usage(usage_agent)
    usage_agent.run_conversation("hello")
    usage = server._turn_usage_delta(usage_before, server._get_usage(usage_agent))
    monkeypatch.setattr(server.threading, "Thread", _ImmediateThread)
    monkeypatch.setattr(server, "_ensure_session_db_row", lambda _session: None)
    monkeypatch.setattr(server, "_persist_branch_seed", lambda _session: None)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # platform toolset in hermes-cli's universe); `project` is GUI-only and
    # `tokenjuice` is the mandatory compressed-output recovery toolset.
...
def test_breadboard_session_create_pins_toolsets_and_system_prompt(monkeypatch):
    monkeypatch.setattr(server, "_start_agent_build", lambda sid, session: None)
    monkeypatch.setattr(server, "_missing_breadboard_required_tools", lambda _toolsets: [])
    server._sessions.clear()
    try:
        response = server.handle_request(
            {
                ...
                "params": {
                    ...
                    "system_prompt": "Breadboard owns this session context.",
                },
            }
        )
        assert "error" not in response
        session = server._sessions[response["result"]["session_id"]]
        assert session["enabled_toolsets_override"] == ["breadboard", "web"]
        assert session["breadboard_contract_validated"] is True
        assert (
            session["ephemeral_system_prompt_override"]
            == "Breadboard owns this session context."
        )
    finally:
        server._sessions.clear()


def test_breadboard_session_create_fails_when_required_web_toolset_is_omitted(monkeypatch):
    monkeypatch.setattr(server, "_start_agent_build", lambda sid, session: None)
    monkeypatch.setattr(server, "_missing_breadboard_required_tools", lambda _toolsets: [])
    server._sessions.clear()
    try:
        response = server.handle_request(
            {
                ...
                "params": {
                    "source": "breadboard",
                    "enabled_toolsets": ["breadboard"],
                },
            }
        )
        assert response["error"]["code"] == 5036
        assert "web" in response["error"]["message"]
        assert server._sessions == {}
    finally:
        server._sessions.clear()


def test_breadboard_session_create_fails_when_required_web_tool_is_missing(monkeypatch):
    monkeypatch.setattr(server, "_start_agent_build", lambda sid, session: None)
    monkeypatch.setattr(
        server,
        "_missing_breadboard_required_tools",
        lambda _toolsets: ["web_search"],
    )
    server._sessions.clear()
    try:
        response = server.handle_request(
...
def test_non_breadboard_session_cannot_override_tool_boundary(monkeypatch):
    monkeypatch.setattr(server, "_start_agent_build", lambda sid, session: None)
    server._sessions.clear()
    try:
        response = server.handle_request(
        ...
        session = server._sessions[response["result"]["session_id"]]
        assert "enabled_toolsets_override" not in session
        assert "ephemeral_system_prompt_override" not in session
    finally:
        server._sessions.clear()


def test_old_breadboard_session_is_rejected_before_prompt_submission():
    server._sessions.clear()
...
def test_breadboard_prompt_updates_only_ephemeral_session_prompt(monkeypatch):
    ...
    session = {
        ...
        "session_key": "stored-1",
        "source": "breadboard",
        "enabled_toolsets_override": ["breadboard", "web"],
    }
    server._sessions.clear()
    ...
    monkeypatch.setattr(
        server,
        "_handle_busy_submit",
        lambda rid, sid, current, text, transport: server._ok(
            rid, {"status": "queued"}
        ),
    )
    try:
        response = server._methods["prompt.submit"](
            "r1",
            {
                "session_id": "live-1",
                "text": "hello",
                "system_prompt": "Fresh Breadboard memory.",
            },
        )
        assert response["result"]["status"] == "queued"
        assert agent.ephemeral_system_prompt == "Fresh Breadboard memory."
        assert session["ephemeral_system_prompt_override"] == "Fresh Breadboard memory."
    finally:
        server._sessions.clear()
...
def test_turn_usage_delta_separates_counters_from_context_gauges():
    before = {
        "model": "test", "input": 100, "output": 10, "total": 110,
        "calls": 1, "cache_read": 40, "context_used": 8_000,
    }
    after = {
        "model": "test", "input": 340, "output": 35, "total": 375,
        "calls": 3, "cache_read": 140, "context_used": 12_500,
        "context_max": 128_000, "context_percent": 10,
    }
    usage = server._turn_usage_delta(before, after)
    assert usage["scope"] == "turn"
    assert usage["input"] == 240
    assert usage["output"] == 25
    assert usage["total"] == 265
    assert usage["calls"] == 2
    assert usage["cache_read"] == 100
    assert usage["context_used"] == 12_500
    assert usage["context_max"] == 128_000


def test_breadboard_web_toolset_requires_a_registered_search_tool(monkeypatch):
    monkeypatch.setattr(
        "model_tools.get_tool_definitions",
        lambda **_kwargs: [{"function": {"name": "web_extract"}}],
    )
    assert server._missing_breadboard_required_tools(["breadboard", "web"]) == [
        "web_search"
    ]
