...
class TestGenerateTitle:
    ...
    def test_default_prompt_matches_user_language(self):
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "Some Chat Title"
    ...
    def test_default_timeout_delegates_to_auxiliary_config(self):
        ...
        def mock_call_llm(**kwargs):
            ...
            resp.choices[0].message.content = "Configured Timeout Chat"
            return resp

        with patch("agent.title_generator.call_llm", side_effect=mock_call_llm):
            assert generate_title("question", "answer") == "Configured Timeout Chat"
    ...
    def test_explicit_timeout_still_overrides_config(self):
        ...
        def mock_call_llm(**kwargs):
            ...
            resp = MagicMock()
            resp.choices = [MagicMock()]
            resp.choices[0].message.content = "Explicit Timeout Chat"
            return resp

        with patch("agent.title_generator.call_llm", side_effect=mock_call_llm):
            assert generate_title("question", "answer", timeout=123.0) == "Explicit Timeout Chat"
    ...
    def test_rejects_titles_longer_than_four_words(self):
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "One Two Three Four Five Six"

        with patch("agent.title_generator.call_llm", return_value=mock_response):
            title = generate_title("question", "answer")
            assert title is None

    def test_rejects_an_answer_instead_of_a_title(self):
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "I'll inspect the chat-renaming"

        with patch("agent.title_generator.call_llm", return_value=mock_response):
            assert generate_title("please fix renaming", "answer") is None
    ...
    def test_sends_only_the_truncated_first_user_prompt(self):
        """The assistant answer never enters the pure title request."""
        captured_kwargs = {}
        ...
        def mock_call_llm(**kwargs):
            ...
            resp = MagicMock()
            resp.choices = [MagicMock()]
            resp.choices[0].message.content = "Short Generated Chat Title"
            return resp

        with patch("agent.title_generator.call_llm", side_effect=mock_call_llm):
            generate_title("x" * 1000, "y" * 1000)

        # The title request contains only the first prompt, capped at 500 chars.
        user_content = captured_kwargs["messages"][1]["content"]
        assert "x" * 500 in user_content
        assert "y" not in user_content
        assert "describe it; do not answer it" in user_content
...
class TestRuntimeValidator:
    ...
    def test_allows_when_validator_returns_true(self):
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "Validated Chat Title"
        ...
        with patch("agent.title_generator.call_llm", return_value=mock_response) as mock_llm:
            ...
            assert title == "Validated Chat Title"
            mock_llm.assert_called_once()

    def test_broken_validator_fails_open(self):
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "Resilient Chat Title"
        ...
        with patch("agent.title_generator.call_llm", return_value=mock_response) as mock_llm:
            ...
            assert title == "Resilient Chat Title"
            mock_llm.assert_called_once()
