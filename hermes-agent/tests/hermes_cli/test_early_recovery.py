...
def test_qa_isolated_main_skips_checkout_recovery_and_project_dotenv(tmp_path):
    hermes_home = tmp_path / "hermes_home"
    managed_dir = tmp_path / "hermes_managed"
    scripts_dir = tmp_path / "shared_venv_scripts"
    hermes_home.mkdir()
    managed_dir.mkdir()
    scripts_dir.mkdir()
    stale_exe = scripts_dir / "hermes.exe.old.qa-proof"
    stale_exe.write_text("do not delete", encoding="utf-8")
    script = tmp_path / "qa_isolated_import.py"
    script.write_text(
        textwrap.dedent(
            f"""
            import hermes_cli._early_recovery as er
            ...
            def recovery_recorder(*args, **kwargs):
                print("EARLY_RECOVERY_CALLED", flush=True)

            original_loader = env_loader.load_hermes_dotenv

            def dotenv_recorder(*args, **kwargs):
                project_env = kwargs.get("project_env", args[0] if args else None)
                print(f"PROJECT_ENV={{project_env!r}}", flush=True)
                return original_loader(*args, **kwargs)

            er.recover_if_needed = recovery_recorder
            env_loader.load_hermes_dotenv = dotenv_recorder

            import hermes_cli.main as main
            main._is_windows = lambda: True
            main._cleanup_quarantined_exes(main.Path({str(scripts_dir)!r}))

            def forbidden_marker_probe():
                raise AssertionError("QA isolation probed a checkout recovery marker")

            main._update_marker_path = forbidden_marker_probe
            main._lazy_refresh_marker_path = forbidden_marker_probe
            main._recover_from_interrupted_install()
            main.cmd_update(object())
            print("QA_UPDATE_GUARD_OK", flush=True)
            print("MAIN_IMPORTED_OK", flush=True)
            """
        ),
        encoding="utf-8",
    ...
    assert "EARLY_RECOVERY_CALLED" not in result.stdout
    assert "PROJECT_ENV=None" in result.stdout
    assert "QA_UPDATE_GUARD_OK" in result.stdout
    assert "Hermes update is disabled in Breadboard QA isolation." in result.stdout
    assert "MAIN_IMPORTED_OK" in result.stdout
...
def test_qa_isolation_never_probes_or_repairs_shared_checkout(tmp_path, monkeypatch):
    root = _project(tmp_path)
    (root / ".update-incomplete").write_text("x", encoding="utf-8")
    probed = []
    installs = []
    monkeypatch.setenv("HERMES_QA_ISOLATED", "1")
    monkeypatch.setattr(er, "_probe_broken_packages", lambda: probed.append(1) or ["PyYAML"])
    monkeypatch.setattr(
        er, "_run_repair_install", lambda specs, project: installs.append((specs, project)) or True
    )
