...
class TestEnsureHermesHome:
    ...
    def test_upgrades_untouched_hermes_agent_identity_to_bread(self, tmp_path):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            soul_path = tmp_path / "SOUL.md"
            soul_path.write_text(LEGACY_HERMES_AGENT_SOUL_MD, encoding="utf-8")
            ensure_hermes_home()
            assert soul_path.read_text(encoding="utf-8") == DEFAULT_SOUL_MD
            assert DEFAULT_SOUL_MD.startswith("You are Bread, the Breadboard assistant.")
