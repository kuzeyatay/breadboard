...
def test_desktop_runtime_environment_is_not_overridden_by_dotenv(
    ...
    runtime_values = {
        ...
        "HERMES_DASHBOARD_SESSION_TOKEN": "runtime-session-token",
        "BREADBOARD_INTERNAL_URL": "http://127.0.0.1:43100",
        "BREADBOARD_HERMES_TOOL_SECRET": "runtime-tool-secret",
    }
    dotenv_values = {
        name: f"dotenv-{index}" for index, name in enumerate(runtime_values)
    }
    env_file.write_text(
        "".join(f"{name}={value}\n" for name, value in dotenv_values.items())
        + "OPENAI_API_KEY=provider-key-from-dotenv\n",
        encoding="utf-8",
    )
    for name, value in runtime_values.items():
        monkeypatch.setenv(name, value)
    monkeypatch.setenv("OPENAI_API_KEY", "stale-provider-key")

    loaded = load_hermes_dotenv(hermes_home=home)

    assert loaded == [env_file]
    assert {
        name: os.environ.get(name) for name in runtime_values
    } == runtime_values
    assert os.environ["OPENAI_API_KEY"] == "provider-key-from-dotenv"


def test_desktop_dotenv_cannot_introduce_runtime_environment(
    tmp_path, monkeypatch
):
    home = tmp_path / "hermes"
    home.mkdir()
    ...
    assert loaded == [env_file]
    assert os.environ["HERMES_DESKTOP"] == "1"
    assert all(name not in os.environ for name in runtime_names)
...
def test_qa_isolation_ignores_and_does_not_sanitize_project_env(tmp_path, monkeypatch):
    home = tmp_path / "hermes"
    project_env = tmp_path / ".env"
    original = "OPENAI_BASE_URL=https://project.example/v1\n"
    project_env.write_text(original, encoding="utf-8")
    monkeypatch.setenv("HERMES_QA_ISOLATED", "1")
    monkeypatch.setenv("OPENAI_BASE_URL", "https://controlled.example/v1")

    loaded = load_hermes_dotenv(hermes_home=home, project_env=project_env)

    assert loaded == []
    assert os.getenv("OPENAI_BASE_URL") == "https://controlled.example/v1"
