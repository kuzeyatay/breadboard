...
class TestWebServerEndpoints:
    ...
    def test_update_hermes_is_disabled_in_qa_without_spawning(self, monkeypatch):
        import hermes_cli.web_server as web_server

        def fail_spawn(*_args, **_kwargs):
            raise AssertionError("QA update guard should not spawn hermes update")
        ...
        assert data["error"] == "qa_isolation"
    ...
    def test_ops_import_rejects_external_archive_in_qa(
        ...
        def fail_spawn(*_args, **_kwargs):
            raise AssertionError("QA import guard should not spawn")

        monkeypatch.setenv("HERMES_QA_ISOLATED", "1")
        monkeypatch.setenv("BREADBOARD_QA_RUN_DIR", str(run_root))
        monkeypatch.setattr(ws, "_spawn_hermes_action", fail_spawn)

        resp = self.client.post(
            "/api/ops/import",
            json={"archive": str(outside), "force": True},
        )

        assert resp.status_code == 403

    def test_ops_backup_rejects_custom_path_in_qa_without_spawning(
        self, tmp_path, monkeypatch
    ):
        import hermes_cli.web_server as ws

        def fail_spawn(*_args, **_kwargs):
            raise AssertionError("QA backup path guard should not spawn")
        ...
        resp = self.client.post(
            "/api/ops/backup",
            json={"output": str(tmp_path / "outside.zip")},
        )
