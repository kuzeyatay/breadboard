"""Tests for tools/tool_result_storage.py -- layered tool result persistence.

Layer 0 (tokenjuice compression) now sits in front of persistence and handles
most oversized results before they reach it. These tests are about the
persistence layers underneath, so they run with compression off; the
compression stage and its precedence over persistence are covered in
tests/tools/test_tokenjuice.py.
"""
...
def compression_off(monkeypatch):
    """Exercise the persistence layers directly, without layer 0 in the way."""
    import tools.tool_result_storage as storage

    monkeypatch.setattr(storage, "_try_compress", lambda *a, **k: None)
