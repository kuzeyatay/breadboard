...
def test_pending_gateway_approval_returns_a_non_owning_snapshot():
    original = {
        "command": "du -sh .",
        "description": "Measure the workspace size",
        "pattern_key": "plugin_rule:terminal:disk-usage",
    }
    approval._gateway_queues["test-session"] = [approval._ApprovalEntry(original)]

    snapshot = approval.get_pending_gateway_approval("test-session")

    assert snapshot == original
    assert snapshot is not original
    snapshot["command"] = "changed by status transport"
    assert approval.get_pending_gateway_approval("test-session")["command"] == "du -sh ."
    assert approval.has_blocking_approval("test-session") is True
...
class TestRequestToolApproval:
    ...
    def test_exact_display_target_is_sent_to_prompt(self, monkeypatch):
        monkeypatch.setattr(approval, "_is_interactive_cli", lambda: True)
        monkeypatch.setattr(approval, "_is_gateway_approval_context", lambda: False)
        seen = {}

        def prompt(target, description, **kwargs):
            seen.update(target=target, description=description, kwargs=kwargs)
            return "once"
        ...
        res = request_tool_approval(
            ...
            "Run this command?",
            rule_key="exact-command",
            display_target="du -sh .",
        )
        assert res["approved"] is True
        assert seen["target"] == "du -sh ."
