...
class TestCheckWebApiKey:
    ...
    def test_no_credentials_fails(self, monkeypatch):
        ...
        # This test is about the credential probes, and the bundled keyless
        # `fetch` extract provider is a separate, legitimate reason for the web
        # tools to light up — so silence the registry to isolate what is under
        # test rather than assert something that is no longer true.
        from agent import web_search_registry

        monkeypatch.setattr(
            web_search_registry, "get_active_search_provider", lambda: None
        )
        monkeypatch.setattr(
            web_search_registry, "get_active_extract_provider", lambda: None
        )
