...
def _forward_compute_host_rpc(message: dict) -> None:
    """Mirror isolated turn lifecycle into the serving session before forwarding.

    The compute host owns the live agent, but ``session.turn_result`` is served
    by this process. Keeping its compact tool journal and terminal payload here
    makes a dropped WebSocket frame recoverable instead of erasing the evidence
    that the turn actually gathered.
    """
    try:
        params = message.get("params") if message.get("method") == "event" else None
        if isinstance(params, dict):
            ...
            if session is not None and isinstance(payload, dict):
                if event_type == "tool.complete":
                    _record_completed_tool(session, payload)
...
def _compute_host_turn_frame(
    ...
    frame = {
        ...
        "ephemeral_system_prompt_override": session.get(
            "ephemeral_system_prompt_override"
        ),
...
def _on_compute_host_turn_done(
    ...
    if is_error:
        ...
        if client_turn_id:
            payload["turn_id"] = client_turn_id
        _remember_turn_result(session, client_turn_id, payload)
...
def _submit_prompt_to_compute_host(
    ...
    frame = _compute_host_turn_frame(rid, sid, session, text, client_turn_id)
    ...
    def _complete(done: dict) -> None:
        ...
        if done.get("reason") == "send_failed":
            return
        _on_compute_host_turn_done(rid, sid, session, done, client_turn_id)
...
def _approval_event_payload(data: dict | None) -> dict:
    """Build the safe, client-facing form of one approval request."""
    ...
    if not payload.get("request_id") and payload.get("pattern_key"):
        payload["request_id"] = str(payload["pattern_key"])
    return payload
...
def _emit_approval_request(sid: str, data: dict | None) -> None:
    ...
    platforms and the SSE/API stream fixed in #50767). Reuse Hermes' authoritative
    forced-redaction seam so all approval transports redact consistently."""
    payload = _approval_event_payload(data)
    ...
    def _build() -> None:
        ...
        try:
            ...
            try:
                ...
                if "enabled_toolsets_override" in current:
                    kw["enabled_toolsets_override"] = list(
                        current.get("enabled_toolsets_override") or []
                    )
                if "ephemeral_system_prompt_override" in current:
                    kw["ephemeral_system_prompt_override"] = str(
                        current.get("ephemeral_system_prompt_override") or ""
                    )
...
_BREADBOARD_REQUIRED_TOOLSETS = frozenset({"breadboard", "web"})
_BREADBOARD_REQUIRED_TOOLS_BY_TOOLSET = {
    # Breadboard's deterministic planner advertises web research whenever this
    # toolset is pinned. A provider check can otherwise silently remove the
    # schema, leaving policy and runtime in contradictory states.
    "web": frozenset({"web_search"}),
}


def _missing_breadboard_required_toolsets(
    enabled_toolsets: list[str] | None,
) -> list[str]:
    return sorted(_BREADBOARD_REQUIRED_TOOLSETS - set(enabled_toolsets or ()))


def _missing_breadboard_required_tools(
    enabled_toolsets: list[str] | None,
) -> list[str]:
    required: set[str] = set()
    for toolset in enabled_toolsets or []:
        required.update(_BREADBOARD_REQUIRED_TOOLS_BY_TOOLSET.get(toolset, ()))
    if not required:
        return []

    # Provider plugins contribute the availability checks used by
    # get_tool_definitions. Discovery is idempotent and normally already warm at
    # this point, but doing it here makes the session contract self-contained.
    try:
        from hermes_cli.plugins import discover_plugins
...
def _turn_usage_delta(before: dict, after: dict) -> dict:
    """Return additive usage for one prompt while preserving context gauges.

    ``_get_usage`` is deliberately a lifetime session snapshot for status bars.
    Attaching that snapshot to every ``message.complete`` row made a late answer
    look as though it alone consumed the whole session. Session counters can be
    reset by model/session switches, so a lower post-turn value is treated as a
    fresh counter instead of producing a negative delta.
    """
    usage = {"scope": "turn", "model": str(after.get("model") or "")}
    for key in _TURN_USAGE_COUNTERS:
        prior = before.get(key, 0)
        current = after.get(key, 0)
        prior = prior if isinstance(prior, (int, float)) and prior >= 0 else 0
        current = current if isinstance(current, (int, float)) and current >= 0 else 0
        usage[key] = int(current - prior if current >= prior else current)

    # These are final-state gauges, not additive counters. Copying them lets the
    # UI explain the difference between cumulative processing and current
    # context occupancy without inventing a meaningless context delta.
    for key in (
        "context_used",
        "context_max",
        "context_percent",
...
def _tool_completion_succeeded(payload: dict) -> bool:
    ...
    if isinstance(result, dict):
        if result.get("timedOut") is True:
            return False
        result_exit_code = result.get("exitCode")
        if isinstance(result_exit_code, (int, float)) and result_exit_code != 0:
            return False
...
def _record_completed_tool(session: dict, payload: dict) -> None:
    """Keep a bounded journal for terminal turn recovery."""
    tool_call_id = str(payload.get("tool_id") or payload.get("tool_call_id") or "").strip()
    name = str(payload.get("name") or payload.get("tool_name") or "tool").strip() or "tool"
    if not tool_call_id:
        return
    item = {
        "tool_id": tool_call_id[:500],
        "name": name[:500],
        "success": _tool_completion_succeeded(payload),
    }
    summary = str(payload.get("summary") or payload.get("error") or "").strip()
    if summary:
        item["summary"] = summary[:2000]
    if payload.get("location"):
        item["location"] = str(payload["location"])[:2000]
    if isinstance(payload.get("args"), dict):
        item["args"] = payload["args"]
        if not item.get("location"):
            query = payload["args"].get("query")
            url = payload["args"].get("url")
            if isinstance(query, str) and query.strip():
                item["location"] = query.strip()[:2000]
            elif isinstance(url, str) and url.strip():
                item["location"] = url.strip()[:2000]
    if payload.get("websites"):
        item["websites"] = payload["websites"]
    elif payload.get("result"):
        item["result"] = payload["result"]
    lock = session.get("history_lock")
    if lock is None:
        tools = session.setdefault("completed_tools", [])
        tools[:] = [tool for tool in tools if tool.get("tool_id") != item["tool_id"]]
        tools.append(item)
        del tools[:-256]
        return
    with lock:
...
def _make_agent(
    ...
    ephemeral_system_prompt_override: str | None = None,
):
    ...
    system_prompt = (
        ephemeral_system_prompt_override
        if ephemeral_system_prompt_override is not None
...
def _remember_turn_result(
    session: dict,
    client_turn_id: str | None,
    payload: dict,
) -> None:
    """Keep the terminal frame queryable after its one-shot transport emit.

    Breadboard supplies a durable per-turn id. The live event remains the fast
    path, while this record lets a reconnect recover the exact terminal answer
    without rerunning tools or guessing from conversational memory.
    """
    if not client_turn_id:
        return
    with session["history_lock"]:
        # An isolated compute host may already have attached its complete tool
        # journal to the terminal frame. Merge that with anything observed by
        # the serving process so a partially dropped RPC stream cannot replace
        # the host's full journal with a partial local one.
        completed_tools_by_id: dict[str, dict] = {}
        for source in (
            payload.get("completed_tools", []),
            session.get("completed_tools", []),
        ):
            ...
            for tool in source:
                ...
                if tool_id:
                    completed_tools_by_id[tool_id] = dict(tool)
        completed_tools = list(completed_tools_by_id.values())[-256:]
        if completed_tools:
            # Mutate the caller's terminal payload too: the live frame and the
            # correlated recovery response then carry the same durable facts.
            payload["completed_tools"] = completed_tools
        result = {
            "turn_id": client_turn_id,
            "payload": dict(payload),
            "completed_at": time.time(),
        }
        session["last_turn_result"] = result
        if session.get("active_client_turn_id") == client_turn_id:
            session["active_client_turn_id"] = None
...
def _(rid, params: dict) -> dict:
    ...
    # Breadboard owns capability policy and canonical context. Its authenticated
    # loopback adapter therefore needs a per-session, immutable toolset boundary
    # and an ephemeral system prompt without mutating Hermes's global config.
    # Keep these extensions source-gated so existing Hermes clients retain their
    # historical behavior.
    enabled_toolsets_override = None
    ephemeral_system_prompt_override = None
    if source == "breadboard":
        if "enabled_toolsets" in params:
            requested_toolsets = params.get("enabled_toolsets")
            if not isinstance(requested_toolsets, list):
                return _err(rid, 4004, "enabled_toolsets must be a list")
            enabled_toolsets_override = []
            for value in requested_toolsets:
                ...
                if (
                    ...
                    return _err(rid, 4004, "enabled_toolsets contains an invalid name")
                enabled_toolsets_override.append(name)
            if len(enabled_toolsets_override) > 32:
                return _err(rid, 4004, "enabled_toolsets contains too many entries")
        if "system_prompt" in params:
            ephemeral_system_prompt_override = str(
                params.get("system_prompt") or ""
            )
            if len(ephemeral_system_prompt_override) > 262_144:
                return _err(rid, 4004, "system_prompt is too large")
        missing_required_toolsets = _missing_breadboard_required_toolsets(
            enabled_toolsets_override
        )
        if missing_required_toolsets:
            return _err(
                rid,
                5036,
                "Breadboard session contract is incomplete; required toolsets "
                "are missing: " + ", ".join(missing_required_toolsets) + ".",
            )
        missing_required_tools = _missing_breadboard_required_tools(
            enabled_toolsets_override
        )
        if missing_required_tools:
            return _err(
                rid,
                5036,
                "Breadboard required tools are unavailable: "
                + ", ".join(missing_required_tools)
                + ". Sync the pinned Hermes runtime dependencies or configure "
                "a working provider before creating a session.",
            )
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "show_reasoning": _load_show_reasoning(),
            "source": source,
            "breadboard_contract_validated": source == "breadboard",
            "slash_worker": None,
            "tool_progress_mode": _load_tool_progress_mode(),
            "tool_started_at": {},
            "transport": current_transport() or _stdio_transport,
        ...
        if enabled_toolsets_override is not None:
            _sessions[sid]["enabled_toolsets_override"] = enabled_toolsets_override
        if ephemeral_system_prompt_override is not None:
            _sessions[sid][
                "ephemeral_system_prompt_override"
            ] = ephemeral_system_prompt_override
...
@method("session.turn_result")
def _(rid, params: dict) -> dict:
    """Return the terminal result for one Breadboard-correlated turn.

    Results are first read from the live session's exactly-matching turn cache.
    The durable transcript is a recovery fallback for a frame emitted before
    this RPC existed or before the gateway restarted. That fallback is accepted
    only when the expected prompt is the latest user turn, preventing an older
    assistant answer from completing a newer destructive request.
    """
    turn_id = str(params.get("turn_id") or "").strip()
    if not turn_id or len(turn_id) > 200:
        return _err(rid, 4004, "turn_id is required")

    sid = str(params.get("session_id") or "")
    session = _sessions.get(sid)
    if session is not None:
        with session["history_lock"]:
            cached = session.get("last_turn_result")
            running = bool(session.get("running"))
            active_turn_id = str(session.get("active_client_turn_id") or "")
            inflight = dict(session.get("inflight_turn") or {})
        if isinstance(cached, dict) and cached.get("turn_id") == turn_id:
            payload = dict(cached.get("payload") or {})
            status = str(payload.get("status") or "complete").lower()
            state = (
                "aborted"
                if status in {"interrupted", "aborted", "cancelled"}
                else "failed"
        ...
        if running and (not active_turn_id or active_turn_id == turn_id):
                    ...
                    if approval is not None:
                        return _ok(
                            rid,
                            {
                                ...
                                "turn_id": turn_id,
                                "approval": _approval_event_payload(approval),
                            },
                        )
                return _ok(rid, {"state": "running", "turn_id": turn_id})

    expected = str(params.get("expected_user_text") or "").strip()
    if not expected:
        return _ok(rid, {"state": "unknown", "turn_id": turn_id})
    ...
    if session is not None:
        with session["history_lock"]:
            history = list(session.get("history", []))
        stored_session_id = str(session.get("session_key") or stored_session_id)
        if stored_session_id:
            with _session_db(session) as db:
                if db is not None:
                    try:
                        history = db.get_messages_as_conversation(
                            stored_session_id, include_ancestors=True
                        )
                    except Exception:
                        pass
    ...
    if latest_user_index is None:
        return _ok(rid, {"state": "unknown", "turn_id": turn_id})
    latest_user = str(messages[latest_user_index].get("text") or "").strip()
    if latest_user != expected:
        return _ok(rid, {"state": "unknown", "turn_id": turn_id})

    # A transcript ending in an assistant tool request or its tool result is a
    # protocol-incomplete turn, not a completed answer. This is the durable
    # shape left behind when the gateway is restarted between a tool call and
    # the model's follow-up response. Do not promote an earlier narration such
    # as "I'm building it now" into a successful terminal result.
    latest_raw_user_index = next(
        (
            index
            for index in range(len(history) - 1, -1, -1)
            if isinstance(history[index], dict)
            and history[index].get("role") == "user"
            and not _is_display_hidden_marker(
                "user", _coerce_message_text(history[index].get("content"))
            )
        ),
        None,
    )
    protocol_tail = None
    if latest_raw_user_index is not None:
        protocol_tail = next(
            (
                message
                for message in reversed(history[latest_raw_user_index + 1 :])
                if isinstance(message, dict)
                and message.get("role") in {"assistant", "tool"}
            ),
            None,
        )
    if isinstance(protocol_tail, dict) and (
        protocol_tail.get("role") == "tool"
        or (
            protocol_tail.get("role") == "assistant"
            and bool(protocol_tail.get("tool_calls"))
        )
    ):
        return _ok(
            rid,
...
def _(rid, params: dict) -> dict:
    ...
    raw_client_turn_id = params.get("client_turn_id")
    client_turn_id = None
    if raw_client_turn_id is not None:
        if not isinstance(raw_client_turn_id, str):
            return _err(rid, 4004, "client_turn_id must be a string")
        client_turn_id = raw_client_turn_id.strip()
        if (
            not client_turn_id
            or len(client_turn_id) > 200
            or not re.fullmatch(r"[A-Za-z0-9_.:-]+", client_turn_id)
        ):
            return _err(rid, 4004, "client_turn_id is invalid")
    ...
    if err:
        return err
    if (
        _session_source(session) == "breadboard"
        and not session.get("breadboard_contract_validated")
    ):
        ...
        if missing_required_tools:
            return _err(
                ...
                + ", ".join(missing_required_tools)
                + ". Sync the pinned Hermes runtime dependencies or configure "
                "a working provider before submitting a prompt.",
            )
        session["breadboard_contract_validated"] = True
    ...
    if (t := current_transport()) is not None:
        session["transport"] = t
    if _session_source(session) == "breadboard" and "system_prompt" in params:
        system_prompt = str(params.get("system_prompt") or "")
        if len(system_prompt) > 262_144:
            return _err(rid, 4004, "system_prompt is too large")
        session["ephemeral_system_prompt_override"] = system_prompt
        if session.get("agent") is not None:
            session["agent"].ephemeral_system_prompt = system_prompt or None
    while True:
        busy_transport = None
        with session["history_lock"]:
            if session.get("running"):
    ...
    if turn_isolation:
        isolated_response = _submit_prompt_to_compute_host(
            rid, sid, session, text, client_turn_id
        )
...
def _run_prompt_submit(
    ...
    _emit(
        "message.start",
        sid,
        {"turn_id": client_turn_id} if client_turn_id else None,
    )
    ...
    def run():
        ...
        try:
            ...
            payload = {
                "text": raw,
                "usage": _turn_usage_delta(usage_before, usage_after),
                "status": status,
            }
            if client_turn_id:
                payload["turn_id"] = client_turn_id
            ...
            _remember_turn_result(
                ...
                client_turn_id,
                {
                    "text": f"Error: {e}",
                    "status": "error",
                    **({"turn_id": client_turn_id} if client_turn_id else {}),
                },
            )
