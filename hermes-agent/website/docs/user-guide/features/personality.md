...
If the file is empty, whitespace-only, or cannot be read, Hermes falls back to Bread's built-in identity ("You are Bread, the Breadboard assistant..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
