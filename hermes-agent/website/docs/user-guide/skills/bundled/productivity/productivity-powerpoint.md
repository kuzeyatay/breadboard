...
| Version | `2.1.0` |
| Author | Anthropic and Onyx (adapted by Nous Research) |
...
Create, read, and edit PowerPoint decks — from-scratch generation with pptxgenjs, template-based editing via direct XML manipulation, speaker notes, charts, and design QA. A `.pptx` is a ZIP archive of XML files. This private-use copy incorporates Onyx's component, chart, icon, lint, preview, and safe pack/unpack workflow; provenance is recorded in `NOTICE.txt`.
...
No API key or hosted model is required. Maintainers package the terminal workflow with these pinned local dependencies:

Node: `pptxgenjs@4.0.1`, `lucide-static@1.24.0`, `@tabler/icons@3.44.0`, `sharp@0.35.3`.

Python: the exact local packages in `requirements.txt`, including `python-pptx`, `matplotlib`, Pillow, `defusedxml`, and `lxml`.

LibreOffice (`soffice`) is required for rendering and visual QA; Poppler (`pdftoppm` and `pdfimages`) is required for PDF/image conversion. Linux: install `libreoffice poppler-utils fontconfig`. macOS: `brew install libreoffice poppler fontconfig`. Windows: install LibreOffice and Poppler and put their executables on `PATH`. Dependency installation may need package-registry access, but never credentials or a paid API key.

Never run those install commands mid-turn. If a packaged dependency is unavailable, use the `office_run` path when present or deliver the valid deck without the optional chart/icon/render pass and say which QA step was skipped.

## Quick Reference

| Task | Approach |
|---|---|
| **Create** a new deck | Prefer the tested component library in [components.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/productivity\powerpoint/components.md); use raw [pptxgenjs.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/productivity\powerpoint/pptxgenjs.md) only for layouts it does not cover |
| **Charts** | Use real data with the deck-styled helper in [charts.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/productivity\powerpoint/charts.md) |
| **Edit** an existing deck, or build from a template | Follow [editing.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/productivity\powerpoint/editing.md): unpack → edit → clean → pack |
| **Read** content | `python "${HERMES_SKILL_DIR}/scripts/extract_text.py" deck.pptx`; visual grid: `python "${HERMES_SKILL_DIR}/scripts/thumbnail.py" deck.pptx` |

## How to Run

Choose the execution path from the tools actually available:

- **Breadboard/Garden Chat:** use `office_run` for document creation and edits, and call `office_export` once after the final edit. Keep every path relative to the active conversation document workspace. The imported component and Python/Node helpers require terminal execution, so use their design and QA guidance but do not pretend those scripts ran.
- **Terminal or sandbox session:** use the imported local scripts. Hermes reports this skill's absolute directory when it loads; resolve every relative guide or script path against that directory, keep the session workspace as the current directory, and place generated files in `outputs/`.

Do not mix the two filesystems in one deck run. A file created in the terminal workspace is not automatically present in OfficeCLI's conversation workspace, and `office_export` only publishes files from the latter.

For the terminal path, first run `python "${HERMES_SKILL_DIR}/scripts/preflight.py"`. It is read-only, keyless, and offline. Use only capabilities reported `true`; if a capability is missing, fall back to OfficeCLI or skip that optional enhancement and disclose it.

Before asset work, save a complete text-viable deck to `outputs/<name>.pptx`. Enhance it in bounded passes, re-saving after each pass. If time runs short, ship the last valid deck rather than leaving work only in an unpacked directory.

Check the user's workspace for uploaded `.pptx`/`.potx` templates first. The skill also includes these local, keyless templates:

- `templates/presentation-template.pptx`
- `templates/pitch-deck.pptx`
- `templates/all-hands-template.pptx`

If more than one suitable template exists, show the choices and ask which to use. If none fits, build from scratch with [components.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/productivity\powerpoint/components.md).
...
| `scripts/preflight.py` | Report local Python, Node, LibreOffice, and Poppler capabilities without installing or contacting anything |
| `scripts/extract_text.py deck.pptx [--output deck.md]` | Safely extract ordered slide text and speaker notes without a hosted converter |
...
| `scripts/office/unpack.py deck.pptx outputs/unpacked/` | Safely unpack and pretty-print Office XML for editing |
| `scripts/office/pack.py outputs/unpacked/ outputs/deck.pptx --original source.pptx` | Condense, validate, and rebuild the deck |
...
| `scripts/lint.py deck.pptx [--profile standard\|dense]` | Deterministic bounds, margin, overflow, overlap, contrast, and font QA |
| `scripts/preview.py deck.pptx outputs/rendered` | Render slide images through LibreOffice and Poppler, reusing a fresh cache |
| `scripts/chart.py` | Matplotlib theme helpers for deck-styled chart PNGs |
| `scripts/icon.js <name> --color <hex> -o outputs/icons/name.png` | Render local Lucide/Tabler icons; no image API |
| `scripts/components_demo.js outputs/components-demo.pptx` | Smoke-test the imported component library |
...
- **Default to deck-styled chart PNGs** with [charts.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/productivity\powerpoint/charts.md) for consistent rendering. Use `addChart()` only when the user explicitly needs editable native PowerPoint charts. For native charts, set the title, labels, palette, axes, grid, and legend deliberately.
...
- **After `writeFile()`, run `python "${HERMES_SKILL_DIR}/scripts/office/validate.py" deck.pptx`.** It reports the two chart faults above and the slide-XML defects PowerPoint refuses, and names the fix for each. Fix them in your generator, not by hand-editing the packed XML.
...
- **Icons:** render a local Lucide or Tabler icon with `node "${HERMES_SKILL_DIR}/scripts/icon.js" <name> --color <hex> -o outputs/icons/name.png`, then insert the PNG. This uses no image API.

## Editing existing decks and templates

Pick layouts first: `python "${HERMES_SKILL_DIR}/scripts/thumbnail.py" template.pptx template-thumbs` writes a labeled grid of every slide and prints the file(s) it created — `template-thumbs.jpg`, split into `template-thumbs-N.jpg` past 12 slides. **Always pass that second argument, named after the deck.** It defaults to `thumbnails`, so two decks thumbnailed in one directory silently overwrite each other's grids (template analysis only — visual QA needs the full-resolution renders from [Converting to Images](#converting-to-images); it only accepts `.pptx`, so copy a `.potx` to a `.pptx` name first). Use it with `scripts/extract_text.py` to map each content section onto a template slide, and vary the layouts — don't put every section on the same title-and-bullets slide.

```text
python "${HERMES_SKILL_DIR}/scripts/office/unpack.py" deck.pptx unpacked
python "${HERMES_SKILL_DIR}/scripts/add_slide.py" unpacked slide2.xml --after slide2.xml
# reorder/delete slides by editing <p:sldIdLst>, then edit slide XML
python "${HERMES_SKILL_DIR}/scripts/clean.py" unpacked
python "${HERMES_SKILL_DIR}/scripts/office/pack.py" unpacked out.pptx --original deck.pptx
python "${HERMES_SKILL_DIR}/scripts/office/validate.py" out.pptx --original deck.pptx
```
...
- Legacy `.ppt` must be converted first: `python "${HERMES_SKILL_DIR}/scripts/office/soffice.py" --headless --convert-to pptx file.ppt`. `.potx` templates unpack and pack identically — keep the `.potx` extension on the output.
...
```text
python "${HERMES_SKILL_DIR}/scripts/extract_text.py" output.pptx
```

Check for missing content, typos, wrong order.

When using templates, scan the extracted text for placeholder patterns such as `xxx`, `lorem`, `ipsum`, `TODO`, `[insert`, or `this ... slide/page ... layout`. Fix every match before declaring success.

### File QA (required)

```bash
python "${HERMES_SKILL_DIR}/scripts/office/validate.py" output.pptx                      # built from scratch
python "${HERMES_SKILL_DIR}/scripts/office/validate.py" output.pptx --original src.pptx  # built from a template
```
...
Convert the slides to images (see [Converting to Images](#converting-to-images)) and inspect every rendered slide with the available image-analysis tool. After staring at the generating code you tend to see what you expect rather than what rendered, so look at the images fresh. User-visible defects to look for:
...
```text
python "${HERMES_SKILL_DIR}/scripts/preview.py" output.pptx outputs/output-preview
```

The helper prints absolute paths for the rendered slide JPEGs and invalidates its cache when the `.pptx` changes. Pass those paths directly to the available image-analysis tool.

**After fixes, rerun the preview command** so the images reflect the edited `.pptx`.
