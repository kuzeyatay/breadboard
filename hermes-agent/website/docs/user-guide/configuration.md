...
terminal:
  ...
  docker_security_profile: standard # strict = keyless local isolation
...
**`terminal.docker_security_profile: strict`** enables a keyless, fail-closed
profile for untrusted local code. It does not add a model provider, service, or
API-key requirement; Docker is the only prerequisite. Strict mode enforces all
of the following regardless of weaker sibling settings in the same config:

- a non-root UID/GID, `--cap-drop ALL`, `no-new-privileges`, and a read-only
  image filesystem;
- `--network=none` with no forwarded or literal environment variables;
- no automatic credential, cache, or skill mounts, no `docker_volumes`, and no
  `docker_extra_args` overrides;
- only the explicitly opted-in launch directory may cross the host boundary,
  mounted at `/workspace` with `docker_mount_cwd_to_workspace: true`;
- an ephemeral container (no cross-process reuse), at most 2 CPUs, 2 GB memory,
  128 processes, a 120-second command ceiling, and size-bounded writable tmpfs
  directories.

Strict mode refuses to start when the Docker host cannot enforce cgroup CPU,
memory, and PID limits, or when the selected image uses a root-dependent s6
entrypoint. Because egress and package-manager downloads are unavailable, use a
plain execution image that already contains the dependencies you need.
...
terminal:
  ...
  docker_mount_cwd_to_workspace: true  # optional: expose only this project
```
...
The long-lived lifecycle below describes the standard profile. Strict
containers also carry `hermes-security-profile=strict` and are never reused
across Hermes processes.
