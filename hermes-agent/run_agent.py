...
class AIAgent:
    ...
    def _model_supports_vision(self) -> bool:
        ...
        misclassified as non-vision and have their images stripped — under
        ``agent.image_input_mode: native`` that unknown resolves to True
        instead, so forced-native pixels actually reach the wire and the
        provider gets the last word (an explicit ``supports_vision: false``
        override still strips).
        """
        try:
            ...
            if supports is None:
                # decide_image_input_mode returns "native" for unknown
                # capability only when the mode is forced in config.
