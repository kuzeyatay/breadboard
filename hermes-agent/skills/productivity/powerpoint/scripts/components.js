...
// overflow. pptxgenjs is resolved from local Node modules first, with a
// globally provisioned module directory retained as a compatibility fallback.
"use strict";
...
// Fonts commonly available in supported environments (keep in sync with the
// skill's typography guidance).
...
function requirePptxgen() {
  ...
  try {
    ...
    throw new Error(
      "Packaged pptxgenjs dependency is missing. Run scripts/preflight.py " +
        "and use the OfficeCLI fallback; do not install packages during a deck run."
    );
  }
