...
def _safe_members(zf: zipfile.ZipFile, output_path: Path) -> list[zipfile.ZipInfo]:
    """Reject archive entries that could escape or link outside the output."""
    ...
    for member in infos:
        ...
        if (
            ...
            raise ValueError(f"archive contains an absolute path: {member.filename}")
        path_key = raw.rstrip("/").casefold()
        ...
        try:
            ...
            raise ValueError(f"archive path escapes output directory: {member.filename}") from exc
        # ZIP stores Unix file type in the high 16 external-attribute bits.
...
def unpack(
    ...
    try:
        ...
        if output_path.exists() and any(output_path.iterdir()):
            return None, f"Error: {output_directory} must be empty or absent"
        ...
        with tempfile.TemporaryDirectory(
            ...
            with zipfile.ZipFile(input_path, "r") as zf:
                members = _safe_members(zf, staged)
                zf.extractall(staged, members=members)
