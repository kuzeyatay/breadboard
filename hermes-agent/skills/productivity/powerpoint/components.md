...
> **Path convention**: All commands run from the **session workspace**. Resolve every `scripts/...` path against the absolute PowerPoint skill directory Hermes reports when this skill loads. Generated files go in `outputs/`.
...
```javascript
const C = require("<absolute-powerpoint-skill-directory>/scripts/components.js");
...
//   node <absolute-powerpoint-skill-directory>/scripts/icon.js trending-up --color 0F0F0F -o outputs/icons/up.png
...
```bash
node <absolute-powerpoint-skill-directory>/scripts/components_demo.js outputs/components-demo.pptx
python <absolute-powerpoint-skill-directory>/scripts/lint.py outputs/components-demo.pptx
```
