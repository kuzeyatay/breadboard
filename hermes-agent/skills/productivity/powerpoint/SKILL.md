...
| Task | Approach |
|---|---|
| **Create** a new deck | Prefer the tested component library in [components.md](components.md); use raw [pptxgenjs.md](pptxgenjs.md) only for layouts it does not cover |
| **Charts** | Use real data with the deck-styled helper in [charts.md](charts.md) |
| **Edit** an existing deck, or build from a template | Follow [editing.md](editing.md): unpack → edit → clean → pack |
...
If more than one suitable template exists, show the choices and ask which to use. If none fits, build from scratch with [components.md](components.md).
...
- **Default to deck-styled chart PNGs** with [charts.md](charts.md) for consistent rendering. Use `addChart()` only when the user explicitly needs editable native PowerPoint charts. For native charts, set the title, labels, palette, axes, grid, and legend deliberately.
