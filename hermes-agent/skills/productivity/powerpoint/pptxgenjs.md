...
Render icons with the bundled script. Its local dependencies (`lucide-static`, `@tabler/icons`, and `sharp`) are keyless packages listed in the skill prerequisites.

### Render an Icon PNG

```bash
node <absolute-powerpoint-skill-directory>/scripts/icon.js circle-check --color FFFFFF --size 512 -o outputs/icons/check.png
```
...
```bash
node <absolute-powerpoint-skill-directory>/scripts/icon.js --list "shield"               # search both sets
node <absolute-powerpoint-skill-directory>/scripts/icon.js --list "chart" --set lucide   # scope to one set
```
