...
1. Call `capability_search` with a concise query. Breadboard calls the official
   `/api/v1/skills/search` endpoint server-side and returns real skills.sh
   candidates.
...
4. If the user chooses a candidate, explain that Breadboard must run a separate
   explicit quarantine action. Quarantine retrieves the official immutable
   detail snapshot, hashes and inspects the downloaded files, and does not make
   the skill agent-accessible.
