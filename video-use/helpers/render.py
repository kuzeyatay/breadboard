...
def build_final_composite(
    ...
    if has_subs:
        # The path is a filter *option value*, where ffmpeg's own parser treats
        # ':' and '\' as syntax. Escaping the colon is not enough on Windows:
        # "C\:\Users\..." still loses every backslash, and the filtergraph fails
        # to initialize after the cut has already been extracted and joined.
        # Passing a bare filename and running ffmpeg from the subtitle's own
        # directory sidesteps the escaping entirely, on every platform.
        subs_name = subtitles_path.resolve().name.replace("'", r"\'")
        filter_parts.append(
            f"{current}subtitles='{subs_name}':force_style='{SUB_FORCE_STYLE}'[outv]"
        )
        out_label = "[outv]"
    else:
    ...
    # Run from the subtitle's directory so the bare filename above resolves.
    # Every other path in the command is absolute, so the cwd affects nothing else.
    subprocess.run(
        cmd,
        check=True,
        cwd=str(subtitles_path.resolve().parent) if has_subs else None,
