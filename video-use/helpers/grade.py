...
def _sample_frame_stats(
    ...
    try:
        # `metadata=print:file=` is a filter *option value*, where ffmpeg's own
        # parser treats ':' and '\' as syntax. A Windows path ("C:\Users\...")
        # is therefore unquotable without escaping, and the filtergraph fails to
        # initialize. Passing a bare filename and running ffmpeg from the file's
        # own directory sidesteps the escaping entirely, on every platform.
        ...
        subprocess.run(
            cmd, check=True, cwd=metadata_dir,
